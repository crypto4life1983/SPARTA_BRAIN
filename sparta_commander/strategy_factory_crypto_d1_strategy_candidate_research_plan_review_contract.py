...
proposed *research-plan review* into a contract which VALIDATES, on paper, whether
the external-bot-evidence research plan recorded by the Block 103 Crypto-D1
External Bot Evidence Research Plan Contract is acceptable before any later
research-only planning step. It consumes the stable constants of the Crypto-D1
External Bot Evidence Research Plan Contract (Block 103), the Strategy Candidate
...
(Block 95), and only when the upstream research-plan signal is clearly the
expected one
(crypto_d1_bot_evidence_research_plan_contract_active is True,
bot_evidence_research_plan_verdict == BOT_EVIDENCE_RESEARCH_PLAN_READY,
next_gate == CRYPTO_D1_STRATEGY_CANDIDATE_RESEARCH_PLAN_REVIEW_CONTRACT_REQUIRED,
mode == RESEARCH_ONLY, read_only is True, executes is False) evaluates a proposed
research-plan-review packet on paper and returns a deterministic verdict
describing whether the review is ready, needs more info, must be rejected, or is
parked.
...
The review preserves every external-bot-evidence safety stance recorded by the
research plan: external bot evidence stays observation-only and never becomes
...
promotion, no paper expansion, and no live activation is authorized.

Reaching a STRATEGY_CANDIDATE_RESEARCH_PLAN_REVIEW_READY verdict unlocks NOTHING
real. It only records, on paper, that a proposed research-plan review stays in
scope and confirms every stance -- and even that still requires a separate, later,
human step. Any upstream research-plan signal that is missing, malformed, not
READY, in the wrong mode, executable, non-read-only, or pointed at a different next
gate yields the AWAIT_CRYPTO_D1_STRATEGY_CANDIDATE_RESEARCH_PLAN_CONTRACT verdict.

Public API:
  - RESEARCH_PLAN_REVIEW_SCHEMA_VERSION
  - DEFAULT_RESEARCH_PLAN_REVIEW_LABEL
  - RESEARCH_PLAN_REVIEW_STATUS
  - RESEARCH_PLAN_REVIEW_SAFETY_POSTURE
  - RESEARCH_PLAN_REVIEW_STATE_ACTIVE
  - RESEARCH_PLAN_REVIEW_STATE_BLOCKED
  - RESEARCH_PLAN_REVIEW_VERDICT_READY
  - RESEARCH_PLAN_REVIEW_VERDICT_NEEDS_MORE_INFO
  - RESEARCH_PLAN_REVIEW_VERDICT_REJECTED
  - RESEARCH_PLAN_REVIEW_VERDICT_PARKED
  - RESEARCH_PLAN_REVIEW_VERDICT_AWAIT
  - ALLOWED_RESEARCH_PLAN_REVIEW_VERDICTS
  - UPSTREAM_REQUIRED_RESEARCH_PLAN_VERDICT
  - UPSTREAM_REQUIRED_RESEARCH_PLAN_NEXT_GATE
  - UPSTREAM_REQUIRED_RESEARCH_PLAN_MODE
  - RESEARCH_PLAN_REVIEW_NEXT_REQUIRED_ACTION
  - RESEARCH_PLAN_REVIEW_CURRENT_STAGE
  - DECISION_CRYPTO_D1_RESEARCH_PLAN_REVIEW_CONTRACT_REQUIRED
  - NEXT_GATE_CRYPTO_D1_RESEARCH_PLAN_REVIEW_READY
  - NEXT_GATE_CRYPTO_D1_RESEARCH_PLAN_REVIEW_FIX_REQUIRED
  - NEXT_GATE_CRYPTO_D1_RESEARCH_PLAN_REVIEW_PARKED
  - NEXT_GATE_CRYPTO_D1_RESEARCH_PLAN_REVIEW_REJECTED
  - NEXT_GATE_AWAIT_CRYPTO_D1_RESEARCH_PLAN_CONTRACT
  ...
  - REQUIRED_RESEARCH_PLAN_REVIEW_FIELDS
  - RESEARCH_PLAN_REVIEW_REQUIRED_TEXT_FIELDS
  - RESEARCH_PLAN_REVIEW_REQUIRED_AFFIRMATIONS
  - RESEARCH_PLAN_REVIEW_FORBIDDEN_ALLOW_FLAGS
  - ALLOWED_RESEARCH_PLAN_REVIEW_MODES
  - AUTOMATED_APPROVAL_MARKERS
  - REMAINING_REAL_WORLD_CAPABILITIES_BLOCKED
  - evaluate_crypto_d1_strategy_candidate_research_plan_review(packet)
  - build_crypto_d1_strategy_candidate_research_plan_review_contract(signal, packet=None)
  - validate_crypto_d1_strategy_candidate_research_plan_review_contract(contract)
  - render_crypto_d1_strategy_candidate_research_plan_review_contract_markdown(contract)
"""
...
from sparta_commander.strategy_factory_crypto_d1_strategy_candidate_research_plan_contract import (  # noqa: E501
    BOT_EVIDENCE_RESEARCH_PLAN_VERDICT_READY as _RESEARCH_PLAN_VERDICT_READY,
    BOT_EVIDENCE_RESEARCH_PLAN_SCHEMA_VERSION as _RESEARCH_PLAN_SCHEMA_VERSION,
    REQUIRED_MARKET_TYPE as _PLAN_REQUIRED_MARKET_TYPE,
    REQUIRED_TIMEFRAME as _PLAN_REQUIRED_TIMEFRAME,
    WATCHED_UNIVERSE as _PLAN_WATCHED_UNIVERSE,
    ASSET_EVIDENCE_STANCES as _PLAN_ASSET_EVIDENCE_STANCES,
    ASSET_EVIDENCE_STANCE_REASONS as _PLAN_ASSET_EVIDENCE_STANCE_REASONS,
    MIN_MULTI_ASSET_SYNC_SCORE as _PLAN_MIN_SYNC_SCORE,
    MIN_SYNCHRONIZED_ASSETS as _PLAN_MIN_SYNC_ASSETS,
    TRADE_COORDINATOR_RULES as _PLAN_TRADE_COORDINATOR_RULES,
    ALLOWED_BOT_STATE_VALUES as _PLAN_ALLOWED_BOT_STATE_VALUES,
    FORBIDDEN_BOT_STATE_VALUES as _PLAN_FORBIDDEN_BOT_STATE_VALUES,
)
...
__all__ = [
    ...
    "AUTOMATED_APPROVAL_MARKERS",
    "REMAINING_REAL_WORLD_CAPABILITIES_BLOCKED",
    "evaluate_crypto_d1_strategy_candidate_research_plan_review",
    "build_crypto_d1_strategy_candidate_research_plan_review_contract",
    "validate_crypto_d1_strategy_candidate_research_plan_review_contract",
    "render_crypto_d1_strategy_candidate_research_plan_review_contract_markdown",
]

RESEARCH_PLAN_REVIEW_SCHEMA_VERSION = (
    "strategy_factory_crypto_d1_strategy_candidate_research_plan_review_contract.v1"  # noqa: E501
)
DEFAULT_RESEARCH_PLAN_REVIEW_LABEL = (
    "Strategy Factory Crypto-D1 Strategy Candidate Research Plan Review Contract"
)
RESEARCH_PLAN_REVIEW_STATUS = (
    "READ_ONLY_CRYPTO_D1_STRATEGY_CANDIDATE_RESEARCH_PLAN_REVIEW_CONTRACT"
)
...
RESEARCH_PLAN_REVIEW_STATE_BLOCKED = (
    "CRYPTO_D1_STRATEGY_CANDIDATE_RESEARCH_PLAN_REVIEW_CONTRACT_BLOCKED"
)
...
RESEARCH_PLAN_REVIEW_VERDICT_NEEDS_MORE_INFO = (
    "STRATEGY_CANDIDATE_RESEARCH_PLAN_REVIEW_NEEDS_MORE_INFO"
)
RESEARCH_PLAN_REVIEW_VERDICT_REJECTED = (
    "STRATEGY_CANDIDATE_RESEARCH_PLAN_REVIEW_REJECTED"
)
RESEARCH_PLAN_REVIEW_VERDICT_PARKED = (
    "STRATEGY_CANDIDATE_RESEARCH_PLAN_REVIEW_PARKED"
)
RESEARCH_PLAN_REVIEW_VERDICT_AWAIT = (
    "AWAIT_CRYPTO_D1_STRATEGY_CANDIDATE_RESEARCH_PLAN_CONTRACT"
)

ALLOWED_RESEARCH_PLAN_REVIEW_VERDICTS: tuple[str, ...] = (
    RESEARCH_PLAN_REVIEW_VERDICT_READY,
    RESEARCH_PLAN_REVIEW_VERDICT_NEEDS_MORE_INFO,
    RESEARCH_PLAN_REVIEW_VERDICT_REJECTED,
    RESEARCH_PLAN_REVIEW_VERDICT_PARKED,
...
# The exact upstream research-plan signal this contract activates from (Block
# 103). The contract only becomes active when the upstream research-plan contract
# is clearly READY, in RESEARCH_ONLY mode, read-only, non-executing, and pointed
# at exactly this research-plan-review contract being built next.
UPSTREAM_REQUIRED_RESEARCH_PLAN_VERDICT = _RESEARCH_PLAN_VERDICT_READY
UPSTREAM_REQUIRED_RESEARCH_PLAN_NEXT_GATE = (
    "CRYPTO_D1_STRATEGY_CANDIDATE_RESEARCH_PLAN_REVIEW_CONTRACT_REQUIRED"
)
UPSTREAM_REQUIRED_RESEARCH_PLAN_MODE = "RESEARCH_ONLY"
...
NEXT_GATE_CRYPTO_D1_RESEARCH_PLAN_REVIEW_FIX_REQUIRED = (
    "CRYPTO_D1_STRATEGY_CANDIDATE_RESEARCH_PLAN_REVIEW_FIX_REQUIRED"
)
NEXT_GATE_CRYPTO_D1_RESEARCH_PLAN_REVIEW_PARKED = (
    "CRYPTO_D1_STRATEGY_CANDIDATE_RESEARCH_PLAN_REVIEW_PARKED"
)
NEXT_GATE_CRYPTO_D1_RESEARCH_PLAN_REVIEW_REJECTED = (
    "CRYPTO_D1_STRATEGY_CANDIDATE_RESEARCH_PLAN_REVIEW_REJECTED"
)
NEXT_GATE_AWAIT_CRYPTO_D1_RESEARCH_PLAN_CONTRACT = (
    "AWAIT_CRYPTO_D1_STRATEGY_CANDIDATE_RESEARCH_PLAN_CONTRACT"
)

# Market/timeframe scope reused from Block 103 so this review can never drift from
# the spot/D1 scope the research plan interprets evidence against.
REQUIRED_MARKET_TYPE = str(_PLAN_REQUIRED_MARKET_TYPE).strip().upper()
REQUIRED_TIMEFRAME = str(_PLAN_REQUIRED_TIMEFRAME).strip().upper()

# The four assets the external bot evidence watches, reused from Block 103.
WATCHED_UNIVERSE: tuple[str, ...] = tuple(_PLAN_WATCHED_UNIVERSE)
_WATCHED_UNIVERSE_SET: frozenset[str] = frozenset(WATCHED_UNIVERSE)

# Per-asset evidence stance, reused verbatim from Block 103 so the review can
# never weaken it.
ASSET_EVIDENCE_STANCES: dict[str, str] = dict(_PLAN_ASSET_EVIDENCE_STANCES)
ASSET_EVIDENCE_STANCE_REASONS: dict[str, str] = dict(
    _PLAN_ASSET_EVIDENCE_STANCE_REASONS
)

# Multi-asset sync gate, reused from Block 103.
MIN_MULTI_ASSET_SYNC_SCORE = _PLAN_MIN_SYNC_SCORE
MIN_SYNCHRONIZED_ASSETS = _PLAN_MIN_SYNC_ASSETS

# Mandatory trade-coordinator rules, reused from Block 103.
TRADE_COORDINATOR_RULES: tuple[str, ...] = tuple(_PLAN_TRADE_COORDINATOR_RULES)

# Bot-state policy, reused from Block 103. A declared bot state must be one of the
# allowed observe-grade values; any GO/live/execute grade value is a hard
# REJECTED.
ALLOWED_BOT_STATE_VALUES: tuple[str, ...] = tuple(_PLAN_ALLOWED_BOT_STATE_VALUES)
FORBIDDEN_BOT_STATE_VALUES: tuple[str, ...] = tuple(
    _PLAN_FORBIDDEN_BOT_STATE_VALUES
)
...
# contract reviews the shape of a proposed research plan and unlocks nothing.
RESEARCH_PLAN_REVIEW_SAFETY_POSTURE: dict[str, bool] = {
    "acquires_data": False,
    "fetches_data": False,
...
# REJECTED (the review tries to operate outside research-only / plan-only scope).
ALLOWED_RESEARCH_PLAN_REVIEW_MODES: tuple[str, ...] = (
    "research_only",
    "research-only",
    "research only",
    "plan_only",
...
# Descriptive text fields a human must record on a proposed research-plan-review
# packet. Absent -> NEEDS_MORE_INFO.
RESEARCH_PLAN_REVIEW_REQUIRED_TEXT_FIELDS: tuple[str, ...] = (
    "research_plan_review_packet_id",
    "upstream_research_plan_id",
    "research_plan_contract_version",
    "review_scope",
    "review_mode",
...
# flags are positive confirmations that the human reviewer checked and upholds
# each external-bot-evidence stance recorded by the research plan; the "no_*"
# flags confirm the review itself permits no real-world thing. A
# present-but-not-affirmed value is a request to admit or relax that thing -- a
# hard REJECTED. An absent value is a missing requirement (NEEDS_MORE_INFO).
RESEARCH_PLAN_REVIEW_REQUIRED_AFFIRMATIONS: tuple[str, ...] = (
    # Generic review acknowledgements
    ...
    # Generic review-side no-op acknowledgements
...
# Full set of required research-plan-review packet fields.
REQUIRED_RESEARCH_PLAN_REVIEW_FIELDS: tuple[str, ...] = (
    RESEARCH_PLAN_REVIEW_REQUIRED_TEXT_FIELDS
    + RESEARCH_PLAN_REVIEW_REQUIRED_AFFIRMATIONS
)

# Positive allow/grant flags a review packet must NOT request -- any truthy value
...
RESEARCH_PLAN_REVIEW_FORBIDDEN_ALLOW_FLAGS: tuple[str, ...] = (
    ...
    # Review-specific: a review may not approve a real next contract or relax
    # the plan it is supposed to merely check.
...
# Research-plan-review-phase blocked capabilities (named for this lane).
_RESEARCH_PLAN_REVIEW_BLOCKED_CAPABILITIES: tuple[str, ...] = (
...
_RESEARCH_PLAN_REVIEW_REFERENCE_PLACEHOLDER = (
    "Crypto-D1 research-plan-review packet is a paper placeholder describing a "
    "proposed review of the external-bot-evidence research plan: whether the "
    "declared observe-grade bot state, the BTC/ETH/SOL/XRP watched universe and "
    "per-asset stances, the multi-asset-sync gate, the lead-lag / lagged-sync "
    "exclusion, the offline-only ATR-threshold study boundary, the mandatory "
    "trade-coordinator rules, the closed-trade-proof requirement, and the "
    "no-promotion / no-paper-expansion / no-live-activation stance are all "
    "upheld -- it reviews on paper only, runs nothing, acquires nothing, "
    "modifies no external bot, selects no live strategy, and executes nothing."
)

_RESEARCH_PLAN_REVIEW_VERDICT_RATIONALE_PLACEHOLDER = (
    "Research-plan-review verdict rationale is a paper placeholder for a "
    "human-recorded judgement of whether a proposed review of the "
    "external-bot-evidence research plan upholds every stance and stays "
    "research-only, and its supporting reason."
)

_OPERATOR_NOTES: tuple[str, ...] = (
    "This is a crypto-d1 strategy candidate research plan review contract "
    "template and is execution-free.",
    "It reviews, on paper, whether the external-bot-evidence research plan "
    "recorded by the Block 103 contract is acceptable; it runs no research and "
    "touches no external bot.",
    "It evaluates a paper research-plan-review packet only and writes no report "
    "file.",
    "The review upholds that external bot evidence stays observation-only and "
...
_HUMAN_OPERATOR_REQUIRED_NEXT_STEPS: tuple[str, ...] = (
    "A human reviewer must record a research-plan-review verdict with a "
    "supporting rationale on paper.",
    "A human reviewer must confirm the research plan keeps the declared bot "
    "state observe-grade (watch / safe_observe / observe) and never GO/live/"
    ...
    "promotion.",
    "A human reviewer must confirm no strategy promotion, no paper expansion, "
    "and no live activation is authorized, and that the review weakens no part "
...
_REQUIRED_CONTRACT_FIELDS: tuple[str, ...] = (
    ...
    "protocol_name",
    "crypto_d1_strategy_candidate_research_plan_review_contract_active",
    "crypto_d1_strategy_candidate_research_plan_review_contract_state",
    "crypto_d1_strategy_candidate_research_plan_contract_recognized",
    "upstream_research_plan_verdict",
    "upstream_research_plan_next_gate",
    "upstream_research_plan_mode",
    "research_plan_review_contract_required",
    "research_plan_review_next_required_action",
    "research_plan_review_current_stage",
    "required_market_type",
    "required_timeframe",
    ...
    "allowed_bot_state_values",
    "forbidden_bot_state_values",
    "research_plan_review_packet_reference_placeholder",
    "research_plan_review_verdict",
    "research_plan_review_verdict_reasons",
    "evaluated_research_plan_review_packet",
    "allowed_research_plan_review_verdicts",
    "required_research_plan_review_fields",
    "research_plan_review_required_text_fields",
    "research_plan_review_required_affirmations",
    ...
    "allowed_research_plan_review_modes",
    "automated_approval_markers",
    "research_plan_review_verdict_rationale_placeholder",
    "research_plan_review_blocked_capabilities",
    "remaining_real_world_capabilities_blocked",
...
def _safety_posture() -> dict[str, bool]:
    ...
    return dict(RESEARCH_PLAN_REVIEW_SAFETY_POSTURE)
...
def _reject_reasons(packet: dict[str, Any]) -> tuple[str, ...]:
    ...
    admitted/relaxed affirmation, a disallowed mode, a GO/disallowed reviewed bot
    state, a market-confirmation claim below the multi-asset-sync gate, a non-core
    asset in the reviewed watched universe, an automated reviewer, or granted
    authority."""
    reasons: list[str] = []

    for flag in RESEARCH_PLAN_REVIEW_FORBIDDEN_ALLOW_FLAGS:
...
def _park_reasons(packet: dict[str, Any]) -> tuple[str, ...]:
    ...
    for flag in ("park", "parked", "defer", "deferred", "hold"):
        if _truthy(packet.get(flag)):
            reasons.append("reviewer_parked_research_plan_review")
            break
...
def _missing_reasons(packet: dict[str, Any]) -> tuple[str, ...]:
    """Return unmet requirements for an otherwise-safe research-plan-review packet:
    absent text fields, unaffirmed flags, or an incomplete BTC/ETH/SOL/XRP
    reviewed watched universe."""
    missing: list[str] = []

    for key in RESEARCH_PLAN_REVIEW_REQUIRED_TEXT_FIELDS:
...
def evaluate_crypto_d1_strategy_candidate_research_plan_review(
    packet: Any,
) -> dict[str, Any]:
    """Return a deterministic verdict for a proposed research-plan-review packet.
    ...
    affirmation / off-scope value / GO reviewed bot state / sub-threshold
    confirmation / non-core asset / automated reviewer / authority-granting) is
    checked before parking, and parking before completeness, so an unsafe review
    is rejected even when it would otherwise park or merely need more info."""
    p = packet if isinstance(packet, dict) else {}

    if not p:
        return {
            "verdict": RESEARCH_PLAN_REVIEW_VERDICT_NEEDS_MORE_INFO,
            "reasons": ("research_plan_review_packet_missing",),
        }

    rejected = _reject_reasons(p)
    if rejected:
        return {
            "verdict": RESEARCH_PLAN_REVIEW_VERDICT_REJECTED,
            "reasons": rejected,
        }
    ...
    if park:
        return {
            "verdict": RESEARCH_PLAN_REVIEW_VERDICT_PARKED,
            "reasons": park,
        }
    ...
    if not missing:
        return {
            "verdict": RESEARCH_PLAN_REVIEW_VERDICT_READY,
            "reasons": (
                "research_only_research_plan_review_fully_specified_upholds_"
                "all_external_bot_evidence_stances_watch_safe_observe_only_sol_"
                "blocked_xrp_watchlist_btc_dormant_eth_early_pressure_sync_gate_"
                "60_2_no_lead_lag_confirmation_atr_offline_only_trade_"
                "coordinator_rules_no_promotion_paper_or_live_and_blocks_every_"
...
def _validate(contract: dict[str, Any]) -> dict[str, Any]:
    ...
    schema_ok = (
        safe.get("schema_version") == RESEARCH_PLAN_REVIEW_SCHEMA_VERSION
    )
    read_only = safe.get("read_only") is True
    research_only = safe.get("mode") == "RESEARCH_ONLY"
    stage_ok = (
        safe.get("stage")
        == "CRYPTO_D1_STRATEGY_CANDIDATE_RESEARCH_PLAN_REVIEW_CONTRACT_ONLY"
    )
    ...
    verdicts_ok = (
        tuple(safe.get("allowed_research_plan_review_verdicts") or ())
        == ALLOWED_RESEARCH_PLAN_REVIEW_VERDICTS
    )
    fields_ok = (
        tuple(safe.get("required_research_plan_review_fields") or ())
        == REQUIRED_RESEARCH_PLAN_REVIEW_FIELDS
    )
    text_fields_ok = (
        tuple(safe.get("research_plan_review_required_text_fields") or ())
        == RESEARCH_PLAN_REVIEW_REQUIRED_TEXT_FIELDS
    )
    affirmations_ok = (
        tuple(safe.get("research_plan_review_required_affirmations") or ())
        == RESEARCH_PLAN_REVIEW_REQUIRED_AFFIRMATIONS
    )
    forbidden_flags_ok = (
        tuple(safe.get("research_plan_review_forbidden_allow_flags") or ())
        == RESEARCH_PLAN_REVIEW_FORBIDDEN_ALLOW_FLAGS
    )
    modes_ok = (
        tuple(safe.get("allowed_research_plan_review_modes") or ())
        == ALLOWED_RESEARCH_PLAN_REVIEW_MODES
    )
    markers_ok = (
    ...
    verdict_value_ok = (
        safe.get("research_plan_review_verdict")
        in ALLOWED_RESEARCH_PLAN_REVIEW_VERDICTS
    )
    blocked_present_ok = (
        len(tuple(safe.get("research_plan_review_blocked_capabilities") or ()))
        >= 1
    )
    ...
    return {
        ...
        "allowed_research_plan_review_verdicts_ok": verdicts_ok,
        "required_research_plan_review_fields_ok": fields_ok,
        "research_plan_review_required_text_fields_ok": text_fields_ok,
        "research_plan_review_required_affirmations_ok": affirmations_ok,
        "research_plan_review_forbidden_allow_flags_ok": forbidden_flags_ok,
        "allowed_research_plan_review_modes_ok": modes_ok,
        ...
        "research_plan_review_verdict_value_ok": verdict_value_ok,
        "research_plan_review_blocked_capabilities_present": blocked_present_ok,
...
def validate_crypto_d1_strategy_candidate_research_plan_review_contract(
    contract: dict[str, Any],
) -> dict[str, Any]:
    """Return deterministic validation of a crypto-d1 strategy candidate research
    plan review contract template. Pure; no I/O."""
    return _validate(contract)


def build_crypto_d1_strategy_candidate_research_plan_review_contract(
    upstream_research_plan_signal: Any,
    research_plan_review_packet: Any = None,
) -> dict[str, Any]:
    """Return a fresh deterministic read-only crypto-d1 strategy candidate research
    plan review contract template plus a paper verdict for a proposed review
    packet.
    ...
    contract becomes active solely when the upstream Block 103 research-plan signal
    is clearly READY for this gate:
    crypto_d1_bot_evidence_research_plan_contract_active is True,
    bot_evidence_research_plan_verdict == BOT_EVIDENCE_RESEARCH_PLAN_READY,
    next_gate == CRYPTO_D1_STRATEGY_CANDIDATE_RESEARCH_PLAN_REVIEW_CONTRACT_REQUIRED,
    ...
    AWAIT_CRYPTO_D1_STRATEGY_CANDIDATE_RESEARCH_PLAN_CONTRACT regardless of the
    packet. Even when active and READY, every authorization field stays False.
    Returned dicts are fresh."""
    signal = (
        upstream_research_plan_signal
        if isinstance(upstream_research_plan_signal, dict)
        else {}
    )

    sig_verdict = _field(signal, "bot_evidence_research_plan_verdict")
    sig_next_gate = _field(signal, "next_gate")
    sig_mode = _field(signal, "mode")
    active_flag_ok = (
        signal.get("crypto_d1_bot_evidence_research_plan_contract_active")
        is True
    )
    read_only_ok = signal.get("read_only") is True
    executes_false = signal.get("executes") is False
    verdict_ok = sig_verdict == UPSTREAM_REQUIRED_RESEARCH_PLAN_VERDICT
    next_gate_ok = sig_next_gate == UPSTREAM_REQUIRED_RESEARCH_PLAN_NEXT_GATE
    mode_ok = sig_mode == UPSTREAM_REQUIRED_RESEARCH_PLAN_MODE
    ...
    if contract_active:
        evaluation = (
            evaluate_crypto_d1_strategy_candidate_research_plan_review(
                research_plan_review_packet
            )
        )
        verdict = evaluation["verdict"]
        reasons = tuple(evaluation["reasons"])
    else:
        verdict = RESEARCH_PLAN_REVIEW_VERDICT_AWAIT
        reasons = (
            "await_crypto_d1_strategy_candidate_research_plan_contract",
        )
    ...
    state = (
        RESEARCH_PLAN_REVIEW_STATE_ACTIVE
        if contract_active
        else RESEARCH_PLAN_REVIEW_STATE_BLOCKED
    )

    if not contract_active:
        next_gate = NEXT_GATE_AWAIT_CRYPTO_D1_RESEARCH_PLAN_CONTRACT
    elif verdict == RESEARCH_PLAN_REVIEW_VERDICT_READY:
        next_gate = NEXT_GATE_CRYPTO_D1_RESEARCH_PLAN_REVIEW_READY
    elif verdict == RESEARCH_PLAN_REVIEW_VERDICT_NEEDS_MORE_INFO:
        next_gate = NEXT_GATE_CRYPTO_D1_RESEARCH_PLAN_REVIEW_FIX_REQUIRED
    elif verdict == RESEARCH_PLAN_REVIEW_VERDICT_PARKED:
        next_gate = NEXT_GATE_CRYPTO_D1_RESEARCH_PLAN_REVIEW_PARKED
    else:
        next_gate = NEXT_GATE_CRYPTO_D1_RESEARCH_PLAN_REVIEW_REJECTED
    ...
    echoed_packet = (
        dict(research_plan_review_packet)
        if isinstance(research_plan_review_packet, dict)
        else {}
    )

    contract = {
        "schema_version": RESEARCH_PLAN_REVIEW_SCHEMA_VERSION,
        "crypto_d1_strategy_candidate_research_plan_contract_schema_version": (
        ...
        "label": DEFAULT_RESEARCH_PLAN_REVIEW_LABEL,
        "status": RESEARCH_PLAN_REVIEW_STATUS,
        "stage": (
            "CRYPTO_D1_STRATEGY_CANDIDATE_RESEARCH_PLAN_REVIEW_CONTRACT_ONLY"
        ),
        "mode": "RESEARCH_ONLY",
        ...
        "upstream_research_plan_verdict": sig_verdict,
        "upstream_research_plan_next_gate": sig_next_gate,
        "upstream_research_plan_mode": sig_mode,
        "research_plan_review_contract_required": (
            research_plan_review_contract_required
        ),
        "research_plan_review_next_required_action": (
        ...
        "research_plan_review_verdict": verdict,
        "research_plan_review_verdict_reasons": reasons,
        "evaluated_research_plan_review_packet": echoed_packet,
        "allowed_research_plan_review_verdicts": (
            ALLOWED_RESEARCH_PLAN_REVIEW_VERDICTS
        ),
        "required_research_plan_review_fields": (
        ...
        "research_plan_review_blocked_capabilities": (
            _RESEARCH_PLAN_REVIEW_BLOCKED_CAPABILITIES
        ),
        "remaining_real_world_capabilities_blocked": (
            REMAINING_REAL_WORLD_CAPABILITIES_BLOCKED
        ),
...
def render_crypto_d1_strategy_candidate_research_plan_review_contract_markdown(
    contract: dict[str, Any],
) -> str:
    """Return deterministic, non-empty markdown for a crypto-d1 strategy candidate
    research plan review contract template. Pure; writes no file. Informational
    only."""
    verdicts = contract.get("allowed_research_plan_review_verdicts") or ()
    fields = contract.get("required_research_plan_review_fields") or ()
    text_fields = (
        contract.get("research_plan_review_required_text_fields") or ()
    )
    affirmations = (
        contract.get("research_plan_review_required_affirmations") or ()
    )
    forbidden_flags = (
        contract.get("research_plan_review_forbidden_allow_flags") or ()
    )
    modes = contract.get("allowed_research_plan_review_modes") or ()
    ...
    reasons = contract.get("research_plan_review_verdict_reasons") or ()
    blocked = contract.get("research_plan_review_blocked_capabilities") or ()
    remaining_blocked = (
    ...
    lines: list[str] = []
    lines.append(
        "# Strategy Factory Crypto-D1 Strategy Candidate Research Plan Review "
        "Contract"
    )
    lines.append("")
    lines.append(
        "Template only: this is a crypto-d1-strategy-candidate-research-plan-"
        "review-only, paper-only, no-research, no-data-acquisition, "
        ...
        "research-plan-review verdict, is not wired into any runtime state, "
    ...
    lines.append(
        "Crypto-D1 strategy candidate research plan review contract active: "
        f"{contract.get('crypto_d1_strategy_candidate_research_plan_review_contract_active', '')}"  # noqa: E501
    )
    lines.append(
        "Contract state: "
        f"{contract.get('crypto_d1_strategy_candidate_research_plan_review_contract_state', '')}"  # noqa: E501
    )
    lines.append(
        "Upstream research plan verdict: "
        f"{contract.get('upstream_research_plan_verdict', '')}"
    )
    lines.append(
        "Upstream research plan next gate: "
        f"{contract.get('upstream_research_plan_next_gate', '')}"
    )
    lines.append(
        "Upstream research plan mode: "
        f"{contract.get('upstream_research_plan_mode', '')}"
    )
    lines.append(
        "Research plan review contract required: "
        f"{contract.get('research_plan_review_contract_required', '')}"
    )
    lines.append(
        "Research plan review next required action: "
        f"{contract.get('research_plan_review_next_required_action', '')}"
    )
    lines.append(
        "Research plan review current stage: "
        f"{contract.get('research_plan_review_current_stage', '')}"
    )
    lines.append(
        f"Verdict: {contract.get('research_plan_review_verdict', '')}"
    )
    ...
    for x in coordinator:
        lines.append(f"- `{x}`")
    lines.append("")
    lines.append("## Research Plan Review Packet Reference")
    lines.append("")
    lines.append(
        "Research plan review packet reference: "
        f"{contract.get('research_plan_review_packet_reference_placeholder', '')}"  # noqa: E501
    )
    lines.append("")
    lines.append("## Research Plan Review Verdict Reasons")
    lines.append("")
    for x in reasons:
        lines.append(f"- `{x}`")
    lines.append("")
    lines.append("## Allowed Research Plan Review Verdicts")
    lines.append("")
    for x in verdicts:
        lines.append(f"- `{x}`")
    lines.append("")
    lines.append("## Required Research Plan Review Fields")
    lines.append("")
    for x in fields:
        lines.append(f"- `{x}`")
    lines.append("")
    lines.append("## Research Plan Review Required Text Fields")
    lines.append("")
    for x in text_fields:
        lines.append(f"- `{x}`")
    lines.append("")
    lines.append("## Research Plan Review Required Affirmations")
    lines.append("")
    for x in affirmations:
        lines.append(f"- `{x}`")
    lines.append("")
    lines.append("## Research Plan Review Forbidden Allow Flags")
    lines.append("")
    for x in forbidden_flags:
        lines.append(f"- `{x}`")
    lines.append("")
    lines.append("## Allowed Research Plan Review Modes")
    lines.append("")
    for x in modes:
        lines.append(f"- `{x}`")
    lines.append("")
    ...
    for x in markers:
        lines.append(f"- `{x}`")
    lines.append("")
    lines.append("## Research Plan Review Verdict Rationale")
    lines.append("")
    lines.append(
        "Research plan review verdict rationale: "
        f"{contract.get('research_plan_review_verdict_rationale_placeholder', '')}"  # noqa: E501
    )
    lines.append("")
    lines.append("## Research Plan Review Blocked Capabilities")
    lines.append("")
    for cap in blocked:
        lines.append(f"- `{cap}`")
    lines.append("")
