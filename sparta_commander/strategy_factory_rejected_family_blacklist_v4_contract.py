"""SPARTA STRATEGY FACTORY REJECTED FAMILY BLACKLIST V4 (READ-ONLY,
RESEARCH ONLY, NO EXECUTION, NO TRADING, NO AUTO-PUSH, NO SCHEDULER):
extends the pushed Rejected Family Blacklist V3 from seven to EIGHT
rejected families by adding `liquidity_sweep_mean_reversion`
(Candidate #8, rejected on origin/master).

V4 is the recommendation-memory layer ON TOP OF the pushed V3
contract. It freezes ONLY:

  - the EIGHT-family blacklist that any future Candidate #9+ family
    proposal must clear,
  - the eight-record ledger gating that produces it,
  - the explicit requirements every C9 hypothesis must satisfy
    BEFORE entering the Autopilot Loop V1 lifecycle: materially
    different from all eight rejected families, 27 bps fee model
    ...
    beyond-pattern-geometry at proposal time (C8 lesson), no winner
    wording, no profitability claim.

V4 DOES NOT:

  - generate or propose Candidate #9,
...
Chain-gated live on:
  - the pushed EIGHT-record rejection ledger (C1-C8 all REJECTED
    _KEPT_ON_RECORD),
  ...
  - the pushed C8 rejection record (which exports the
    future_family_blacklist_addition source-of-truth for the new
    entry).

If ANY of those break, V4 returns BLOCKED with explicit blockers.
"""
...
from sparta_commander.liquidity_sweep_mean_reversion_v1_rejection_record import (
    FUTURE_FAMILY_BLACKLIST_ADDITION as C8_BLACKLIST_ADDITION,
    REJECTION_REASON as C8_REJECTION_REASON,
    REJECTION_STATUS as C8_STATUS,
    VERDICT_RJ8_RECORDED,
    build_c8_rejection_record,
)
...
BL4_SCHEMA_VERSION = (
    "strategy_factory_rejected_family_blacklist_v4.v1")
BL4_LABEL = (
    "SPARTA Strategy Factory Rejected Family Blacklist V4 "
    "(READ-ONLY, RESEARCH ONLY, NO EXECUTION, NO TRADING, NO "
    "AUTO-PUSH, NO SCHEDULER, NOT A PROFITABILITY CLAIM, "
    "EIGHT-FAMILY BLACKLIST)")
BL4_MODE = "RESEARCH_ONLY"
VERDICT_BL4_READY = (
    "STRATEGY_FACTORY_REJECTED_FAMILY_BLACKLIST_V4_READY")
VERDICT_BL4_BLOCKED = (
    "STRATEGY_FACTORY_REJECTED_FAMILY_BLACKLIST_V4_BLOCKED")
NEXT_REQUIRED_ACTION = (
    "HUMAN_DECISION_DRAFT_CANDIDATE_9_FAMILY_PROPOSAL_USING_V4"
    "_BLACKLIST")

# ----------------------------------------------------------------------------
# the eight-record rejection ledger as input (declarative)
# ----------------------------------------------------------------------------
LEDGER_KEYS = ("C1", "C2", "C3", "C4", "C5", "C6", "C7", "C8")
...
# ----------------------------------------------------------------------------
# the EIGHT-family rejected blacklist (the V4 update)
# ----------------------------------------------------------------------------
REJECTED_FAMILY_LOGIC_BLACKLIST_V4 = (
...
PRIOR_V3_BLACKLIST = tuple(V3_BLACKLIST)
NEW_ENTRY_ADDED = C8_BLACKLIST_ADDITION

# ----------------------------------------------------------------------------
# requirements every C9 hypothesis must satisfy at proposal time
# ----------------------------------------------------------------------------
C9_PROPOSAL_REQUIREMENTS = {
    "schema_kind":
        "strategy_factory_rejected_family_blacklist_v4_c9"
        "_requirements",
    "required_fields": (
...
# ----------------------------------------------------------------------------
# lessons inherited from C1-C8 that the C9 hypothesis must respect
# ----------------------------------------------------------------------------
INHERITED_LESSONS = (
...
def get_blacklist_v4_label() -> str:
    return BL4_LABEL


def build_rejected_family_blacklist_v4() -> dict[str, Any]:
    """Assemble the V4 blacklist contract. Pure (no arguments).
    BLOCKED if the eight-record ledger does not hold, the C8 rejection
    record is not certifying, V3 is not certifying, V2 is not
    certifying, the Autopilot Loop V1 is not certifying, or the
    Recommendation V1 is not certifying."""
    record: dict[str, Any] = {
        "schema_version": BL4_SCHEMA_VERSION,
        "label": BL4_LABEL, "mode": BL4_MODE,
        ...
        "rejected_family_logic_blacklist_v4":
            list(REJECTED_FAMILY_LOGIC_BLACKLIST_V4),
        "blacklist_length": len(REJECTED_FAMILY_LOGIC_BLACKLIST_V4),
        "prior_v3_blacklist": list(PRIOR_V3_BLACKLIST),
        "new_entry_added": NEW_ENTRY_ADDED,
        "c9_proposal_requirements": {
            "schema_kind":
                C9_PROPOSAL_REQUIREMENTS["schema_kind"],
            "required_fields":
                list(C9_PROPOSAL_REQUIREMENTS["required_fields"]),
            "differentiation_must_address_each_of":
                list(C9_PROPOSAL_REQUIREMENTS[
                    "differentiation_must_address_each_of"]),
            "rejected_family_logic_blacklist":
                list(C9_PROPOSAL_REQUIREMENTS[
                    "rejected_family_logic_blacklist"]),
        ...
        "c8_rejection_status": C8_STATUS,
        "c8_rejection_reason": C8_REJECTION_REASON,
        "c8_future_family_blacklist_addition":
            C8_BLACKLIST_ADDITION,
        ...
        "generates_candidate_9_proposal_now": False,
        "generates_morning_report_now": False,
        "drafts_candidate_9_in_this_gate": False,
    ...
    if not record["ledger_all_rejected_kept_on_record"]:
        record["verdict"] = VERDICT_BL4_BLOCKED
        record["blockers"].append("eight_record_ledger_broken")
        return record
    # the C8 rejection record itself must certify
    c8_record = build_c8_rejection_record()
    if c8_record["verdict"] != VERDICT_RJ8_RECORDED:
        record["verdict"] = VERDICT_BL4_BLOCKED
        record["blockers"].append(
            "c8_rejection_record_not_certifying")
        record["blockers"].extend(c8_record.get("blockers") or [])
        return record
    # the new blacklist entry must equal the C8 contract's source-of-
    # truth
    if NEW_ENTRY_ADDED != C8_BLACKLIST_ADDITION \
            or NEW_ENTRY_ADDED not in (
            REJECTED_FAMILY_LOGIC_BLACKLIST_V4):
        record["verdict"] = VERDICT_BL4_BLOCKED
        record["blockers"].append(
            "new_blacklist_entry_must_match_c8_record")
        return record
    # the prior V3 blacklist must be a STRICT subset of V4 (we only
    # add; we never remove)
    if not set(PRIOR_V3_BLACKLIST).issubset(set(
            REJECTED_FAMILY_LOGIC_BLACKLIST_V4)):
        record["verdict"] = VERDICT_BL4_BLOCKED
        record["blockers"].append(
            "v3_blacklist_must_remain_a_subset_of_v4")
        return record
    if len(REJECTED_FAMILY_LOGIC_BLACKLIST_V4) != (
            len(PRIOR_V3_BLACKLIST) + 1):
        record["verdict"] = VERDICT_BL4_BLOCKED
        record["blockers"].append(
            "v4_must_add_exactly_one_new_entry_over_v3")
        return record
    ...
    if _loop.build_autopilot_loop_contract()["verdict"] != (
        ...
        return record
    record["verdict"] = VERDICT_BL4_READY
    return record


def validate_rejected_family_blacklist_v4(
    ...
    r = record
    if r.get("verdict") not in (VERDICT_BL4_READY, VERDICT_BL4_BLOCKED):
    ...
    if tuple(r.get("rejected_family_logic_blacklist_v4") or ()) != (
            REJECTED_FAMILY_LOGIC_BLACKLIST_V4):
        errors.append("blacklist_v4_tampered")
    if r.get("blacklist_length") != 8:
        errors.append("blacklist_length_must_be_8")
    if tuple(r.get("prior_v3_blacklist") or ()) != PRIOR_V3_BLACKLIST:
        errors.append("prior_v3_blacklist_tampered")
    if r.get("new_entry_added") != NEW_ENTRY_ADDED:
        errors.append("new_entry_added_tampered")
    if r.get("c8_rejection_status") != "REJECTED_KEPT_ON_RECORD":
        errors.append("c8_rejection_status_must_remain_rejected")
    if r.get("c8_rejection_reason") != C8_REJECTION_REASON:
        errors.append("c8_rejection_reason_tampered")
    if r.get("c8_future_family_blacklist_addition") != (
            C8_BLACKLIST_ADDITION):
        errors.append("c8_blacklist_addition_tampered")
    # c9 proposal requirements
    reqs = r.get("c9_proposal_requirements") or {}
    if reqs.get("schema_kind") != C9_PROPOSAL_REQUIREMENTS[
            "schema_kind"]:
        errors.append("c9_requirements_schema_kind_tampered")
    if tuple(reqs.get("required_fields") or ()) != (
            C9_PROPOSAL_REQUIREMENTS["required_fields"]):
        errors.append("c9_required_fields_tampered")
    if tuple(reqs.get(
            "differentiation_must_address_each_of") or ()) != (
            C9_PROPOSAL_REQUIREMENTS[
                "differentiation_must_address_each_of"]):
        errors.append("c9_differentiation_targets_tampered")
    if tuple(reqs.get("rejected_family_logic_blacklist") or ()) != (
            REJECTED_FAMILY_LOGIC_BLACKLIST_V4):
        errors.append("c9_blacklist_reference_tampered")
    for key in ("fee_assumption_must_equal_27_bps_round_trip",
        ...
        if reqs.get(key) is not True:
            errors.append("c9_requirement_flag_wrong:" + key)
    ...
    if r.get("verdict") == VERDICT_BL4_READY and r.get("blockers"):
