"""SPARTA Offline Strategy Factory - CRYPTO-D1 STRATEGY CANDIDATE RESEARCH DESIGN
CONTRACT.

A PURE, stdlib-only *read-only paper contract* builder and evaluator that records,
on paper, the proposed *research design* for the external-bot-evidence research
plan after it has already been human-approved by the Block 107 Crypto-D1 Strategy
Candidate Research Plan Approval Contract. It consumes the stable constants of the
Research Plan Approval Contract (Block 107), the Research Plan Review Contract
(Block 105), the External Bot Evidence Research Plan Contract (Block 103), the
Strategy Candidate Family Review Contract (Block 101), the Family Selection
Contract (Block 99), the Strategy Candidate Protocol Contract (Block 97), and the
Next Research Protocol (Block 95), and only when the upstream research-plan-approval
signal is clearly the expected READY one
(crypto_d1_strategy_candidate_research_plan_approval_contract_active is True,
research_plan_approval_verdict == STRATEGY_CANDIDATE_RESEARCH_PLAN_APPROVAL_READY,
next_gate ==
CRYPTO_D1_STRATEGY_CANDIDATE_RESEARCH_PLAN_APPROVAL_READY_SEPARATE_HUMAN_NEXT_STEP_REQUIRED,
mode == RESEARCH_ONLY, read_only is True, executes is False) evaluates a proposed
research-design packet on paper and returns a deterministic verdict describing
whether the design is ready, needs more info, must be rejected, or is parked.

It records the SHAPE of a proposed design only -- its candidate-evaluation rules,
observation-only evidence handling, PASS/WATCH/FAIL judgement logic, required
safeguards, and future validation boundaries. It does NOT acquire, fetch, inspect,
load, validate, transform, or compute on any data, does not run QA, baseline,
backtest, or simulation, produces no trade signal, reaches no broker / exchange /
order / account / API surface, trades no paper and no live, triggers no automation,
writes no runtime, registry, ledger, dashboard, or report state, opens no network,
spawns no child process, writes no file, reads no file, lists no directory, records
no timestamp, mints no random id, reads no environment, dynamically imports
nothing, and NEVER modifies the external bot's own repository or dashboard files.
It selects no live trading strategy.

The design preserves every external-bot-evidence safety stance carried by the
approved research plan: external bot evidence stays observation-only and never
...
strategy promotion, no paper expansion, and no live activation is authorized. The
PASS/WATCH/FAIL evaluation logic is a paper judgement only, and every validation
boundary blocks real execution.

Reaching a STRATEGY_CANDIDATE_RESEARCH_DESIGN_READY verdict unlocks NOTHING real.
It only records, on paper, that a human designed the research for the approved plan
for the next research-only step -- and even that still requires a separate, later,
human step. Any upstream research-plan-approval signal that is missing, malformed,
not READY, in the wrong mode, executable, non-read-only, or pointed at a different
next gate yields the
AWAIT_CRYPTO_D1_STRATEGY_CANDIDATE_RESEARCH_PLAN_APPROVAL_CONTRACT verdict.

Public API:
  - RESEARCH_DESIGN_SCHEMA_VERSION
  - DEFAULT_RESEARCH_DESIGN_LABEL
  - RESEARCH_DESIGN_STATUS
  - RESEARCH_DESIGN_SAFETY_POSTURE
  - RESEARCH_DESIGN_STATE_ACTIVE
  - RESEARCH_DESIGN_STATE_BLOCKED
  - RESEARCH_DESIGN_VERDICT_READY
  - RESEARCH_DESIGN_VERDICT_NEEDS_MORE_INFO
  - RESEARCH_DESIGN_VERDICT_REJECTED
  - RESEARCH_DESIGN_VERDICT_PARKED
  - RESEARCH_DESIGN_VERDICT_AWAIT
  - ALLOWED_RESEARCH_DESIGN_VERDICTS
  - UPSTREAM_REQUIRED_RESEARCH_PLAN_APPROVAL_VERDICT
  - UPSTREAM_REQUIRED_RESEARCH_PLAN_APPROVAL_NEXT_GATE
  - UPSTREAM_REQUIRED_RESEARCH_PLAN_APPROVAL_MODE
  - RESEARCH_DESIGN_NEXT_REQUIRED_ACTION
  - RESEARCH_DESIGN_CURRENT_STAGE
  - DECISION_CRYPTO_D1_RESEARCH_DESIGN_CONTRACT_REQUIRED
  - NEXT_GATE_CRYPTO_D1_RESEARCH_DESIGN_READY
  - NEXT_GATE_CRYPTO_D1_RESEARCH_DESIGN_FIX_REQUIRED
  - NEXT_GATE_CRYPTO_D1_RESEARCH_DESIGN_PARKED
  - NEXT_GATE_CRYPTO_D1_RESEARCH_DESIGN_REJECTED
  - NEXT_GATE_AWAIT_CRYPTO_D1_RESEARCH_PLAN_APPROVAL_CONTRACT
  ...
  - RESEARCH_DESIGN_EVALUATION_OUTCOMES
  - RESEARCH_DESIGN_REQUIRED_SAFEGUARDS
  - RESEARCH_DESIGN_VALIDATION_BOUNDARIES
  - REQUIRED_RESEARCH_DESIGN_FIELDS
  - RESEARCH_DESIGN_REQUIRED_TEXT_FIELDS
  - RESEARCH_DESIGN_REQUIRED_AFFIRMATIONS
  - RESEARCH_DESIGN_FORBIDDEN_ALLOW_FLAGS
  - ALLOWED_RESEARCH_DESIGN_MODES
  - AUTOMATED_DESIGN_MARKERS
  - REMAINING_REAL_WORLD_CAPABILITIES_BLOCKED
  - evaluate_crypto_d1_strategy_candidate_research_design(packet)
  - build_crypto_d1_strategy_candidate_research_design_contract(signal, packet=None)
  - validate_crypto_d1_strategy_candidate_research_design_contract(contract)
  - render_crypto_d1_strategy_candidate_research_design_contract_markdown(contract)
"""
...
from sparta_commander.strategy_factory_crypto_d1_strategy_candidate_research_plan_approval_contract import (  # noqa: E501
    ...
    RESEARCH_PLAN_APPROVAL_VERDICT_READY as _RESEARCH_PLAN_APPROVAL_VERDICT_READY,  # noqa: E501
    NEXT_GATE_CRYPTO_D1_RESEARCH_PLAN_APPROVAL_READY as _NEXT_GATE_RESEARCH_PLAN_APPROVAL_READY,  # noqa: E501
    REQUIRED_MARKET_TYPE as _APPROVAL_REQUIRED_MARKET_TYPE,
    REQUIRED_TIMEFRAME as _APPROVAL_REQUIRED_TIMEFRAME,
    WATCHED_UNIVERSE as _APPROVAL_WATCHED_UNIVERSE,
    ASSET_EVIDENCE_STANCES as _APPROVAL_ASSET_EVIDENCE_STANCES,
    ASSET_EVIDENCE_STANCE_REASONS as _APPROVAL_ASSET_EVIDENCE_STANCE_REASONS,
    MIN_MULTI_ASSET_SYNC_SCORE as _APPROVAL_MIN_SYNC_SCORE,
    MIN_SYNCHRONIZED_ASSETS as _APPROVAL_MIN_SYNC_ASSETS,
    TRADE_COORDINATOR_RULES as _APPROVAL_TRADE_COORDINATOR_RULES,
    ALLOWED_BOT_STATE_VALUES as _APPROVAL_ALLOWED_BOT_STATE_VALUES,
    FORBIDDEN_BOT_STATE_VALUES as _APPROVAL_FORBIDDEN_BOT_STATE_VALUES,
)
...
__all__ = [
    ...
    "AUTOMATED_DESIGN_MARKERS",
    "REMAINING_REAL_WORLD_CAPABILITIES_BLOCKED",
    "evaluate_crypto_d1_strategy_candidate_research_design",
    "build_crypto_d1_strategy_candidate_research_design_contract",
    "validate_crypto_d1_strategy_candidate_research_design_contract",
    "render_crypto_d1_strategy_candidate_research_design_contract_markdown",
]

RESEARCH_DESIGN_SCHEMA_VERSION = (
    "strategy_factory_crypto_d1_strategy_candidate_research_design_contract.v1"
)
DEFAULT_RESEARCH_DESIGN_LABEL = (
    "Strategy Factory Crypto-D1 Strategy Candidate Research Design Contract"
)
RESEARCH_DESIGN_STATUS = (
    "READ_ONLY_CRYPTO_D1_STRATEGY_CANDIDATE_RESEARCH_DESIGN_CONTRACT"
)

RESEARCH_DESIGN_STATE_ACTIVE = (
    "CRYPTO_D1_STRATEGY_CANDIDATE_RESEARCH_DESIGN_CONTRACT_ACTIVE"
)
RESEARCH_DESIGN_STATE_BLOCKED = (
    "CRYPTO_D1_STRATEGY_CANDIDATE_RESEARCH_DESIGN_CONTRACT_BLOCKED"
)

RESEARCH_DESIGN_VERDICT_READY = (
    "STRATEGY_CANDIDATE_RESEARCH_DESIGN_READY"
)
RESEARCH_DESIGN_VERDICT_NEEDS_MORE_INFO = (
    "STRATEGY_CANDIDATE_RESEARCH_DESIGN_NEEDS_MORE_INFO"
)
RESEARCH_DESIGN_VERDICT_REJECTED = (
    "STRATEGY_CANDIDATE_RESEARCH_DESIGN_REJECTED"
)
RESEARCH_DESIGN_VERDICT_PARKED = (
    "STRATEGY_CANDIDATE_RESEARCH_DESIGN_PARKED"
)
RESEARCH_DESIGN_VERDICT_AWAIT = (
    "AWAIT_CRYPTO_D1_STRATEGY_CANDIDATE_RESEARCH_PLAN_APPROVAL_CONTRACT"
)

ALLOWED_RESEARCH_DESIGN_VERDICTS: tuple[str, ...] = (
    RESEARCH_DESIGN_VERDICT_READY,
    RESEARCH_DESIGN_VERDICT_NEEDS_MORE_INFO,
    RESEARCH_DESIGN_VERDICT_REJECTED,
    RESEARCH_DESIGN_VERDICT_PARKED,
...
# The exact upstream research-plan-approval signal this contract activates from
# (Block 107). The contract only becomes active when the upstream approval contract
# is clearly READY, in RESEARCH_ONLY mode, read-only, non-executing, and pointed
# at exactly this research-design contract being built next.
UPSTREAM_REQUIRED_RESEARCH_PLAN_APPROVAL_VERDICT = (
    _RESEARCH_PLAN_APPROVAL_VERDICT_READY
)
UPSTREAM_REQUIRED_RESEARCH_PLAN_APPROVAL_NEXT_GATE = (
    _NEXT_GATE_RESEARCH_PLAN_APPROVAL_READY
)
UPSTREAM_REQUIRED_RESEARCH_PLAN_APPROVAL_MODE = "RESEARCH_ONLY"
...
NEXT_GATE_CRYPTO_D1_RESEARCH_DESIGN_READY = (
    "CRYPTO_D1_STRATEGY_CANDIDATE_RESEARCH_DESIGN_READY_SEPARATE_HUMAN_"
    "NEXT_STEP_REQUIRED"
)
NEXT_GATE_CRYPTO_D1_RESEARCH_DESIGN_FIX_REQUIRED = (
    "CRYPTO_D1_STRATEGY_CANDIDATE_RESEARCH_DESIGN_FIX_REQUIRED"
)
NEXT_GATE_CRYPTO_D1_RESEARCH_DESIGN_PARKED = (
    "CRYPTO_D1_STRATEGY_CANDIDATE_RESEARCH_DESIGN_PARKED"
)
NEXT_GATE_CRYPTO_D1_RESEARCH_DESIGN_REJECTED = (
    "CRYPTO_D1_STRATEGY_CANDIDATE_RESEARCH_DESIGN_REJECTED"
)
NEXT_GATE_AWAIT_CRYPTO_D1_RESEARCH_PLAN_APPROVAL_CONTRACT = (
    "AWAIT_CRYPTO_D1_STRATEGY_CANDIDATE_RESEARCH_PLAN_APPROVAL_CONTRACT"
)

# Market/timeframe scope reused from Block 107 so this design can never drift from
# the spot/D1 scope the approved research plan interprets evidence against.
REQUIRED_MARKET_TYPE = str(_APPROVAL_REQUIRED_MARKET_TYPE).strip().upper()
REQUIRED_TIMEFRAME = str(_APPROVAL_REQUIRED_TIMEFRAME).strip().upper()

# The four assets the external bot evidence watches, reused from Block 107.
WATCHED_UNIVERSE: tuple[str, ...] = tuple(_APPROVAL_WATCHED_UNIVERSE)
_WATCHED_UNIVERSE_SET: frozenset[str] = frozenset(WATCHED_UNIVERSE)

# Per-asset evidence stance, reused verbatim from Block 107 so the design can never
# weaken it.
ASSET_EVIDENCE_STANCES: dict[str, str] = dict(_APPROVAL_ASSET_EVIDENCE_STANCES)
ASSET_EVIDENCE_STANCE_REASONS: dict[str, str] = dict(
    _APPROVAL_ASSET_EVIDENCE_STANCE_REASONS
)

# Multi-asset sync gate, reused from Block 107.
MIN_MULTI_ASSET_SYNC_SCORE = _APPROVAL_MIN_SYNC_SCORE
MIN_SYNCHRONIZED_ASSETS = _APPROVAL_MIN_SYNC_ASSETS

# Mandatory trade-coordinator rules, reused from Block 107.
TRADE_COORDINATOR_RULES: tuple[str, ...] = tuple(
    _APPROVAL_TRADE_COORDINATOR_RULES
)

# Bot-state policy, reused from Block 107. A declared bot state must be one of the
# allowed observe-grade values; any GO/live/execute grade value is a hard REJECTED.
ALLOWED_BOT_STATE_VALUES: tuple[str, ...] = tuple(
    _APPROVAL_ALLOWED_BOT_STATE_VALUES
)
FORBIDDEN_BOT_STATE_VALUES: tuple[str, ...] = tuple(
    _APPROVAL_FORBIDDEN_BOT_STATE_VALUES
)
...
RESEARCH_DESIGN_EVALUATION_OUTCOMES: tuple[str, ...] = (
    "PASS",
    "WATCH",
    "FAIL",
)
...
RESEARCH_DESIGN_REQUIRED_SAFEGUARDS: tuple[str, ...] = (
...
RESEARCH_DESIGN_VALIDATION_BOUNDARIES: tuple[str, ...] = (
...
RESEARCH_DESIGN_SAFETY_POSTURE: dict[str, bool] = {
    "acquires_data": False,
    "fetches_data": False,
...
# Strings that mark a non-human / automated designer. A packet authored by any of
# these is rejected: only a human designer may record this paper decision.
AUTOMATED_DESIGN_MARKERS: tuple[str, ...] = (
    "auto",
    "automated",
    "automation",
    "bot",
...
# REJECTED (the design tries to operate outside research-only / design-only scope).
ALLOWED_RESEARCH_DESIGN_MODES: tuple[str, ...] = (
    "research_only",
    "research-only",
    "research only",
    "design_only",
    "design-only",
    "design only",
    "research_only_design_only",
)

# Descriptive text fields a human must record on a proposed research-design packet.
# Absent -> NEEDS_MORE_INFO.
RESEARCH_DESIGN_REQUIRED_TEXT_FIELDS: tuple[str, ...] = (
    "research_design_packet_id",
    "upstream_research_plan_approval_id",
    "research_plan_approval_contract_version",
    "design_scope",
    "design_mode",
    "designed_declared_bot_state",
    "designed_watched_universe",
    "designed_asset_stance_summary",
    "designed_multi_asset_sync_interpretation",
    "designed_lead_lag_interpretation",
    "designed_atr_threshold_study_boundary",
    "designed_trade_coordinator_rule_summary",
    "designed_closed_trade_proof_requirement",
    "design_candidate_evaluation_rule_summary",
    "design_pass_watch_fail_logic_summary",
    "design_observation_only_evidence_handling",
    "design_required_safeguard_summary",
    "design_future_validation_boundary",
    "designer_name_or_id",
...
# Affirmation flags the packet must carry (each affirmed True). The "designed_*"
# flags are positive confirmations that the human designer checked and upholds each
# external-bot-evidence stance carried by the approved research plan; the "no_*" /
# "design_*" flags confirm the design itself permits no real-world thing. A
...
RESEARCH_DESIGN_REQUIRED_AFFIRMATIONS: tuple[str, ...] = (
    # Generic design acknowledgements
    "explicit_human_design",
    "research_only_acknowledgement",
    "design_only_acknowledgement",
    "no_execution_acknowledgement",
    # Design-specific scope acknowledgements
    "designed_research_plan_approval_was_ready",
    "design_is_research_only_human_designed_for_next_step_only",
    "design_authorizes_no_execution_or_real_work",
    # Rule 1
    "designed_external_bot_state_is_watch_safe_observe_not_go",
    # Rule 2
    "designed_bot_evidence_is_external_evidence_only",
    "designed_bot_evidence_not_execution_permission",
    # Rule 3
    "designed_failsafe_or_insufficient_data_is_observe_log_only",
    # Rule 4
    "designed_closed_trade_proof_required_before_promotion",
    # Rule 5
    "designed_sol_main_candidate_blocked_from_execution",
    # Rule 6
    "designed_xrp_watchlist_only_insufficient_history",
    # Rule 7
    "designed_btc_dormant_blocked_from_expansion",
    # Rule 8
    "designed_eth_early_pressure_only",
    # Rule 9
    "designed_multi_asset_sync_threshold_required_before_confirmation",
    # Rule 10
    "designed_lead_lag_not_used_as_confirmation",
    "designed_lagged_sync_not_used_as_confirmation",
    # Rule 11
    "designed_adaptive_atr_thresholds_offline_study_only",
    "designed_no_live_threshold_changes",
    # Rule 12
    "designed_trade_coordinator_max_one_open_trade_per_symbol",
    "designed_trade_coordinator_no_same_direction_adds",
    "designed_trade_coordinator_treat_exchanges_as_same_symbol",
    "designed_trade_coordinator_block_opposite_direction",
    # Rule 13
    "designed_no_strategy_promotion",
    "designed_no_paper_expansion",
    "designed_no_live_activation",
    # Design-specific structure acknowledgements
    "designed_candidate_evaluation_is_observation_only",
    "designed_pass_watch_fail_is_paper_judgement_only",
    "designed_validation_boundary_blocks_real_execution",
...
# Full set of required research-design packet fields.
REQUIRED_RESEARCH_DESIGN_FIELDS: tuple[str, ...] = (
    RESEARCH_DESIGN_REQUIRED_TEXT_FIELDS
    + RESEARCH_DESIGN_REQUIRED_AFFIRMATIONS
)
...
# real next contract, or relax the plan/review/approval it builds on).
RESEARCH_DESIGN_FORBIDDEN_ALLOW_FLAGS: tuple[str, ...] = (
    ...
    # validation, nor relax the plan / review / approval it is supposed to design.
    "design_for_execution",
    "design_grants_execution",
    "treat_design_as_execution_permission",
    "proceed_to_real_validation",
    "design_authorizes_real_validation",
    ...
    "weaken_research_plan_approval",
    "relax_research_plan_approval_safety",
)
...
_RESEARCH_DESIGN_BLOCKED_CAPABILITIES: tuple[str, ...] = (
...
_RESEARCH_DESIGN_REFERENCE_PLACEHOLDER = (
    "Crypto-D1 research-design packet is a paper placeholder describing a proposed "
    "human research design for the approved external-bot-evidence research plan: "
...
_RESEARCH_DESIGN_VERDICT_RATIONALE_PLACEHOLDER = (
    "Research-design verdict rationale is a paper placeholder for a human-recorded "
    "judgement of whether a proposed research design of the approved "
...
_OPERATOR_NOTES: tuple[str, ...] = (
    "This is a crypto-d1 strategy candidate research design contract template and "
    "is execution-free.",
    "It records, on paper, the proposed human research design of the approved "
    "external-bot-evidence research plan recorded by the Block 107 contract; it "
    "runs no research and touches no external bot.",
    "It records a paper research-design packet only and writes no report file.",
    ...
    "A human designer alone may record this verdict; no automated designer is "
    "accepted, and any READY still requires a separate, later, human step.",
)

_HUMAN_OPERATOR_REQUIRED_NEXT_STEPS: tuple[str, ...] = (
    "A human designer must record a research-design verdict with a supporting "
    "rationale on paper.",
    "A human designer must confirm the approved research plan keeps the declared "
    ...
    "permission.",
    "A human designer must confirm the candidate-evaluation rules stay "
    "observation-only, the PASS/WATCH/FAIL logic stays a paper judgement, and "
    "every validation boundary blocks real execution.",
    "A human designer must confirm the per-asset stances are upheld: SOL blocked "
    "from execution, XRP watchlist-only, BTC dormant/blocked from expansion, ETH "
    "early-pressure only.",
    "A human designer must confirm market confirmation stays gated behind a 60+ "
    ...
    "promotion.",
    "A human designer must confirm no strategy promotion, no paper expansion, and "
...
_REQUIRED_CONTRACT_FIELDS: tuple[str, ...] = (
    ...
    "protocol_name",
    "crypto_d1_strategy_candidate_research_design_contract_active",
    "crypto_d1_strategy_candidate_research_design_contract_state",
    "crypto_d1_strategy_candidate_research_plan_approval_contract_recognized",
    "upstream_research_plan_approval_verdict",
    "upstream_research_plan_approval_next_gate",
    "upstream_research_plan_approval_mode",
    "research_design_contract_required",
    "research_design_next_required_action",
    "research_design_current_stage",
    "required_market_type",
    "required_timeframe",
    ...
    "research_design_required_safeguards",
    "research_design_validation_boundaries",
    "research_design_packet_reference_placeholder",
    "research_design_verdict",
    "research_design_verdict_reasons",
    "evaluated_research_design_packet",
    "allowed_research_design_verdicts",
    "required_research_design_fields",
    "research_design_required_text_fields",
    "research_design_required_affirmations",
    ...
    "allowed_research_design_modes",
    "automated_design_markers",
    "research_design_verdict_rationale_placeholder",
    "research_design_blocked_capabilities",
    "remaining_real_world_capabilities_blocked",
...
def _safety_posture() -> dict[str, bool]:
    ...
    return dict(RESEARCH_DESIGN_SAFETY_POSTURE)
...
def _reject_reasons(packet: dict[str, Any]) -> tuple[str, ...]:
    ...
    outcome, an automated designer, or granted authority."""
    reasons: list[str] = []

    for flag in RESEARCH_DESIGN_FORBIDDEN_ALLOW_FLAGS:
    ...
    bot_state = packet.get("designed_declared_bot_state")
    if _present(bot_state):
        state_value = _scalar(bot_state)
        if state_value in _FORBIDDEN_BOT_STATE_SET:
            reasons.append("designed_bot_state_go_not_permitted")
        elif state_value not in _ALLOWED_BOT_STATE_SET:
            reasons.append("disallowed_designed_bot_state")

    if _truthy(packet.get("designed_claims_market_confirmation")):
        score = _as_number(packet.get("designed_multi_asset_sync_score"))
        count = _as_number(packet.get("designed_synchronized_asset_count"))
        if (
            score is None
            ...
            reasons.append(
                "designed_market_confirmation_below_multi_asset_sync_threshold"
            )

    watched = _upper_token_set(packet.get("designed_watched_universe"))
    extras = watched - _WATCHED_UNIVERSE_SET
    if extras:
        reasons.append("non_core_assets_in_designed_watched_universe")

    outcome = packet.get("designed_candidate_evaluation_outcome")
    if _present(outcome):
    ...
    for key in (
        ...
        if _scalar(packet.get(key)) in AUTOMATED_DESIGN_MARKERS:
            reasons.append(f"automated_designer:{key}")
...
def _park_reasons(packet: dict[str, Any]) -> tuple[str, ...]:
    """Return parking reasons when a designer explicitly parks or defers the whole
    research-design lane."""
    reasons: list[str] = []
    park_values = {
    ...
    for flag in ("park", "parked", "defer", "deferred", "hold"):
        if _truthy(packet.get(flag)):
            reasons.append("designer_parked_research_design")
            break

    if _scalar(packet.get("designer_decision")) in park_values:
        reasons.append("designer_decision_parked")
    if _scalar(packet.get("design_decision")) in park_values:
        reasons.append("design_decision_parked")

    return tuple(reasons)


def _missing_reasons(packet: dict[str, Any]) -> tuple[str, ...]:
    """Return unmet requirements for an otherwise-safe research-design packet:
    absent text fields, unaffirmed flags, or an incomplete BTC/ETH/SOL/XRP designed
    watched universe."""
    missing: list[str] = []

    for key in RESEARCH_DESIGN_REQUIRED_TEXT_FIELDS:
    ...
    for asset in WATCHED_UNIVERSE:
        if asset not in watched:
            missing.append(f"designed_watched_universe_missing_{asset}")

    return tuple(missing)


def evaluate_crypto_d1_strategy_candidate_research_design(
    packet: Any,
) -> dict[str, Any]:
    """Return a deterministic verdict for a proposed research-design packet. Pure;
    ...
    asset / bad evaluation outcome / automated designer / authority-granting) is
    ...
    if not p:
        return {
            "verdict": RESEARCH_DESIGN_VERDICT_NEEDS_MORE_INFO,
            "reasons": ("research_design_packet_missing",),
        }

    rejected = _reject_reasons(p)
    if rejected:
        return {
            "verdict": RESEARCH_DESIGN_VERDICT_REJECTED,
            "reasons": rejected,
        }
    ...
    if park:
        return {
            "verdict": RESEARCH_DESIGN_VERDICT_PARKED,
            "reasons": park,
        }
    ...
    if not missing:
        return {
            "verdict": RESEARCH_DESIGN_VERDICT_READY,
            "reasons": (
                "research_only_research_design_fully_specified_human_designed_"
...
def _validate(contract: dict[str, Any]) -> dict[str, Any]:
    ...
    schema_ok = (
        safe.get("schema_version") == RESEARCH_DESIGN_SCHEMA_VERSION
    )
    read_only = safe.get("read_only") is True
    research_only = safe.get("mode") == "RESEARCH_ONLY"
    stage_ok = (
        safe.get("stage")
        == "CRYPTO_D1_STRATEGY_CANDIDATE_RESEARCH_DESIGN_CONTRACT_ONLY"
    )
    ...
    verdicts_ok = (
        tuple(safe.get("allowed_research_design_verdicts") or ())
        == ALLOWED_RESEARCH_DESIGN_VERDICTS
    )
    fields_ok = (
        tuple(safe.get("required_research_design_fields") or ())
        == REQUIRED_RESEARCH_DESIGN_FIELDS
    )
    text_fields_ok = (
        tuple(safe.get("research_design_required_text_fields") or ())
        == RESEARCH_DESIGN_REQUIRED_TEXT_FIELDS
    )
    affirmations_ok = (
        tuple(safe.get("research_design_required_affirmations") or ())
        == RESEARCH_DESIGN_REQUIRED_AFFIRMATIONS
    )
    forbidden_flags_ok = (
        tuple(safe.get("research_design_forbidden_allow_flags") or ())
        == RESEARCH_DESIGN_FORBIDDEN_ALLOW_FLAGS
    )
    modes_ok = (
        tuple(safe.get("allowed_research_design_modes") or ())
        == ALLOWED_RESEARCH_DESIGN_MODES
    )
    markers_ok = (
        tuple(safe.get("automated_design_markers") or ())
        == AUTOMATED_DESIGN_MARKERS
    )
    watched_ok = (
    ...
    outcomes_ok = (
        tuple(safe.get("research_design_evaluation_outcomes") or ())
        == RESEARCH_DESIGN_EVALUATION_OUTCOMES
    )
    safeguards_ok = (
        tuple(safe.get("research_design_required_safeguards") or ())
        == RESEARCH_DESIGN_REQUIRED_SAFEGUARDS
    )
    boundaries_ok = (
        tuple(safe.get("research_design_validation_boundaries") or ())
        == RESEARCH_DESIGN_VALIDATION_BOUNDARIES
    )
    ...
    verdict_value_ok = (
        safe.get("research_design_verdict")
        in ALLOWED_RESEARCH_DESIGN_VERDICTS
    )
    blocked_present_ok = (
        len(tuple(safe.get("research_design_blocked_capabilities") or ()))
        >= 1
    )
    ...
    return {
        ...
        "allowed_research_design_verdicts_ok": verdicts_ok,
        "required_research_design_fields_ok": fields_ok,
        "research_design_required_text_fields_ok": text_fields_ok,
        "research_design_required_affirmations_ok": affirmations_ok,
        "research_design_forbidden_allow_flags_ok": forbidden_flags_ok,
        "allowed_research_design_modes_ok": modes_ok,
        "automated_design_markers_ok": markers_ok,
        ...
        "research_design_evaluation_outcomes_ok": outcomes_ok,
        "research_design_required_safeguards_ok": safeguards_ok,
        "research_design_validation_boundaries_ok": boundaries_ok,
        ...
        "research_design_verdict_value_ok": verdict_value_ok,
        "research_design_blocked_capabilities_present": blocked_present_ok,
...
def validate_crypto_d1_strategy_candidate_research_design_contract(
...
def build_crypto_d1_strategy_candidate_research_design_contract(
    upstream_research_plan_approval_signal: Any,
    research_design_packet: Any = None,
) -> dict[str, Any]:
    ...
    contract becomes active solely when the upstream Block 107
    research-plan-approval signal is clearly READY for this gate:
    crypto_d1_strategy_candidate_research_plan_approval_contract_active is True,
    research_plan_approval_verdict ==
    STRATEGY_CANDIDATE_RESEARCH_PLAN_APPROVAL_READY, next_gate ==
    CRYPTO_D1_STRATEGY_CANDIDATE_RESEARCH_PLAN_APPROVAL_READY_SEPARATE_HUMAN_NEXT_STEP_REQUIRED,
    ...
    AWAIT_CRYPTO_D1_STRATEGY_CANDIDATE_RESEARCH_PLAN_APPROVAL_CONTRACT regardless of
    the packet. Even when active and READY, every authorization field stays False.
    Returned dicts are fresh."""
    signal = (
        upstream_research_plan_approval_signal
        if isinstance(upstream_research_plan_approval_signal, dict)
        else {}
    )

    sig_verdict = _field(signal, "research_plan_approval_verdict")
    ...
    verdict_ok = (
        sig_verdict == UPSTREAM_REQUIRED_RESEARCH_PLAN_APPROVAL_VERDICT
    )
    next_gate_ok = (
        sig_next_gate == UPSTREAM_REQUIRED_RESEARCH_PLAN_APPROVAL_NEXT_GATE
    )
    mode_ok = sig_mode == UPSTREAM_REQUIRED_RESEARCH_PLAN_APPROVAL_MODE
    ...
    if contract_active:
        evaluation = (
            evaluate_crypto_d1_strategy_candidate_research_design(
                research_design_packet
            )
        )
        verdict = evaluation["verdict"]
        reasons = tuple(evaluation["reasons"])
    else:
        verdict = RESEARCH_DESIGN_VERDICT_AWAIT
        reasons = (
            "await_crypto_d1_strategy_candidate_research_plan_approval_contract",
        )
    ...
    state = (
        RESEARCH_DESIGN_STATE_ACTIVE
        if contract_active
        else RESEARCH_DESIGN_STATE_BLOCKED
    )

    if not contract_active:
        next_gate = NEXT_GATE_AWAIT_CRYPTO_D1_RESEARCH_PLAN_APPROVAL_CONTRACT
    elif verdict == RESEARCH_DESIGN_VERDICT_READY:
        next_gate = NEXT_GATE_CRYPTO_D1_RESEARCH_DESIGN_READY
    elif verdict == RESEARCH_DESIGN_VERDICT_NEEDS_MORE_INFO:
        next_gate = NEXT_GATE_CRYPTO_D1_RESEARCH_DESIGN_FIX_REQUIRED
    elif verdict == RESEARCH_DESIGN_VERDICT_PARKED:
        next_gate = NEXT_GATE_CRYPTO_D1_RESEARCH_DESIGN_PARKED
    else:
        next_gate = NEXT_GATE_CRYPTO_D1_RESEARCH_DESIGN_REJECTED
    ...
    echoed_packet = (
        dict(research_design_packet)
        if isinstance(research_design_packet, dict)
        else {}
    )
    ...
    contract = {
        ...
        "label": DEFAULT_RESEARCH_DESIGN_LABEL,
        "status": RESEARCH_DESIGN_STATUS,
        "stage": (
            "CRYPTO_D1_STRATEGY_CANDIDATE_RESEARCH_DESIGN_CONTRACT_ONLY"
        ),
        "mode": "RESEARCH_ONLY",
        ...
        "crypto_d1_strategy_candidate_research_plan_approval_contract_recognized": (  # noqa: E501
            contract_active
        ),
        "upstream_research_plan_approval_verdict": sig_verdict,
        "upstream_research_plan_approval_next_gate": sig_next_gate,
        "upstream_research_plan_approval_mode": sig_mode,
        "research_design_contract_required": (
            research_design_contract_required
        ),
        "research_design_next_required_action": (
        ...
        "research_design_verdict": verdict,
        "research_design_verdict_reasons": reasons,
        "evaluated_research_design_packet": echoed_packet,
        "allowed_research_design_verdicts": (
            ALLOWED_RESEARCH_DESIGN_VERDICTS
        ),
        "required_research_design_fields": (
        ...
        "automated_design_markers": AUTOMATED_DESIGN_MARKERS,
        "research_design_verdict_rationale_placeholder": (
            _RESEARCH_DESIGN_VERDICT_RATIONALE_PLACEHOLDER
        ),
        "research_design_blocked_capabilities": (
            _RESEARCH_DESIGN_BLOCKED_CAPABILITIES
        ),
        "remaining_real_world_capabilities_blocked": (
            REMAINING_REAL_WORLD_CAPABILITIES_BLOCKED
        ),
...
def render_crypto_d1_strategy_candidate_research_design_contract_markdown(
    contract: dict[str, Any],
) -> str:
    """Return deterministic, non-empty markdown for a crypto-d1 strategy candidate
    research design contract template. Pure; writes no file. Informational only."""
    verdicts = contract.get("allowed_research_design_verdicts") or ()
    fields = contract.get("required_research_design_fields") or ()
    text_fields = (
        contract.get("research_design_required_text_fields") or ()
    )
    affirmations = (
        contract.get("research_design_required_affirmations") or ()
    )
    forbidden_flags = (
        contract.get("research_design_forbidden_allow_flags") or ()
    )
    modes = contract.get("allowed_research_design_modes") or ()
    markers = contract.get("automated_design_markers") or ()
    watched = contract.get("watched_universe") or ()
    stances = contract.get("asset_evidence_stances") or {}
    reasons = contract.get("research_design_verdict_reasons") or ()
    blocked = contract.get("research_design_blocked_capabilities") or ()
    remaining_blocked = (
        contract.get("remaining_real_world_capabilities_blocked") or ()
    )
    coordinator = contract.get("trade_coordinator_rules") or ()
    outcomes = contract.get("research_design_evaluation_outcomes") or ()
    safeguards = contract.get("research_design_required_safeguards") or ()
    boundaries = contract.get("research_design_validation_boundaries") or ()
    ...
    lines: list[str] = []
    lines.append(
        "# Strategy Factory Crypto-D1 Strategy Candidate Research Design "
        "Contract"
    )
    lines.append("")
    lines.append(
        "Template only: this is a crypto-d1-strategy-candidate-research-design-"
        ...
        "-- it records only a paper research-design verdict, is not wired into "
    ...
    lines.append(
        ...
        f"`{contract.get('crypto_d1_strategy_candidate_research_plan_approval_contract_schema_version', '')}`"  # noqa: E501
    )
    lines.append(
    ...
    lines.append(
        "Crypto-D1 strategy candidate research design contract active: "
        f"{contract.get('crypto_d1_strategy_candidate_research_design_contract_active', '')}"  # noqa: E501
    )
    lines.append(
        "Contract state: "
        f"{contract.get('crypto_d1_strategy_candidate_research_design_contract_state', '')}"  # noqa: E501
    )
    lines.append(
        "Upstream research plan approval verdict: "
        f"{contract.get('upstream_research_plan_approval_verdict', '')}"
    )
    lines.append(
        "Upstream research plan approval next gate: "
        f"{contract.get('upstream_research_plan_approval_next_gate', '')}"
    )
    lines.append(
        "Upstream research plan approval mode: "
        f"{contract.get('upstream_research_plan_approval_mode', '')}"
    )
    lines.append(
        "Research design contract required: "
        f"{contract.get('research_design_contract_required', '')}"
    )
    lines.append(
        "Research design next required action: "
        f"{contract.get('research_design_next_required_action', '')}"
    )
    lines.append(
        "Research design current stage: "
        f"{contract.get('research_design_current_stage', '')}"
    )
    lines.append(
        f"Verdict: {contract.get('research_design_verdict', '')}"
    )
    ...
    for x in coordinator:
        lines.append(f"- `{x}`")
    lines.append("")
    lines.append("## Research Design Evaluation Outcomes")
    lines.append("")
    for x in outcomes:
        lines.append(f"- `{x}`")
    lines.append("")
    lines.append("## Research Design Required Safeguards")
    lines.append("")
    for x in safeguards:
        lines.append(f"- `{x}`")
    lines.append("")
    lines.append("## Research Design Validation Boundaries")
    lines.append("")
    for x in boundaries:
        lines.append(f"- `{x}`")
    lines.append("")
    lines.append("## Research Design Packet Reference")
    lines.append("")
    lines.append(
        "Research design packet reference: "
        f"{contract.get('research_design_packet_reference_placeholder', '')}"
    )
    lines.append("")
    lines.append("## Research Design Verdict Reasons")
    lines.append("")
    for x in reasons:
        lines.append(f"- `{x}`")
    lines.append("")
    lines.append("## Allowed Research Design Verdicts")
    lines.append("")
    for x in verdicts:
        lines.append(f"- `{x}`")
    lines.append("")
    lines.append("## Required Research Design Fields")
    lines.append("")
    for x in fields:
        lines.append(f"- `{x}`")
    lines.append("")
    lines.append("## Research Design Required Text Fields")
    lines.append("")
    for x in text_fields:
        lines.append(f"- `{x}`")
    lines.append("")
    lines.append("## Research Design Required Affirmations")
    lines.append("")
    for x in affirmations:
        lines.append(f"- `{x}`")
    lines.append("")
    lines.append("## Research Design Forbidden Allow Flags")
    lines.append("")
    for x in forbidden_flags:
        lines.append(f"- `{x}`")
    lines.append("")
    lines.append("## Allowed Research Design Modes")
    lines.append("")
    for x in modes:
        lines.append(f"- `{x}`")
    lines.append("")
    ...
    for x in markers:
        lines.append(f"- `{x}`")
    lines.append("")
    lines.append("## Research Design Verdict Rationale")
    lines.append("")
    lines.append(
        "Research design verdict rationale: "
        f"{contract.get('research_design_verdict_rationale_placeholder', '')}"
    )
    lines.append("")
    lines.append("## Research Design Blocked Capabilities")
    lines.append("")
    for cap in blocked:
        lines.append(f"- `{cap}`")
    lines.append("")
