"""SPARTA CANDIDATE #9 REAL-CANDLE DETECTOR LABELS REVIEW / EVIDENCE
FREEZE (READ-ONLY, RESEARCH ONLY, SAMPLE-SIZE ADEQUACY STRUCTURALLY
FAILED, NOT A PROFITABILITY CLAIM, NOT A RESCUE):
...
by the untracked runner tools/c9_real_candle_detection_once.py:

  - 3,840 staged BTCUSD 15m bars scanned (closed window
    2026-05-02T00:00:00Z .. 2026-06-10T23:45:00Z);
  - 8 joint-trigger attempts (the joint price-AND-volume trigger
    fired on exactly 8 bars; this is structurally MUCH rarer than
    the C8 single-condition liquidity-sweep trigger -- exactly what
    the C9 edge argument predicts, because committed institutional
    selling drives volume UP with price, leaving real "thin-book
    panic" excursions rare);
  - 1 accepted setup before the proposal-level anti-cluster filter;
  - 1 accepted setup after the proposal-level anti-cluster filter;
  - 0 dropped by anti-cluster (only one accepted event; nothing to
    cluster against);
  - 7 rejected by the scanner; breakdown:
      * 7 rejected_geometry_floor
      * 0 on entry-bar invalidation, invalid stop geometry, or no
        evaluation bar (every triggered bar produced a valid stop
        and a valid entry-bar; the 7 rejected were small-magnitude
        excursions screened out by the 81 bps floor exactly as the
        fee geometry was designed to do);
  - identity check 1: 1 + 7 = 8 (accepted-pre + rejected =
    attempts);
  - identity check 2: 1 + 0 = 1 (accepted-post + drops =
    accepted-pre);
  - SAMPLE-SIZE ADEQUACY STRUCTURALLY FAILED: 1 accepted-post < 20
    threshold; below-minimum-at-dry-run = True; sample-size
    adequacy enforced at this labels-review gate triggers a
    STRUCTURAL REJECTION WITHOUT consuming the single C9 edit
    token;
  - anti-cluster: 8-bar min gap (unused since only 1 accepted),
    proposal-locked, NOT edit token;
  - explicit-edge-argument field remains proposal/spec-level
    locked, NOT edit token;
  - replay_executed_now: False; pnl_computed: False;
  - single C9 edit token NOT applied (sample-size structural
    failure does not consume the token).

EVIDENCE LANGUAGE ONLY. NO edge has been demonstrated. NO
profitability is claimed. The honest finding is that on the
2026-05-02_2026-06-10 BTCUSD 15m window, the JOINT price-AND-
volume trigger fired on only 8 bars; of those, 7 had stop distances
too small to clear the 81 bps fee-aware floor, leaving 1 accepted
post-anti-cluster setup -- WELL BELOW the proposal-locked
sample-size adequacy threshold of 20. This is a STRUCTURAL
REJECTION condition: the family did not produce enough qualifying
events on this sample window. The C9 hypothesis's explicit edge
argument predicted that thin-book panic excursions would be rare;
the data confirms they are. Whether to spend the single C9 edit
token on a structural relaxation (z-score threshold, volume
percentile, rolling-window size, or stop-buffer multiplier) or
reject Candidate #9 on sample-size inadequacy is the HUMAN
...
self-claimed flags, and certifies every frozen fact. It runs
...
review, the pushed V4 rejected-family blacklist, the pushed V3
blacklist, the pushed Overnight Research Autopilot V2, the pushed
...
C9L_SCHEMA_VERSION = (
    "low_volume_downside_capitulation_mean_reversion_v1_real"
    "_candle_labels_review.v1")
C9L_LABEL = ("SPARTA Candidate #9 Real-Candle Labels Review / "
             "Evidence Freeze (READ-ONLY, RESEARCH ONLY, 1 "
             "ACCEPTED POST ANTI-CLUSTER, SAMPLE-SIZE ADEQUACY "
             "STRUCTURALLY FAILED, NOT A PROFITABILITY CLAIM, "
             "NOT A RESCUE)")
C9L_MODE = "RESEARCH_ONLY"
VERDICT_C9L_FROZEN = (
    "CANDIDATE_9_REAL_CANDLE_LABELS_REVIEW_FROZEN")
VERDICT_C9L_REJECTED = (
    "CANDIDATE_9_REAL_CANDLE_LABELS_REVIEW_REJECTED")
VERDICT_C9L_BLOCKED = (
    "CANDIDATE_9_REAL_CANDLE_LABELS_REVIEW_BLOCKED")
NEXT_REQUIRED_ACTION = (
    "HUMAN_DECISION_C9_SPEND_SINGLE_EDIT_OR_REJECT_ON_SAMPLE_SIZE"
    "_INADEQUACY")
CURRENT_LOOP_STAGE = "detector_and_label_review"

HEAD_AT_DETECTION = "78026474f2f6798032863d1840aab5788f378b34"
RUNNER_PATH = "tools/c9_real_candle_detection_once.py"
LABELS_PATH = ("data/low_volume_capitulation_c9/detector_labels/"
               "c9_detector_labels_2026-05-02_2026-06-10.json")
SUMMARY_PATH = ("data/low_volume_capitulation_c9/detector_labels/"
                "c9_detector_summary_2026-05-02_2026-06-10.json")
EXPECTED_LABELS_SHA256 = (
    "8a9bb08d9e03f7c49961830c53c182a61b619013c48ed862e2be5d4b7dd40fbf")
EXPECTED_SUMMARY_SHA256 = (
    "8ff429165b8cfbfaa9ed1c8cde3674cf901cd373ca1404d414b0d01e8cc90d4e")
...
EXPECTED_SAMPLE_SIZE_STRUCTURAL_FAILURE = True
...
EXPECTED_ANTI_CLUSTER_FACTS = {
    ...
    "accepted_before_anti_cluster": 1,
    "accepted_after_anti_cluster": 1,
...
EXPECTED_SCOPE_LOCKS = {
    ...
    "explicit_edge_argument_field_proposal_locked_not_edit_token":
        True,
}
...
FROZEN_DETECTION_FACTS = (
    ...
    "8 joint-trigger attempts; this is structurally MUCH rarer "
    "than C8's 133 attempts on the same sample window, exactly as "
    "the C9 edge argument predicts (committed institutional "
    "selling drives volume UP with price, leaving real thin-book "
    "panic excursions rare)",
    "1 accepted before anti-cluster; 1 accepted after anti-cluster",
    "7 rejected by scanner: all 7 on geometry_floor; 0 on entry-"
    "bar invalidation, invalid stop geometry, or no evaluation bar",
    "0 dropped by 8-bar anti-cluster (only one accepted event; "
    "nothing to cluster against)",
    "1 + 7 = 8 (accepted-pre + rejected = attempts)",
    "1 + 0 = 1 (accepted-post + drops = accepted-pre)",
    "SAMPLE-SIZE ADEQUACY STRUCTURALLY FAILED: 1 accepted-post < "
    "20 threshold; the C9 hypothesis did not produce enough "
    "qualifying events on this sample window to support a "
    "meaningful replay evaluation",
    "anti-cluster gap remains proposal-level locked at 8 bars and "
    "does NOT consume the single c9 edit token",
    "sample-size adequacy threshold remains proposal-level locked "
    "at 20 accepted setups and does NOT consume the single c9 "
    "edit token (the structural failure flag is recorded WITHOUT "
    "spending the token)",
    "explicit-edge-argument field remains proposal/spec-level "
...
def get_candidate_9_labels_review_label() -> str:
    return C9L_LABEL


def observe_c9_labels(repo_root: Any,
                      tracked_paths: Any = ()
                      ) -> dict[str, Any]:
    ...
    # If dropped is empty, vacuously True; otherwise verify each has
    # the clustered status.
...
def certify_c9_labels_review(observation: Any) -> dict[str, Any]:
    ...
    if o.get("labels_accepted_post_count") != (
            EXPECTED_ACCEPTED_POST_ANTI_CLUSTER):
        failures.append("labels_accepted_post_count_must_equal_1")
...
def build_c9_labels_review(repo_root: Any,
                           tracked_paths: Any = ()
                           ) -> dict[str, Any]:
    """Observe read-only and certify; chain-gated on the full pushed
    C9 lane (proposal -> spec -> detector spec/dry-run -> dry-run
    review) plus V4 + V3 + V2 + Recommendation V1 + Autopilot V1 +
    eight-record rejection ledger."""
    record: dict[str, Any] = {
        "schema_version": C9L_SCHEMA_VERSION, "label": C9L_LABEL,
        "mode": C9L_MODE, "lane": "crypto_d1_auto_research",
    ...
    if not record["ledger_all_rejected_kept_on_record"]:
        record["verdict"] = VERDICT_C9L_BLOCKED
    ...
    if _loop.build_autopilot_loop_contract()["verdict"] != (
        ...
        return record
    observation = observe_c9_labels(repo_root, tracked_paths)
    result = certify_c9_labels_review(observation)
    record["failures"] = result["failures"]
    record["verdict"] = (VERDICT_C9L_FROZEN if result["certified"]
                         else VERDICT_C9L_REJECTED)
    return record


def validate_c9_labels_review(record: Any) -> dict[str, Any]:
    ...
    r = record
    if r.get("verdict") not in (VERDICT_C9L_FROZEN,
                                VERDICT_C9L_REJECTED,
                                VERDICT_C9L_BLOCKED):
    ...
    if r.get("verdict") == VERDICT_C9L_FROZEN and r.get("failures"):
