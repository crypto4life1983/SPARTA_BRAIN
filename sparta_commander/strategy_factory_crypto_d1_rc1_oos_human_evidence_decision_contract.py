"""Crypto-D1 V2 RC1 Out-of-Sample HUMAN EVIDENCE Decision Contract (READ-ONLY).

A PURE, stdlib-only, read-only module that RECORDS a human's decision over the completed
RC1 out-of-sample evidence for ``V2_trend_plus_cash_regime``. It is the human-judgment
companion to the automated Block 182 results review: where Block 182 mechanically scored
the persisted RC1 replay report and computed the degradation analysis, THIS contract
captures the human's recorded verdict on that evidence and the human's chosen research
direction.

It consumes the Block 182 reviewer output (which itself reads only
``reports/crypto_d1_rc1_out_of_sample_replay/rc1_oos_replay_report.json`` read-only),
re-validates it with Block 182's own validator, and emits a human-evidence record that:
  - acknowledges that RC1 reviewed the evidence-leading policy's out-of-sample behavior;
  - acknowledges the USEFUL held-out survival (the policy survived genuinely unseen 2020
    history) AND the MATERIAL DEGRADATION versus the in-sample aggregate;
  ...
  - records the selected research direction -- by default
    ``CONTINUE_RESEARCH_VIA_RC2_CROSS_POLICY_STABILITY``: before any promotion discussion,
    determine whether the leading policy is still the best versus the other fixed
    candidates (RP1..RP5) across the SAME fixed out-of-sample windows;
  - carries the reviewer's blockers, risk notes, and degradation numbers forward.
...
Public API:
  ...
  - DIRECTION_RC2_CROSS_POLICY_STABILITY / DIRECTION_FURTHER_VALIDATION
  - DIRECTION_CLOSE_RC1_THREAD / ALLOWED_DIRECTIONS / NEXT_REQUIRED_ACTION
  - get_rc1_oos_human_evidence_decision_label()
  - allowed_directions()
  - record_rc1_oos_human_evidence_decision(results_review_decision, *, selected_direction=..., decision_reason=...)
  - build_rc1_oos_human_evidence_decision(repo_root, *, selected_direction=..., decision_reason=...)
  - validate_rc1_oos_human_evidence_decision(decision)
  - render_rc1_oos_human_evidence_decision_markdown(decision)
"""
...
from sparta_commander.strategy_factory_crypto_d1_rc1_out_of_sample_results_review_contract import (
    ...
    build_rc1_oos_results_review_decision,
    validate_rc1_oos_results_review_decision,
)

HUMAN_EVIDENCE_SCHEMA_VERSION = (
    "strategy_factory_crypto_d1_rc1_oos_human_evidence_decision_contract.v1"
)
HUMAN_EVIDENCE_LABEL = "Crypto-D1 V2 RC1 Out-of-Sample Human Evidence Decision (READ-ONLY)"
HUMAN_EVIDENCE_MODE = "RESEARCH_ONLY"

VERDICT_DECISION_RECORDED = "RC1_OOS_HUMAN_EVIDENCE_DECISION_RECORDED"
VERDICT_DECISION_BLOCKED = "RC1_OOS_HUMAN_EVIDENCE_DECISION_BLOCKED"
...
DIRECTION_RC2_CROSS_POLICY_STABILITY = "CONTINUE_RESEARCH_VIA_RC2_CROSS_POLICY_STABILITY"
DIRECTION_FURTHER_VALIDATION = "REQUIRE_FURTHER_VALIDATION"
DIRECTION_CLOSE_RC1_THREAD = "CLOSE_RC1_RESEARCH_THREAD"
ALLOWED_DIRECTIONS: tuple[str, ...] = (
    DIRECTION_RC2_CROSS_POLICY_STABILITY,
    DIRECTION_FURTHER_VALIDATION,
    DIRECTION_CLOSE_RC1_THREAD,
)

# After this decision the only next step remains research-only: the human-approved RC2
# cross-policy stability research over the SAME fixed out-of-sample windows. Nothing here
# is an execution authorization.
NEXT_REQUIRED_ACTION = "HUMAN_APPROVED_RC2_CROSS_POLICY_STABILITY_RESEARCH"

# The research question RC2 exists to answer; recorded for traceability.
RC2_RESEARCH_QUESTION = (
    "Is the evidence-leading policy still the best policy versus the other fixed "
    "candidates (RP1..RP5) across the SAME fixed out-of-sample windows?"
)

DEFAULT_DECISION_REASON = (
    "RC1 showed the leading policy is interesting but degraded out-of-sample. Before any "
    "promotion discussion, determine whether it is still the best policy compared with "
    "the other fixed candidates across the same out-of-sample windows. That is exactly "
    "what RC2 should answer."
)


def get_rc1_oos_human_evidence_decision_label() -> str:
    """Human label for the recognized Crypto-D1 RC1 OOS human evidence decision contract."""
...
def record_rc1_oos_human_evidence_decision(
    results_review_decision: Any,
    *,
    selected_direction: str = DIRECTION_RC2_CROSS_POLICY_STABILITY,
    decision_reason: str = DEFAULT_DECISION_REASON,
) -> dict[str, Any]:
    """Record a human's decision over the Block 182 results-review decision. PURE: takes
    ...
    ``DECISION_DO_NOT_PROMOTE``; this contract has no promote branch and unlocks nothing."""
    blockers: list[str] = []
    risk_notes: list[str] = []
    ...
    if not isinstance(results_review_decision, dict):
        blockers.append("rc1_results_review_decision_missing")
        return _decision(
            VERDICT_DECISION_BLOCKED, blockers, risk_notes,
            selected_direction=selected_direction_value,
            decision_reason=decision_reason, policy_id=None, degradation={},
            review_blockers=[], report_found=False,
        )

    rd = results_review_decision

    review_validation = validate_rc1_oos_results_review_decision(rd)
    if not review_validation.get("valid"):
        blockers.append("rc1_results_review_decision_invalid")
        blockers.extend("review:" + e for e in review_validation.get("errors", []))

    if rd.get("verdict") != VERDICT_RC1_REVIEW_COMPLETE:
        blockers.append("rc1_results_review_not_complete")

    if rd.get("rc1_oos_replay_report_found") is not True:
    ...
    degradation = dict(rd.get("degradation") or {})
    policy_id = rd.get("policy_id")

    # Acknowledge the evidence, both sides honestly. Recording is descriptive only; it
    # never promotes the policy.
    if policy_id:
        risk_notes.append("acknowledged_rc1_reviewed_oos_evidence_for:" + str(policy_id))
    if degradation.get("held_out_window_evaluated") and degradation.get(
        "held_out_window_positive"
    ):
        risk_notes.append("acknowledged_useful_held_out_survival")
    else:
        risk_notes.append("held_out_survival_not_confirmed")
    if degradation.get("materially_degraded_versus_in_sample"):
        risk_notes.append("acknowledged_material_degradation_versus_in_sample")
    else:
        risk_notes.append("material_degradation_not_flagged_by_review")
    ...
    recorded = (
        not blockers
        and rd.get("verdict") == VERDICT_RC1_REVIEW_COMPLETE
        and rd.get("rc1_oos_replay_report_found") is True
    )
    ...
    return _decision(
        ...
        policy_id=policy_id, degradation=degradation,
        review_blockers=list(rd.get("blockers") or []),
        report_found=bool(rd.get("rc1_oos_replay_report_found")),
    )
...
def _decision(
    ...
    *,
    ...
    human decision is always DO_NOT_PROMOTE, and the selected direction is a research
    direction only -- never an execution authorization."""
    return {
        ...
        "rc2_research_question": RC2_RESEARCH_QUESTION,
...
def build_rc1_oos_human_evidence_decision(
    repo_root: str = ".",
    *,
    selected_direction: str = DIRECTION_RC2_CROSS_POLICY_STABILITY,
    decision_reason: str = DEFAULT_DECISION_REASON,
) -> dict[str, Any]:
    """Load the Block 182 results-review decision (which reads the local RC1 replay report
    read-only) and record a human decision over it. Reads nothing itself beyond what Block
    182 reads; writes nothing; runs no replay; unlocks no gate."""
    review_decision = build_rc1_oos_results_review_decision(repo_root)
    decision = record_rc1_oos_human_evidence_decision(
        review_decision, selected_direction=selected_direction,
        decision_reason=decision_reason,
    )
    decision["rc1_oos_replay_report_path"] = review_decision.get(
        "rc1_oos_replay_report_path"
    )
    decision["results_review_verdict"] = review_decision.get("verdict")
    return decision


def validate_rc1_oos_human_evidence_decision(decision: Any) -> dict[str, Any]:
...
def render_rc1_oos_human_evidence_decision_markdown(decision: Any) -> str:
    ...
    lines.append("")
    lines.append("## RC2 research question")
    lines.append(str(d.get("rc2_research_question", "")))
    lines.append("")
    ...
    lines.append("- Held-out window positive: " + str(deg.get("held_out_window_positive")))
