"""Tests for the Crypto-D1 Strategy Candidate Research Design Approval Contract (Block 113).

Pure stdlib, read-only, side-effect-free. These tests assert the contract records
the proposed human approval of the Block 111 research design review, stays gated behind a
clean READY research-design-review signal pointed at this approval gate, upholds every
...
import sparta_commander.strategy_factory_crypto_d1_strategy_candidate_research_design_approval_contract as m  # noqa: E501
...
def _active_signal() -> dict:
    """A clean, READY Block 111 research-design-review signal pointed at this gate."""
    return {
        "crypto_d1_strategy_candidate_research_design_review_contract_active": True,
        "research_design_review_verdict": (
            m.UPSTREAM_REQUIRED_RESEARCH_DESIGN_REVIEW_VERDICT
        ),
        "next_gate": m.UPSTREAM_REQUIRED_RESEARCH_DESIGN_REVIEW_NEXT_GATE,
        "mode": m.UPSTREAM_REQUIRED_RESEARCH_DESIGN_REVIEW_MODE,
        "read_only": True,
        "executes": False,
    }
...
def _valid_packet() -> dict:
    ...
    p: dict = {}
    for f in m.RESEARCH_DESIGN_APPROVAL_REQUIRED_TEXT_FIELDS:
        p[f] = f + "_value"
    ...
    for a in m.RESEARCH_DESIGN_APPROVAL_REQUIRED_AFFIRMATIONS:
        p[a] = True
    return p
...
def test_schema_version_preserved():
    assert m.RESEARCH_DESIGN_APPROVAL_SCHEMA_VERSION == (
        "strategy_factory_crypto_d1_strategy_candidate_research_design_approval_"
        "contract.v1"
    )


def test_label_is_research_design_approval():
    assert "Research Design Approval" in m.DEFAULT_RESEARCH_DESIGN_APPROVAL_LABEL


def test_status_is_read_only_research_design_approval():
    assert m.RESEARCH_DESIGN_APPROVAL_STATUS == (
        "READ_ONLY_CRYPTO_D1_STRATEGY_CANDIDATE_RESEARCH_DESIGN_APPROVAL_CONTRACT"
    )


def test_market_and_timeframe_scope_from_block_111():
...
def test_evaluation_outcomes_are_pass_watch_fail():
    assert m.RESEARCH_DESIGN_APPROVAL_EVALUATION_OUTCOMES == (
        "PASS", "WATCH", "FAIL"
    )


def test_required_safeguards_and_validation_boundaries_present():
    assert len(m.RESEARCH_DESIGN_APPROVAL_REQUIRED_SAFEGUARDS) >= 1
    assert "observation_only_evidence_handling" in (
        m.RESEARCH_DESIGN_APPROVAL_REQUIRED_SAFEGUARDS
    )
    assert len(m.RESEARCH_DESIGN_APPROVAL_VALIDATION_BOUNDARIES) >= 1
    assert "validation_is_a_separate_later_human_step" in (
        m.RESEARCH_DESIGN_APPROVAL_VALIDATION_BOUNDARIES
    )


def test_upstream_gating_constants():
    assert m.UPSTREAM_REQUIRED_RESEARCH_DESIGN_REVIEW_NEXT_GATE == (
        "CRYPTO_D1_STRATEGY_CANDIDATE_RESEARCH_DESIGN_REVIEW_READY_SEPARATE_"
        "HUMAN_NEXT_STEP_REQUIRED"
    )
    assert m.UPSTREAM_REQUIRED_RESEARCH_DESIGN_REVIEW_MODE == "RESEARCH_ONLY"
    assert m.UPSTREAM_REQUIRED_RESEARCH_DESIGN_REVIEW_VERDICT == (
        "STRATEGY_CANDIDATE_RESEARCH_DESIGN_REVIEW_READY"
    )


def test_verdict_constants_distinct():
    assert len(set(m.ALLOWED_RESEARCH_DESIGN_APPROVAL_VERDICTS)) == 5


def test_stage_and_action_match_registry_values():
    assert m.RESEARCH_DESIGN_APPROVAL_CURRENT_STAGE == (
        "CRYPTO_D1_STRATEGY_CANDIDATE_RESEARCH_DESIGN_APPROVAL_CONTRACT_REQUIRED"
    )
    assert m.RESEARCH_DESIGN_APPROVAL_NEXT_REQUIRED_ACTION == (
        "BUILD_CRYPTO_D1_STRATEGY_CANDIDATE_RESEARCH_DESIGN_APPROVAL_CONTRACT"
    )


# --------------------------------------------------------------------------- #
# Upstream gating (only a clean Block 111 READY-for-this-gate signal activates)
# --------------------------------------------------------------------------- #

def test_inactive_signal_awaits():
    c = m.build_crypto_d1_strategy_candidate_research_design_approval_contract(
        {}, None
    )
    assert c["research_design_approval_verdict"] == (
        m.RESEARCH_DESIGN_APPROVAL_VERDICT_AWAIT
    )
    assert c[
...
def test_active_signal_activates_and_is_ready():
    ...
    assert c["research_design_approval_verdict"] == (
        m.RESEARCH_DESIGN_APPROVAL_VERDICT_READY
    )


def test_dirty_signal_missing_active_flag_awaits():
    sig = _active_signal()
    del sig["crypto_d1_strategy_candidate_research_design_review_contract_active"]
    c = m.build_crypto_d1_strategy_candidate_research_design_approval_contract(
        sig, _valid_packet()
    )
    assert c["research_design_approval_verdict"] == (
...
def test_wrong_verdict_awaits():
    sig = _active_signal()
    sig["research_design_review_verdict"] = "SOMETHING_ELSE"
    c = m.build_crypto_d1_strategy_candidate_research_design_approval_contract(
        sig, _valid_packet()
    )
    assert c["research_design_approval_verdict"] == (
...
def test_valid_packet_is_ready():
    r = m.evaluate_crypto_d1_strategy_candidate_research_design_approval(
        _valid_packet()
    )
    assert r["verdict"] == m.RESEARCH_DESIGN_APPROVAL_VERDICT_READY


def test_empty_packet_needs_more_info():
    r = m.evaluate_crypto_d1_strategy_candidate_research_design_approval({})
    assert r["verdict"] == m.RESEARCH_DESIGN_APPROVAL_VERDICT_NEEDS_MORE_INFO


def test_missing_text_field_needs_more_info():
    p = _valid_packet()
    del p["approval_decision_rationale"]
    r = m.evaluate_crypto_d1_strategy_candidate_research_design_approval(p)
    assert r["verdict"] == m.RESEARCH_DESIGN_APPROVAL_VERDICT_NEEDS_MORE_INFO
...
def test_go_approved_bot_state_rejected():
    ...
    r = m.evaluate_crypto_d1_strategy_candidate_research_design_approval(p)
    assert r["verdict"] == m.RESEARCH_DESIGN_APPROVAL_VERDICT_REJECTED
...
def test_disallowed_evaluation_outcome_rejected():
    p = _valid_packet()
    p["approved_candidate_evaluation_outcome"] = "EXECUTE"
    r = m.evaluate_crypto_d1_strategy_candidate_research_design_approval(p)
    assert r["verdict"] == m.RESEARCH_DESIGN_APPROVAL_VERDICT_REJECTED
...
def test_pass_evaluation_outcome_ready():
    p = _valid_packet()
    p["approved_candidate_evaluation_outcome"] = "PASS"
    r = m.evaluate_crypto_d1_strategy_candidate_research_design_approval(p)
    assert r["verdict"] == m.RESEARCH_DESIGN_APPROVAL_VERDICT_READY
...
def test_approval_for_execution_rejected():
    p = _valid_packet()
    p["approval_for_execution"] = True
    r = m.evaluate_crypto_d1_strategy_candidate_research_design_approval(p)
    assert r["verdict"] == m.RESEARCH_DESIGN_APPROVAL_VERDICT_REJECTED
...
def test_weaken_research_design_review_rejected():
    p = _valid_packet()
    p["weaken_research_design_review"] = True
    r = m.evaluate_crypto_d1_strategy_candidate_research_design_approval(p)
    assert r["verdict"] == m.RESEARCH_DESIGN_APPROVAL_VERDICT_REJECTED
...
def test_relaxed_approval_research_only_affirmation_rejected():
    p = _valid_packet()
    p["approval_is_research_only_human_approved_for_next_step_only"] = False
    r = m.evaluate_crypto_d1_strategy_candidate_research_design_approval(p)
    assert r["verdict"] == m.RESEARCH_DESIGN_APPROVAL_VERDICT_REJECTED
...
def test_relaxed_validation_boundary_affirmation_rejected():
    p = _valid_packet()
    p["approved_validation_boundary_blocks_real_execution"] = False
    r = m.evaluate_crypto_d1_strategy_candidate_research_design_approval(p)
    assert r["verdict"] == m.RESEARCH_DESIGN_APPROVAL_VERDICT_REJECTED
...
def test_approver_parks_lane():
    ...
    r = m.evaluate_crypto_d1_strategy_candidate_research_design_approval(p)
    assert r["verdict"] == m.RESEARCH_DESIGN_APPROVAL_VERDICT_PARKED
...
def test_build_returns_fresh_dicts():
    c1 = m.build_crypto_d1_strategy_candidate_research_design_approval_contract(
        _active_signal(), _valid_packet()
    )
    c2 = m.build_crypto_d1_strategy_candidate_research_design_approval_contract(
        _active_signal(), _valid_packet()
    )
    assert c1 is not c2
...
def test_validate_rejects_tampered_evaluation_outcomes():
    ...
    c["research_design_approval_evaluation_outcomes"] = ("PASS", "GO")
    v = (
        m
        .validate_crypto_d1_strategy_candidate_research_design_approval_contract(
            c
...
def test_render_markdown_non_empty():
    ...
    md = (
        m
        .render_crypto_d1_strategy_candidate_research_design_approval_contract_markdown(  # noqa: E501
            c
        )
    )
    assert isinstance(md, str)
    assert "Research Design Approval" in md
    assert "SOL" in md
    assert "XRP" in md
    assert "PASS" in md
...
def test_evaluate_never_raises_on_garbage():
    for junk in (None, 1, "x", [], (), 3.14, True):
        r = m.evaluate_crypto_d1_strategy_candidate_research_design_approval(junk)
        assert r["verdict"] in m.ALLOWED_RESEARCH_DESIGN_APPROVAL_VERDICTS
