...
def test_canonical_ledger_is_21_and_reconciles():
    assert _R["canonical_rejected_families_count"] == 21
    ...
    # SARA's ledger bumps (C14, C15, C16) have been applied; canonical == SARA,
    # no duplicates. Reconciliation holds by set equality.
    assert recon["sara_count"] == 21
    assert recon["canonical_count"] == 21
...
def test_morning_report_summary_has_portfolio_fit_fields():
    ...
    assert summ["excluded_rejected_families_count"] == 21
    top = summ["ranked"][0]
