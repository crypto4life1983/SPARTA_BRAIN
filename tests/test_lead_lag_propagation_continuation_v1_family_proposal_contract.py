"""Tests for the Candidate #13 family proposal contract
(lead_lag_propagation_continuation_v1).

Verifies: chain-gate on the closed C12 rejection record (REJECTED_KEPT_ON_RECORD);
verdict READY; materially-new family; full differentiation from every rejected
family C1-C12 (17 incl. C10/C11/C12); the C10+C11+C12 lessons applied; the hard
early-generalization + anti-drift battery present (incl. must-beat-random-entry,
must-beat-buy-and-hold, must-beat-leader, and the target-capture / horizon cap);
leader != follower (cross-asset propagation, not same-asset trend); proposal-only
...
import sparta_commander.lead_lag_propagation_continuation_v1_family_proposal_contract as c13p  # noqa: E501
...
try:
    _R = c13p.build_candidate_13_family_proposal(".", _tracked_paths())
finally:
...
def test_proposal_ready_and_validates():
    assert _R["verdict"] == c13p.VERDICT_C13P_READY
    assert _R["blockers"] == []
    assert _R["c12_rejection_status"] == "REJECTED_KEPT_ON_RECORD"
    assert c13p.validate_candidate_13_family_proposal(_R)["valid"] is True


def test_chain_gate_requires_c12_closed():
    assert _R["head_at_c12_rejection"] == (
        "7a04fa723afa3f33dc41445af6a36be64f325c07")


def test_identity_number_13_new_family():
    assert _R["candidate_number"] == 13
    assert _R["candidate_family"] == "lead_lag_propagation_continuation"
    assert _R["candidate_id"] == "LEAD_LAG_PROPAGATION_CONTINUATION_V1"
    assert _R["is_materially_new_family"] is True
    assert _R["candidate_family"] not in _R["rejected_families_c1_to_c12"]


# ---- differentiation from EVERY rejected family C1-C12 ---------------------

def test_rejected_ledger_includes_c10_c11_c12():
    led = _R["rejected_families_c1_to_c12"]
    assert "intraweek_calendar_seasonality_drift" in led      # C10
    assert "cross_asset_dispersion_reversion" in led          # C11
    assert "failed_breakdown_reclaim_reversal" in led         # C12
    assert len(led) == 17


def test_differentiation_present_for_every_rejected_family():
    diff = _R["material_difference_from_each_rejected"]
    for f in _R["rejected_families_c1_to_c12"]:
        assert diff.get(f), f
    ...
    assert c13p.validate_candidate_13_family_proposal(bad)["valid"] is False


# ---- C10 + C11 + C12 lessons + the hard anti-drift battery ------------------

def test_lessons_applied():
    joined = " || ".join(_R["c10_c11_c12_lessons_applied"]).lower()
...
def test_hard_early_generalization_and_anti_drift_checks():
    ...
    # tamper: drop the must-beat-random-entry gate -> invalid
    bad = copy.deepcopy(_R)
...
def test_anti_trap_filters_cover_random_drift_leader_forward_oos():
    joined = " || ".join(_R["anti_trap_filters"]).lower()
    assert "random-entry" in joined
    assert "buy-the-leader" in joined or "leader" in joined
    assert "horizon" in joined
    assert "forward-oos" in joined


# ---- entry/exit: leader != follower, not calendar, not carry ---------------

def test_leader_distinct_from_follower_cross_asset():
    assert _R["leader"] == "BTCUSD"
    assert "ETHUSD" in _R["followers"] and "SOLUSD" in _R["followers"]
    assert _R["leader"] not in _R["followers"]
    entry = _R["entry_concept"].lower()
    assert "leader" in entry and "follower" in entry
    assert "trigger asset differs from the traded asset" in entry


def test_entry_not_calendar_not_single_asset_not_carry():
...
def test_data_needs_no_fetch_required():
    ...
    assert dn["leader_asset"] == "BTC"
    for s in ("ETH", "SOL"):
        assert s in dn["follower_assets"]
...
def test_capability_flags_all_false_and_tamper_rejected():
    for flag in c13p._CAPABILITY_FLAGS_FALSE:
        assert _R[flag] is False, flag
        bad = copy.deepcopy(_R)
        bad[flag] = True
        assert c13p.validate_candidate_13_family_proposal(bad)["valid"] is False, flag


def test_next_action_is_human_decision_proposal_only():
    nra = c13p.get_candidate_13_family_proposal_next_action()
    assert nra == "HUMAN_DECISION_C13_ADVANCE_TO_SPEC_OR_REJECT_PROPOSAL"
    label = c13p.get_candidate_13_family_proposal_label()
...
def test_module_purity_no_io_no_main_no_banned_imports():
    src = open(c13p.__file__, encoding="utf-8").read()
