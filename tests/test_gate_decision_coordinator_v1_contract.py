...
# --- 1. clean synced + no open gate in state -> defer to lane (C20 open gate) ---

def test_clean_synced_defers_to_lane_c20_open_gate():
    # the lane now has C20 as the ACTIVE open candidate, so the idle/default
    # recommendation defers to the C20 human spec-decision gate -- it does NOT drift
    # to next-candidate and it is NOT automation readiness.
...
def test_supports_morning_report_output():
    ...
    # idle now defers to the C20 open-candidate spec-decision gate, not a new candidate
    assert summ["next_research_recommended"] is False
    assert summ["paste_this"] == "HUMAN_DECISION_C20_ADVANCE_TO_CANDIDATE_SPEC_OR_REJECT"
...
def test_integrates_with_existing_pieces_does_not_replace():
    ...
    assert gdc.EXPECTED_LEDGER_COUNT == 24
