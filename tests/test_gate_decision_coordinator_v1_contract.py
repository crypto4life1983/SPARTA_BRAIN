...
def test_clean_synced_defers_to_lane_c19_open_gate():
    # the lane now has C19 as the ACTIVE open candidate, so the idle/default
    # recommendation defers to the C19 human spec-decision gate -- it does NOT drift
...
def test_supports_morning_report_output():
    ...
    # idle now defers to the C19 open-candidate spec-decision gate, not a new candidate
    assert summ["next_research_recommended"] is False
    assert summ["paste_this"] == "HUMAN_DECISION_C19_ADVANCE_TO_CANDIDATE_SPEC_OR_REJECT"
