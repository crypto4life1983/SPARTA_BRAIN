...
def test_panel_shows_c16_complete_and_ledger_25():
    p = panel.build_autopilot_morning_panel(_success_report())
    ar = p["automation_readiness"]
    assert ar["c16_lifecycle_complete"] is True
    assert ar["rejected_ledger_count"] == 26
    h = p["html"]
    assert "ACTIVE CANDIDATE" in h    # section header (body says active: none)
    assert "26" in h
...
def test_panel_shows_c21_active_at_proposal_gate():
    ...
    h = p["html"]
    assert "HUMAN_DECISION_OPEN_CANDIDATE_22_FAMILY_PROPOSAL_OR_HOLD" in h
    assert "Active candidate: <b>none</b>" in h
    assert "C21_REJECTED_AT_FEE_HONEST_REPLAY" in h
...
def test_panel_human_gate_workflow_c21_open_gate():
    ...
    assert w["has_open_human_gate"] is False     # C21 rejected -> no open advance gate
    ...
    h = p["html"]
    # dashboard surfaces the workflow header + the no-open-gate message
...
def test_panel_dirty_tree_warns_but_does_not_hide_c16():
    ...
    assert p["automation_readiness"]["rejected_ledger_count"] == 26
...
def test_no_report_still_shows_c16_and_c21_active():
    ...
    assert ar["next_required_action"] == (
        "HUMAN_DECISION_OPEN_CANDIDATE_22_FAMILY_PROPOSAL_OR_HOLD")
    assert "C21_REJECTED_AT_FEE_HONEST_REPLAY" in p["html"]
