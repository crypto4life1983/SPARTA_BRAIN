...
def test_panel_shows_c20_active_at_proposal_gate():
    p = panel.build_autopilot_morning_panel(_success_report())
    ar = p["automation_readiness"]
    assert ar["active_candidate"] == "C20"
    ...
    assert ar["active_candidate_verdict"] == "C20_PROPOSAL_FROZEN_FOR_HUMAN_REVIEW"
    assert ar["last_rejected_candidate"] == "C19"
    assert ar["last_rejected_candidate_verdict"] == "C19_REJECTED_AT_REAL_CANDLE_LABELS"
    h = p["html"]
    assert "HUMAN_DECISION_C20_ADVANCE_TO_CANDIDATE_SPEC_OR_REJECT" in h
    assert "ACTIVE CANDIDATE" in h
    assert "Active candidate: <b>C20</b>" in h
    assert "C20_PROPOSAL_FROZEN_FOR_HUMAN_REVIEW" in h


def test_panel_human_gate_workflow_c20_open_gate():
    p = panel.build_autopilot_morning_panel(_success_report())
    w = p["human_gate_workflow"]
    assert w["available"] is True
    assert w["has_open_human_gate"] is True
    assert w["active_candidate"] == "C20"
    assert w["approval_text_to_paste"] is not None
    assert w["ready_for_commit"] is False
    assert w["commit_approval_text"] is None
    h = p["html"]
    # dashboard surfaces the open C20 gate + a copyable approval
    assert "Human-gate approval workflow" in h
    assert "ADVANCE C20 TO CANDIDATE SPEC" in h
...
def test_panel_dirty_tree_warns_but_does_not_hide_c16():
    ...
    assert p["automation_readiness"]["active_candidate"] == "C20"
    assert "ACTIVE CANDIDATE" in h


def test_no_report_still_shows_c16_and_c20_active():
    p = panel.build_autopilot_morning_panel(None)
    ar = p["automation_readiness"]
    assert ar["c16_lifecycle_complete"] is True
    assert ar["rejected_ledger_count"] == 24
    ...
    assert "C20_PROPOSAL_FROZEN_FOR_HUMAN_REVIEW" in p["html"]
