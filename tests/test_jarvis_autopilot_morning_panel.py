...
def test_panel_shows_c21_active_at_proposal_gate():
    ...
    h = p["html"]
    assert "HUMAN_DECISION_C21_ADVANCE_TO_REAL_CANDLE_LABELS_OR_REJECT" in h
    assert "ACTIVE CANDIDATE" in h
    assert "Active candidate: <b>C21</b>" in h
    assert "C21_DETECTOR_DRY_RUN_FROZEN_FOR_HUMAN_REVIEW" in h
...
def test_panel_human_gate_workflow_c21_open_gate():
    ...
    assert "ADVANCE C21 TO REAL-CANDLE LABELS / REVIEW (FROZEN PUBLIC DATA ONLY)" in h
...
def test_no_report_still_shows_c16_and_c21_active():
    ...
    assert "C21_DETECTOR_DRY_RUN_FROZEN_FOR_HUMAN_REVIEW" in p["html"]
