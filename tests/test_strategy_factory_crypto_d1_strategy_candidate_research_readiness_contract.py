"""Tests for the Crypto-D1 Strategy Candidate Research Readiness Contract (Block 115).

Pure stdlib, read-only, side-effect-free. These tests assert the contract records
the proposed human approval of the Block 113 research design approval, stays gated behind a
clean READY research-design-approval signal pointed at this approval gate, upholds every
...
import sparta_commander.strategy_factory_crypto_d1_strategy_candidate_research_readiness_contract as m  # noqa: E501
...
def _active_signal() -> dict:
    """A clean, READY Block 113 research-design-approval signal pointed at this gate."""
    return {
        "crypto_d1_strategy_candidate_research_design_approval_contract_active": True,
        "research_design_approval_verdict": (
            m.UPSTREAM_REQUIRED_RESEARCH_DESIGN_APPROVAL_VERDICT
        ),
        "next_gate": m.UPSTREAM_REQUIRED_RESEARCH_DESIGN_APPROVAL_NEXT_GATE,
        "mode": m.UPSTREAM_REQUIRED_RESEARCH_DESIGN_APPROVAL_MODE,
        "read_only": True,
        "executes": False,
    }
...
def _valid_packet() -> dict:
    ...
    p: dict = {}
    for f in m.RESEARCH_READINESS_REQUIRED_TEXT_FIELDS:
        p[f] = f + "_value"
    ...
    for a in m.RESEARCH_READINESS_REQUIRED_AFFIRMATIONS:
        p[a] = True
    return p
...
def test_schema_version_preserved():
    assert m.RESEARCH_READINESS_SCHEMA_VERSION == (
        "strategy_factory_crypto_d1_strategy_candidate_research_readiness_"
        "contract.v1"
    )


def test_label_is_research_readiness():
    assert "Research Readiness" in m.DEFAULT_RESEARCH_READINESS_LABEL


def test_status_is_read_only_research_readiness():
    assert m.RESEARCH_READINESS_STATUS == (
...
def test_evaluation_outcomes_are_pass_watch_fail():
    assert m.RESEARCH_READINESS_EVALUATION_OUTCOMES == (
        "PASS", "WATCH", "FAIL"
    )


def test_required_safeguards_and_validation_boundaries_present():
    assert len(m.RESEARCH_READINESS_REQUIRED_SAFEGUARDS) >= 1
    assert "observation_only_evidence_handling" in (
        m.RESEARCH_READINESS_REQUIRED_SAFEGUARDS
    )
    assert len(m.RESEARCH_READINESS_VALIDATION_BOUNDARIES) >= 1
    assert "validation_is_a_separate_later_human_step" in (
        m.RESEARCH_READINESS_VALIDATION_BOUNDARIES
    )


def test_upstream_gating_constants():
    assert m.UPSTREAM_REQUIRED_RESEARCH_DESIGN_APPROVAL_NEXT_GATE == (
        "CRYPTO_D1_STRATEGY_CANDIDATE_RESEARCH_DESIGN_APPROVAL_READY_SEPARATE_"
        "HUMAN_NEXT_STEP_REQUIRED"
    )
    assert m.UPSTREAM_REQUIRED_RESEARCH_DESIGN_APPROVAL_MODE == "RESEARCH_ONLY"
    assert m.UPSTREAM_REQUIRED_RESEARCH_DESIGN_APPROVAL_VERDICT == (
        "STRATEGY_CANDIDATE_RESEARCH_DESIGN_APPROVAL_READY"
    )


def test_verdict_constants_distinct():
    assert len(set(m.ALLOWED_RESEARCH_READINESS_VERDICTS)) == 5


def test_stage_and_action_match_registry_values():
    assert m.RESEARCH_READINESS_CURRENT_STAGE == (
        "CRYPTO_D1_STRATEGY_CANDIDATE_RESEARCH_READINESS_CONTRACT_REQUIRED"
    )
    assert m.RESEARCH_READINESS_NEXT_REQUIRED_ACTION == (
        "BUILD_CRYPTO_D1_STRATEGY_CANDIDATE_RESEARCH_READINESS_CONTRACT"
    )


# --------------------------------------------------------------------------- #
# Upstream gating (only a clean Block 113 READY-for-this-gate signal activates)
# --------------------------------------------------------------------------- #

def test_inactive_signal_awaits():
    c = m.build_crypto_d1_strategy_candidate_research_readiness_contract(
        {}, None
    )
    assert c["research_readiness_verdict"] == (
        m.RESEARCH_READINESS_VERDICT_AWAIT
    )
    assert c[
...
def test_active_signal_activates_and_is_ready():
    ...
    assert c["research_readiness_verdict"] == (
        m.RESEARCH_READINESS_VERDICT_READY
    )


def test_dirty_signal_missing_active_flag_awaits():
    sig = _active_signal()
    del sig["crypto_d1_strategy_candidate_research_design_approval_contract_active"]
    c = m.build_crypto_d1_strategy_candidate_research_readiness_contract(
        sig, _valid_packet()
    )
    assert c["research_readiness_verdict"] == (
...
def test_wrong_verdict_awaits():
    sig = _active_signal()
    sig["research_design_approval_verdict"] = "SOMETHING_ELSE"
    c = m.build_crypto_d1_strategy_candidate_research_readiness_contract(
        sig, _valid_packet()
    )
    assert c["research_readiness_verdict"] == (
...
def test_valid_packet_is_ready():
    r = m.evaluate_crypto_d1_strategy_candidate_research_readiness(
        _valid_packet()
    )
    assert r["verdict"] == m.RESEARCH_READINESS_VERDICT_READY


def test_empty_packet_needs_more_info():
    r = m.evaluate_crypto_d1_strategy_candidate_research_readiness({})
    assert r["verdict"] == m.RESEARCH_READINESS_VERDICT_NEEDS_MORE_INFO


def test_missing_text_field_needs_more_info():
    p = _valid_packet()
    del p["approval_decision_rationale"]
    r = m.evaluate_crypto_d1_strategy_candidate_research_readiness(p)
    assert r["verdict"] == m.RESEARCH_READINESS_VERDICT_NEEDS_MORE_INFO
...
def test_go_approved_bot_state_rejected():
    ...
    r = m.evaluate_crypto_d1_strategy_candidate_research_readiness(p)
    assert r["verdict"] == m.RESEARCH_READINESS_VERDICT_REJECTED
...
def test_weaken_research_design_approval_rejected():
    p = _valid_packet()
    p["weaken_research_design_approval"] = True
    r = m.evaluate_crypto_d1_strategy_candidate_research_readiness(p)
    assert r["verdict"] == m.RESEARCH_READINESS_VERDICT_REJECTED
...
def test_approver_parks_lane():
    ...
    r = m.evaluate_crypto_d1_strategy_candidate_research_readiness(p)
    assert r["verdict"] == m.RESEARCH_READINESS_VERDICT_PARKED
...
def test_build_returns_fresh_dicts():
    c1 = m.build_crypto_d1_strategy_candidate_research_readiness_contract(
        _active_signal(), _valid_packet()
    )
    c2 = m.build_crypto_d1_strategy_candidate_research_readiness_contract(
        _active_signal(), _valid_packet()
    )
    assert c1 is not c2
...
def test_validate_rejects_tampered_evaluation_outcomes():
    ...
    c["research_readiness_evaluation_outcomes"] = ("PASS", "GO")
    v = (
        m
        .validate_crypto_d1_strategy_candidate_research_readiness_contract(
            c
...
def test_render_markdown_non_empty():
    ...
    md = (
        m
        .render_crypto_d1_strategy_candidate_research_readiness_contract_markdown(  # noqa: E501
            c
        )
    )
    assert isinstance(md, str)
    assert "Research Readiness" in md
    assert "SOL" in md
    assert "XRP" in md
    assert "PASS" in md
...
def test_evaluate_never_raises_on_garbage():
    for junk in (None, 1, "x", [], (), 3.14, True):
        r = m.evaluate_crypto_d1_strategy_candidate_research_readiness(junk)
        assert r["verdict"] in m.ALLOWED_RESEARCH_READINESS_VERDICTS
...
def test_module_and_test_in_commander_allowlist():
    ...
    test_rel = (
        "tests/"
        "test_strategy_factory_crypto_d1_strategy_candidate_research_readiness_"
        "contract.py"
    )
    all_listed = []
