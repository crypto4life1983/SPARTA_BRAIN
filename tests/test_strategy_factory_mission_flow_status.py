...
def test_next_required_action_is_human_controlled_real_data_qa_boundary_decision():
    ...
    assert NEXT_REQUIRED_ACTION == (
        "HUMAN_SELECT_RESEARCH_CONTINUATION_DIRECTION"
    )
    assert not NEXT_REQUIRED_ACTION.startswith("BUILD_")
    # a safe research-only direction selection, not an execution / authorization
    assert "RESEARCH" in NEXT_REQUIRED_ACTION
    assert "CONTINUATION" in NEXT_REQUIRED_ACTION
    assert NEXT_REQUIRED_ACTION != (
    ...
    # the research-continuation direction selection is the active STATE_NEXT human step
    nxt = pipe["crypto_d1_post_resume_policy_research_continuation_direction_selection"]
...
def test_research_continuation_direction_selection_is_next_and_qa_boundary_is_blocked():
    pipe = {r["id"]: r for r in machine_pipeline_lane()}
    # the resume-policy results review and human review record are now complete
    assert (
    ...
    # the research-continuation direction selection is the active human-gated next step
    nxt = pipe["crypto_d1_post_resume_policy_research_continuation_direction_selection"]
    assert nxt["state"] == STATE_NEXT
    assert NEXT_REQUIRED_ACTION in nxt["reason"]
    ...
    # the DO_NOT_PROMOTE decision is preserved, never overturned by the plan
...
def test_latest_completed_bitcoin_cycle_timing_evidence_contract_is_block_123():
    ...
    assert s["current_stage"] == (
        "POST_RESUME_POLICY_RESEARCH_CONTINUATION_DIRECTION_SELECTION_REQUIRED"
    )
...
def test_latest_completed_overnight_research_autopilot_controller_is_block_152():
    ...
    # the research-continuation direction selection is the active next step (STATE_NEXT)
    assert pipe[
        "crypto_d1_post_resume_policy_research_continuation_direction_selection"
    ]["state"] == STATE_NEXT
    # the QA boundary decision remains its own separate BLOCKED step
...
def test_current_stage_is_human_controlled_real_data_qa_boundary_decision():
    ...
    assert "RESEARCH_CONTINUATION" in CURRENT_STAGE
