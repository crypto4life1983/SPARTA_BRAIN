"""Tests for the Candidate #13 candidate-spec contract
(lead_lag_propagation_continuation_v1).

Verifies: chain-gate on the READY C13 family proposal; verdict READY; frozen spec
rules (leader BTC != followers ETH/SOL daily spot, no fetch; confirmed leader move
z>=1.5; lag r_F < 0.5*r_L; ATR(14) stop on the follower; 81 bps floor over 37 bps
cost; 1R/1.5R/2R; <=2-bar hold; conservative stop-first straddle; per-follower
resolved-exit non-overlap); ALL THREE baseline gates incl. buy-the-leader; the
full early battery incl. horizon cap; the one-edit allowance; no calendar/weekday
trigger; no long-drift/bull-carry reliance; all execution gates locked; ~44
...
import sparta_commander.lead_lag_propagation_continuation_v1_candidate_spec_contract as c13s  # noqa: E501
...
try:
    _R = c13s.build_candidate_13_spec(".", _tracked_paths())
finally:
...
def test_spec_ready_and_validates():
    assert _R["verdict"] == c13s.VERDICT_C13S_READY
    assert _R["blockers"] == []
    assert _R["proposal_verdict"] == c13s.VERDICT_C13P_READY
    assert c13s.validate_candidate_13_spec(_R)["valid"] is True


def test_chain_gate_on_pushed_proposal():
    assert _R["head_at_c13_proposal"] == (
        "ae97e896ba2fc924df82e97beafb2ecdbaa4c739")
    assert _R["candidate_number"] == 13
    assert _R["candidate_family"] == "lead_lag_propagation_continuation"


# ---- 1-2 leader != follower, daily spot, no fetch --------------------------

def test_leader_follower_daily_spot_no_fetch():
    assert _R["leader"] == "BTCUSD"
    assert _R["followers"] == ["ETHUSD", "SOLUSD"]
    assert _R["leader"] not in _R["followers"]
    assert _R["min_followers_required"] == 2
    assert _R["timeframe"] == "1d" and _R["market_type"] == "spot"
    assert _R["no_data_fetch"] is True
    bad = copy.deepcopy(_R)
    bad["followers"] = ["BTCUSD", "ETHUSD"]   # leader in followers
    assert c13s.validate_candidate_13_spec(bad)["valid"] is False


# ---- 3 leader move + lag + entry -------------------------------------------

def test_leader_move_lag_and_entry():
    assert _R["leader_z_entry_threshold"] == 1.5
    assert _R["lag_margin_fraction"] == 0.5
    ec = _R["entry_concept"]
    assert ec["trigger_asset_differs_from_traded_asset"] is True
    assert ec["weekday_or_calendar_trigger"] is False
    assert ec["relies_on_long_drift_or_bull_carry"] is False
    assert _R["leader_move_definition"] and _R["lag_condition_definition"]
    for tamper, val in (("leader_z_entry_threshold", 0.0),
                        ("lag_margin_fraction", 9.0)):
        bad = copy.deepcopy(_R)
        bad[tamper] = val
        assert c13s.validate_candidate_13_spec(bad)["valid"] is False, tamper
...
def test_stop_targets_floor_cost_hold():
    ...
    assert rg["stop_atr_multiplier"] == 1.0
    ...
    for tamper, val in (("target_distance_floor_bps", 1.0),
                        ("max_hold_bars", 9)):
        bad = copy.deepcopy(_R)
        bad[tamper] = val
        assert c13s.validate_candidate_13_spec(bad)["valid"] is False, tamper
...
def test_non_overlap_per_follower():
    assert _R["non_overlap_policy"]["rule"] == (
        "per_follower_resolved_exit_reduce_or_keep_only_never_add")


# ---- 9 ALL THREE baseline gates (incl. buy-the-leader) ---------------------

def test_baseline_comparisons_all_three_required():
    bc = _R["baseline_comparisons"]
    assert bc["matched_buy_and_hold"] and bc["matched_random_entry"]
    assert bc["buy_the_leader"]
    assert bc["all_three_required"] is True
    assert bc["failing_any_is_structural_rejection"] is True
    bad = copy.deepcopy(_R)
    bad["baseline_comparisons"].pop("buy_the_leader")
    assert c13s.validate_candidate_13_spec(bad)["valid"] is False
...
def test_early_battery_and_horizon_cap():
    ...
    assert aog["must_beat_buy_the_leader"] is True
...
def test_sample_size_minimums():
    ...
    assert ss["min_accepted_per_follower"] == 20
...
def test_capability_flags_all_false_and_tamper_rejected():
    for flag in c13s._CAPABILITY_FLAGS_FALSE:
        assert _R[flag] is False, flag
        bad = copy.deepcopy(_R)
        bad[flag] = True
        assert c13s.validate_candidate_13_spec(bad)["valid"] is False, flag


def test_next_action_and_label():
    nra = c13s.get_candidate_13_spec_next_action()
    assert nra == "HUMAN_DECISION_C13_ADVANCE_TO_DETECTOR_SPEC_OR_REJECT"
    label = c13s.get_candidate_13_spec_label()
...
def test_module_purity_no_io_no_main_no_banned_imports():
    src = open(c13s.__file__, encoding="utf-8").read()
