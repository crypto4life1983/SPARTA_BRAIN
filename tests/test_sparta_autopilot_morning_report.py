...
def test_autopilot_plan_idle_defers_to_automation_readiness():
    # C20 is REJECTED with no active candidate, so the idle plan defers to AUTOMATION
    # READINESS -- NOT a new candidate, NOT an open candidate gate.
    report = mr.build_morning_report(_success_run_state(), _git_summary(),
...
def test_morning_report_shows_c20_rejected_section():
    report = mr.build_morning_report(_success_run_state(), _git_summary(),
                                     _all_rejected_status())
    ...
    assert "C20_REJECTED_AT_FEE_HONEST_REPLAY" in md
    assert "BUILD_AUTOMATION_READINESS_STEP_RESEARCH_ONLY" in md
    # the what-to-do-next line points at automation readiness
    assert "BUILD_AUTOMATION_READINESS_STEP_RESEARCH_ONLY" in (
