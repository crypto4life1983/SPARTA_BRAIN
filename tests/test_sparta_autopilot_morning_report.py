...
def test_autopilot_plan_idle_defers_to_c21_active_gate():
    ...
    assert "HUMAN_DECISION_C21_ADVANCE_TO_CANDIDATE_SPEC_OR_REJECT" in md
...
def test_morning_report_shows_c21_active_section():
    ...
    assert ar["next_stage"] == "c21_candidate_spec_decision"
    ...
    assert "C21_PROPOSAL_FROZEN_FOR_HUMAN_REVIEW" in md
    assert "HUMAN_DECISION_C21_ADVANCE_TO_CANDIDATE_SPEC_OR_REJECT" in md
    # the what-to-do-next line points at the C21 spec-decision gate
    assert "HUMAN_DECISION_C21_ADVANCE_TO_CANDIDATE_SPEC_OR_REJECT" in (
