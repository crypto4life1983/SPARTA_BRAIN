...
Review Contract (Bundle 52).

Bundle 52 is a PURE, stdlib-only, read-only *paper decision-review contract*
builder. It consumes a Bundle 51 crypto-d1 research-only dry-run DECISION
contract and, only when that decision contract is active + carries the Bundle 51
READY verdict (DRY_RUN_DECISION_READY_FOR_NEXT_RESEARCH_ONLY_CONTRACT) + the
Bundle 51 ready next-gate
(CRYPTO_D1_RESEARCH_ONLY_DRY_RUN_DECISION_READY_SEPARATE_HUMAN_NEXT_CONTRACT_REQUIRED),
evaluates a proposed dry-run DECISION REVIEW packet and returns a deterministic
verdict describing whether a human reviewed, on paper, the research-only dry-run
decision and confirmed the proposed next step is another research-only paper
contract -- or whether the lane should park, need more info, or be rejected. It
performs NO dry run, reviews on paper only, acquires nothing, fetches nothing,
...
nothing. A READY verdict unlocks NOTHING real and still requires a separate,
later, human step to build the proposed next contract.

Coverage:
- schema / label / status / state constants stable
- verdict + gate constants stable; ordering
- activation strictly gated on Bundle 51 active + READY + ready-gate
...
- a real Bundle 49->50->51 chain activates Bundle 52 end-to-end
"""
...
_MODPATH = (
    ...
    / "sparta_commander"
    / "strategy_factory_crypto_d1_research_only_dry_run_decision_review_contract.py"  # noqa: E501
)
_SAFETY_PATH = (
    pathlib.Path(__file__).resolve().parents[1]
    / "sparta_commander"
...
_DECISION_PACKET_ID = "DP-1"


# --- fixtures --------------------------------------------------------------

def _synthetic_decision_contract():
    """A Bundle-51-shaped dict that activates the Bundle 52 build."""
    return {
        "crypto_d1_research_only_dry_run_decision_contract_active": True,
        "dry_run_decision_verdict": UPSTREAM_REQUIRED_DECISION_VERDICT,
        "next_gate": UPSTREAM_REQUIRED_DECISION_NEXT_GATE,
        "evaluated_decision_packet": {
            "decision_packet_id": _DECISION_PACKET_ID,
        },
        "idea_id": "IDEA-1",
...
def _good_decision_review_packet():
    """A complete, safe, research-only dry-run decision-review packet."""
    p = {
        "decision_review_packet_id": "DRP-1",
        "upstream_decision_id": _DECISION_PACKET_ID,
...
def _build(upstream=None, packet=None):
    return build_crypto_d1_research_only_dry_run_decision_review_contract(
        upstream if upstream is not None else _synthetic_decision_contract(),
        packet if packet is not None else _good_decision_review_packet(),
    )
...
def test_schema_version_stable():
    assert DECISION_REVIEW_SCHEMA_VERSION == (
...
def test_verdict_constants_stable():
    assert DECISION_REVIEW_VERDICT_READY == "DRY_RUN_DECISION_REVIEW_READY"
    assert DECISION_REVIEW_VERDICT_NEEDS_MORE_INFO == (
        "DRY_RUN_DECISION_REVIEW_NEEDS_MORE_INFO"
    )
    assert DECISION_REVIEW_VERDICT_REJECTED == "DRY_RUN_DECISION_REVIEW_REJECTED"
    assert DECISION_REVIEW_VERDICT_PARKED == "DRY_RUN_DECISION_REVIEW_PARKED"
    assert DECISION_REVIEW_VERDICT_AWAIT == (
        "AWAIT_CRYPTO_D1_RESEARCH_ONLY_DRY_RUN_DECISION_CONTRACT"
    )


def test_allowed_verdicts_order():
    assert ALLOWED_DECISION_REVIEW_VERDICTS == (
        DECISION_REVIEW_VERDICT_READY,
        DECISION_REVIEW_VERDICT_NEEDS_MORE_INFO,
        DECISION_REVIEW_VERDICT_REJECTED,
        DECISION_REVIEW_VERDICT_PARKED,
...
def test_upstream_required_signal_constants():
    assert UPSTREAM_REQUIRED_DECISION_VERDICT == (
        "DRY_RUN_DECISION_READY_FOR_NEXT_RESEARCH_ONLY_CONTRACT"
    )
    assert UPSTREAM_REQUIRED_DECISION_NEXT_GATE == (
...
def test_next_gate_constants_stable():
    assert NEXT_GATE_CRYPTO_D1_RESEARCH_ONLY_DRY_RUN_DECISION_REVIEW_READY_HUMAN_NEXT_CONTRACT_REQUIRED == (  # noqa: E501
        "CRYPTO_D1_RESEARCH_ONLY_DRY_RUN_DECISION_REVIEW_READY_SEPARATE_HUMAN_"
        "NEXT_CONTRACT_REQUIRED"
    )
    assert NEXT_GATE_CRYPTO_D1_DRY_RUN_DECISION_REVIEW_FIX_REQUIRED == (
        "CRYPTO_D1_DRY_RUN_DECISION_REVIEW_FIX_REQUIRED"
    )
    assert NEXT_GATE_CRYPTO_D1_DRY_RUN_DECISION_REVIEW_PARKED == (
        "CRYPTO_D1_DRY_RUN_DECISION_REVIEW_PARKED"
    )
    assert NEXT_GATE_CRYPTO_D1_DRY_RUN_DECISION_REVIEW_REJECTED == (
        "CRYPTO_D1_DRY_RUN_DECISION_REVIEW_REJECTED"
    )
    assert NEXT_GATE_AWAIT_CRYPTO_D1_RESEARCH_ONLY_DRY_RUN_DECISION_CONTRACT == (
        "AWAIT_CRYPTO_D1_RESEARCH_ONLY_DRY_RUN_DECISION_CONTRACT"
    )


def test_next_required_action_and_stage_are_strings():
    assert isinstance(DRY_RUN_DECISION_REVIEW_NEXT_REQUIRED_ACTION, str)
    assert isinstance(DRY_RUN_DECISION_REVIEW_CURRENT_STAGE, str)
    assert DECISION_CRYPTO_D1_DRY_RUN_DECISION_REVIEW_REQUIRED == (
...
def test_required_text_fields_exact():
    assert DECISION_REVIEW_REQUIRED_TEXT_FIELDS == (
...
def test_required_affirmations_complete():
    for flag in (
        ...
        assert flag in DECISION_REVIEW_REQUIRED_AFFIRMATIONS, flag
    assert len(DECISION_REVIEW_REQUIRED_AFFIRMATIONS) == 21


def test_required_decision_review_fields_is_union():
    assert REQUIRED_DECISION_REVIEW_FIELDS == (
        DECISION_REVIEW_REQUIRED_TEXT_FIELDS
        + DECISION_REVIEW_REQUIRED_AFFIRMATIONS
    )
    assert len(REQUIRED_DECISION_REVIEW_FIELDS) == 32


def test_allowed_modes_and_scopes():
    assert ALLOWED_REVIEW_MODES == ("research_only", "research-only")
    assert "dry_run_decision_review_only" in ALLOWED_REVIEW_SCOPES
    assert "decision_review" in ALLOWED_REVIEW_SCOPES


def test_safety_posture_all_false():
    assert len(DECISION_REVIEW_SAFETY_POSTURE) > 0
    assert all(v is False for v in DECISION_REVIEW_SAFETY_POSTURE.values())
...
def test_forbidden_allow_flags_cover_real_capabilities():
    for flag in (
        ...
        assert flag in DECISION_REVIEW_FORBIDDEN_ALLOW_FLAGS, flag
...
def test_active_when_upstream_ready_and_gate():
    ...
    assert c["dry_run_decision_review_required"] == (
        DECISION_CRYPTO_D1_DRY_RUN_DECISION_REVIEW_REQUIRED
    )


def test_inactive_when_upstream_empty():
    c = build_crypto_d1_research_only_dry_run_decision_review_contract({}, {})
    assert c[
        "crypto_d1_research_only_dry_run_decision_review_contract_active"
    ] is False
    assert c["dry_run_decision_review_verdict"] == DECISION_REVIEW_VERDICT_AWAIT
    assert c["next_gate"] == (
        NEXT_GATE_AWAIT_CRYPTO_D1_RESEARCH_ONLY_DRY_RUN_DECISION_CONTRACT
    )
...
def test_inactive_when_upstream_not_active():
    up = _synthetic_decision_contract()
    up["crypto_d1_research_only_dry_run_decision_contract_active"] = False
    c = _build(up, _good_decision_review_packet())
    assert c[
        "crypto_d1_research_only_dry_run_decision_review_contract_active"
    ] is False
    assert c["dry_run_decision_review_verdict"] == DECISION_REVIEW_VERDICT_AWAIT
...
def test_inactive_when_wrong_verdict():
    up = _synthetic_decision_contract()
    up["dry_run_decision_verdict"] = "DRY_RUN_DECISION_NEEDS_MORE_INFO"
    c = _build(up, _good_decision_review_packet())
    assert c[
        "crypto_d1_research_only_dry_run_decision_review_contract_active"
    ] is False
...
def test_inactive_when_upstream_garbage():
    for bad in (None, [], "x", 7, {"foo": "bar"}):
        c = build_crypto_d1_research_only_dry_run_decision_review_contract(
            bad, _good_decision_review_packet()
        )
        assert c[
            "crypto_d1_research_only_dry_run_decision_review_contract_active"
        ] is False
        assert c["dry_run_decision_review_verdict"] == (
            DECISION_REVIEW_VERDICT_AWAIT
        )
...
def test_await_verdict_ignores_packet_shape():
    ...
    bad_packet = {**_good_decision_review_packet(), "allow_data_fetch": True}
    c = build_crypto_d1_research_only_dry_run_decision_review_contract(
        {}, bad_packet
    )
    assert c["dry_run_decision_review_verdict"] == DECISION_REVIEW_VERDICT_AWAIT
...
def test_ready_when_complete_and_safe():
    c = _build()
    assert c["dry_run_decision_review_verdict"] == DECISION_REVIEW_VERDICT_READY
    assert c["next_gate"] == (
        NEXT_GATE_CRYPTO_D1_RESEARCH_ONLY_DRY_RUN_DECISION_REVIEW_READY_HUMAN_NEXT_CONTRACT_REQUIRED  # noqa: E501
    )
...
def test_evaluate_ready_direct():
    ev = evaluate_crypto_d1_research_only_dry_run_decision_review(
        _good_decision_review_packet(),
        {"decision_packet_id": _DECISION_PACKET_ID},
    )
    assert ev["verdict"] == DECISION_REVIEW_VERDICT_READY
...
def test_ready_research_dash_only_mode_accepted():
    pkt = {**_good_decision_review_packet(), "review_mode": "research-only"}
    ev = evaluate_crypto_d1_research_only_dry_run_decision_review(pkt, {})
    assert ev["verdict"] == DECISION_REVIEW_VERDICT_READY
...
def test_empty_packet_needs_more_info():
    ev = evaluate_crypto_d1_research_only_dry_run_decision_review({}, {})
    assert ev["verdict"] == DECISION_REVIEW_VERDICT_NEEDS_MORE_INFO
    assert ev["reasons"] == ("dry_run_decision_review_packet_missing",)


def test_missing_each_text_field_needs_more_info():
    for key in DECISION_REVIEW_REQUIRED_TEXT_FIELDS:
        pkt = _good_decision_review_packet()
        del pkt[key]
        ev = evaluate_crypto_d1_research_only_dry_run_decision_review(
            pkt, {"decision_packet_id": _DECISION_PACKET_ID}
        )
        assert ev["verdict"] == DECISION_REVIEW_VERDICT_NEEDS_MORE_INFO, key
...
def test_missing_each_affirmation_needs_more_info():
    for flag in DECISION_REVIEW_REQUIRED_AFFIRMATIONS:
        ...
        assert ev["verdict"] == DECISION_REVIEW_VERDICT_NEEDS_MORE_INFO, flag
...
def test_build_needs_more_info_sets_fix_gate():
    ...
    c = _build(_synthetic_decision_contract(), pkt)
    assert c["dry_run_decision_review_verdict"] == (
        DECISION_REVIEW_VERDICT_NEEDS_MORE_INFO
    )
    assert c["next_gate"] == (
...
def test_reject_each_forbidden_allow_flag():
    for flag in DECISION_REVIEW_FORBIDDEN_ALLOW_FLAGS:
        pkt = {**_good_decision_review_packet(), flag: True}
        ev = evaluate_crypto_d1_research_only_dry_run_decision_review(
            pkt, {"decision_packet_id": _DECISION_PACKET_ID}
        )
        assert ev["verdict"] == DECISION_REVIEW_VERDICT_REJECTED, flag
        assert f"forbidden_allow:{flag}" in ev["reasons"], flag


def test_reject_real_data_acquisition_allow_flag():
    pkt = {**_good_decision_review_packet(), "allow_real_data_acquisition": True}
    ev = evaluate_crypto_d1_research_only_dry_run_decision_review(pkt, {})
    assert ev["verdict"] == DECISION_REVIEW_VERDICT_REJECTED


def test_reject_data_fetch_allow_flag():
    pkt = {**_good_decision_review_packet(), "approves_data_fetch": True}
    ev = evaluate_crypto_d1_research_only_dry_run_decision_review(pkt, {})
    assert ev["verdict"] == DECISION_REVIEW_VERDICT_REJECTED


def test_reject_data_inspection_allow_flag():
    pkt = {**_good_decision_review_packet(), "allow_data_inspection": True}
    ev = evaluate_crypto_d1_research_only_dry_run_decision_review(pkt, {})
    assert ev["verdict"] == DECISION_REVIEW_VERDICT_REJECTED


def test_reject_dataset_loading_allow_flag():
    pkt = {**_good_decision_review_packet(), "allow_dataset_loading": True}
    ev = evaluate_crypto_d1_research_only_dry_run_decision_review(pkt, {})
    assert ev["verdict"] == DECISION_REVIEW_VERDICT_REJECTED
...
def test_reject_trade_signal_allow_flag():
    pkt = {**_good_decision_review_packet(), "approves_trade_signal": True}
    ev = evaluate_crypto_d1_research_only_dry_run_decision_review(pkt, {})
    assert ev["verdict"] == DECISION_REVIEW_VERDICT_REJECTED


def test_reject_market_data_validation_allow_flag():
    pkt = {**_good_decision_review_packet(), "validates_market_data": True}
    ev = evaluate_crypto_d1_research_only_dry_run_decision_review(pkt, {})
    assert ev["verdict"] == DECISION_REVIEW_VERDICT_REJECTED
...
def test_reject_automation_trigger_allow_flag():
    pkt = {**_good_decision_review_packet(), "allow_automation_trigger": True}
    ev = evaluate_crypto_d1_research_only_dry_run_decision_review(pkt, {})
    assert ev["verdict"] == DECISION_REVIEW_VERDICT_REJECTED
...
def test_reject_dry_run_execution_allow_flag():
    pkt = {**_good_decision_review_packet(), "executes_dry_run": True}
    ev = evaluate_crypto_d1_research_only_dry_run_decision_review(pkt, {})
    assert ev["verdict"] == DECISION_REVIEW_VERDICT_REJECTED


def test_reject_review_specific_allow_flags():
    for flag in (
        "review_approves_execution",
        "review_allows_real_data",
        "review_unlocks_qa",
...
def test_reject_proceed_to_real_contract_flag():
    pkt = {**_good_decision_review_packet(), "proceed_to_real_contract": True}
    ev = evaluate_crypto_d1_research_only_dry_run_decision_review(pkt, {})
    assert ev["verdict"] == DECISION_REVIEW_VERDICT_REJECTED


def test_reject_next_real_contract_flag():
    pkt = {**_good_decision_review_packet(), "allow_next_real_contract": True}
    ev = evaluate_crypto_d1_research_only_dry_run_decision_review(pkt, {})
    assert ev["verdict"] == DECISION_REVIEW_VERDICT_REJECTED
...
def test_reject_relaxed_each_affirmation():
    for flag in DECISION_REVIEW_REQUIRED_AFFIRMATIONS:
        pkt = {**_good_decision_review_packet(), flag: False}
        ev = evaluate_crypto_d1_research_only_dry_run_decision_review(
            pkt, {"decision_packet_id": _DECISION_PACKET_ID}
        )
        assert ev["verdict"] == DECISION_REVIEW_VERDICT_REJECTED, flag
...
def test_reject_disallowed_review_mode():
    pkt = {**_good_decision_review_packet(), "review_mode": "live"}
    ev = evaluate_crypto_d1_research_only_dry_run_decision_review(pkt, {})
    assert ev["verdict"] == DECISION_REVIEW_VERDICT_REJECTED
    assert "disallowed_value:review_mode" in ev["reasons"]


def test_reject_disallowed_review_scope():
    pkt = {**_good_decision_review_packet(), "review_scope": "full_execution"}
    ev = evaluate_crypto_d1_research_only_dry_run_decision_review(pkt, {})
    assert ev["verdict"] == DECISION_REVIEW_VERDICT_REJECTED
    assert "disallowed_value:review_scope" in ev["reasons"]


def test_reject_automated_reviewer_name():
    pkt = {**_good_decision_review_packet(), "reviewer_name_or_id": "automation"}
    ev = evaluate_crypto_d1_research_only_dry_run_decision_review(pkt, {})
    assert ev["verdict"] == DECISION_REVIEW_VERDICT_REJECTED
...
def test_reject_automated_reviewer_type_key():
    pkt = {**_good_decision_review_packet(), "reviewer_type": "bot"}
    ev = evaluate_crypto_d1_research_only_dry_run_decision_review(pkt, {})
    assert ev["verdict"] == DECISION_REVIEW_VERDICT_REJECTED
...
def test_reject_upstream_id_mismatch():
    pkt = {**_good_decision_review_packet(), "upstream_decision_id": "WRONG"}
    ev = evaluate_crypto_d1_research_only_dry_run_decision_review(
        pkt, {"decision_packet_id": _DECISION_PACKET_ID}
    )
    assert ev["verdict"] == DECISION_REVIEW_VERDICT_REJECTED
    assert "mismatch:upstream_decision_id" in ev["reasons"]


def test_no_mismatch_when_ref_absent():
    pkt = {**_good_decision_review_packet(), "upstream_decision_id": "ANYTHING"}
    ev = evaluate_crypto_d1_research_only_dry_run_decision_review(pkt, {})
    assert ev["verdict"] == DECISION_REVIEW_VERDICT_READY


def test_build_rejected_sets_reject_gate():
    pkt = {**_good_decision_review_packet(), "allow_data_fetch": True}
    c = _build(_synthetic_decision_contract(), pkt)
    assert c["dry_run_decision_review_verdict"] == (
        DECISION_REVIEW_VERDICT_REJECTED
    )
...
def test_park_via_flag():
    pkt = {**_good_decision_review_packet(), "park": True}
    ev = evaluate_crypto_d1_research_only_dry_run_decision_review(pkt, {})
    assert ev["verdict"] == DECISION_REVIEW_VERDICT_PARKED


def test_park_via_reviewer_decision():
    pkt = {**_good_decision_review_packet(), "reviewer_decision": "deferred"}
    ev = evaluate_crypto_d1_research_only_dry_run_decision_review(pkt, {})
    assert ev["verdict"] == DECISION_REVIEW_VERDICT_PARKED


def test_build_parked_sets_park_gate():
    pkt = {**_good_decision_review_packet(), "parked": True}
    c = _build(_synthetic_decision_contract(), pkt)
    assert c["dry_run_decision_review_verdict"] == DECISION_REVIEW_VERDICT_PARKED
    assert c["next_gate"] == NEXT_GATE_CRYPTO_D1_DRY_RUN_DECISION_REVIEW_PARKED
...
def test_no_verdict_authorizes_or_executes():
    cases = [
        _good_decision_review_packet(),                                 # READY
        {**_good_decision_review_packet(), "allow_data_fetch": True},   # REJECT
        {**_good_decision_review_packet(), "park": True},               # PARKED
        {},                                                             # MORE
    ]
    for pkt in cases:
        c = _build(_synthetic_decision_contract(), pkt)
...
def test_contract_mode_always_research_only():
    ...
    assert build_crypto_d1_research_only_dry_run_decision_review_contract(
        {}, {}
    )["mode"] == "RESEARCH_ONLY"


def test_contract_stage_always_decision_review_only():
    assert _build()["stage"] == (
...
def test_blocked_capabilities_present_and_nonempty():
    c = _build()
    assert len(c["decision_review_blocked_capabilities"]) >= 1
...
def test_no_verdict_carries_real_capability_marker():
    for pkt in (
        _good_decision_review_packet(),
        {},
        {**_good_decision_review_packet(), "park": True},
    ):
        c = _build(_synthetic_decision_contract(), pkt)
        for flag in DECISION_REVIEW_FORBIDDEN_ALLOW_FLAGS:
...
def test_no_verdict_unlocks_upstream_capability():
    ...
    c = _build(up, _good_decision_review_packet())
    echoed = c["crypto_d1_research_only_dry_run_decision_contract"]
...
def test_validate_public_matches_embedded():
    c = _build()
    v = validate_crypto_d1_research_only_dry_run_decision_review_contract(c)
...
def test_validate_handles_garbage():
    for bad in (None, {}, [], "x", 7):
        v = validate_crypto_d1_research_only_dry_run_decision_review_contract(
            bad
        )
        assert v["valid"] is False
...
def test_build_is_deterministic():
    a = _build()
    b = _build()
    a.pop("crypto_d1_research_only_dry_run_decision_contract", None)
    b.pop("crypto_d1_research_only_dry_run_decision_contract", None)
    assert a == b


def test_evaluate_is_deterministic():
    p = _good_decision_review_packet()
    assert (
        evaluate_crypto_d1_research_only_dry_run_decision_review(p, {})
        == evaluate_crypto_d1_research_only_dry_run_decision_review(p, {})
    )
...
def test_input_packet_not_mutated():
    p = _good_decision_review_packet()
    snapshot = dict(p)
    _build(_synthetic_decision_contract(), p)
    assert p == snapshot
...
def test_markdown_nonempty_and_titled():
    md = render_crypto_d1_research_only_dry_run_decision_review_contract_markdown(  # noqa: E501
        _build()
    )
...
def test_commander_2_safety_allowlist_includes_new_paths():
    ...
    assert (
        ...
        'decision_review_contract.py"' in src
    )
    assert (
...
def test_real_bundle_51_chain_activates_bundle_52():
    """Build a REAL Bundle 49->50->51 contract and confirm it activates
    Bundle 52 end-to-end (no synthetic upstream)."""
    ...
    # Sanity: the real Bundle 51 contract is active + READY + ready-gate.
    assert decision[
        "crypto_d1_research_only_dry_run_decision_contract_active"
    ] is True
    assert decision["dry_run_decision_verdict"] == (
        UPSTREAM_REQUIRED_DECISION_VERDICT
    )
    assert decision["next_gate"] == UPSTREAM_REQUIRED_DECISION_NEXT_GATE

    review_packet_52 = {**_good_decision_review_packet(),
                        "upstream_decision_id": "DP-1"}
    c = build_crypto_d1_research_only_dry_run_decision_review_contract(
        decision, review_packet_52
    )
    assert c[
        "crypto_d1_research_only_dry_run_decision_review_contract_active"
    ] is True
