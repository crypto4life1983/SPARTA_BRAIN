...
# ---- 2. next directive = C19 spec decision (C19 active, C18 last rejected) ----

def test_next_directive_is_c19_spec_decision():
    ...
    assert _R["active_candidate_verdict"] == "C19_PROPOSAL_FROZEN_FOR_HUMAN_REVIEW"
...
def test_surfaces_agree_on_c19_directive():
    assert _R["surfaces_agree"] is True
    assert _R["all_tokens_match"] is True
    s = _R["surfaces"]
    token = "HUMAN_DECISION_C19_ADVANCE_TO_CANDIDATE_SPEC_OR_REJECT"
...
def test_coordinator_idle_defers_to_lane_c19_open_gate():
    # the coordinator defers to the lane, which now has C19 as the ACTIVE open
    # candidate -> recommend its human spec decision, not automation readiness.
    d = gdc.coordinate({
        "repo": {"clean": True, "ahead": 0, "behind": 0,
                 "uncommitted_changes": False},
        "ledger": {"canonical_count": 23, "expected_count": 23, "reconciles": True},
