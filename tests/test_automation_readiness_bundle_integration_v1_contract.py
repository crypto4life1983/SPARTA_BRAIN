...
# ---- 2. next directive = C21 spec decision (C21 active, C20 last rejected) ----
...
def test_next_directive_is_c21_spec_decision():
    ...
    assert _R["active_candidate_verdict"] == "C21_DETECTOR_DRY_RUN_FROZEN_FOR_HUMAN_REVIEW"
    assert _R["active_candidate_stage"] == "detector_spec_dry_run"
    ...
    assert _R["last_rejected_candidate_verdict"] == "C20_REJECTED_AT_FEE_HONEST_REPLAY"
...
def test_surfaces_agree_on_c21_directive():
    ...
    s = _R["surfaces"]
    token = "HUMAN_DECISION_C21_ADVANCE_TO_REAL_CANDLE_LABELS_OR_REJECT"
...
def test_coordinator_idle_defers_to_lane_c21_open_gate():
    # the coordinator defers to the lane, which now has C21 as the ACTIVE open
    # candidate -> recommend its human spec decision, not automation readiness.
    d = gdc.coordinate({
        "repo": {"clean": True, "ahead": 0, "behind": 0,
                 "uncommitted_changes": False},
        "ledger": {"canonical_count": 25, "expected_count": 25, "reconciles": True},
