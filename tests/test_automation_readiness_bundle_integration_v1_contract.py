...
def test_next_directive_is_c17_spec_decision():
    ...
    assert _R["active_candidate_verdict"] == "C17_DETECTOR_DRY_RUN_FROZEN_FOR_HUMAN_REVIEW"
    assert _R["active_candidate_stage"] == "detector_spec_dry_run"
    assert _R["active_candidate_stage_label"] == "DETECTOR_DRY_RUN_FROZEN_FOR_HUMAN_REVIEW"
    ...
    assert _R["active_candidate_synthetic_fixtures_only"] is True
    assert _R["active_candidate_dry_run_all_checks_pass"] is True
    assert _R["active_candidate_dry_run_summary"]
...
def test_surfaces_agree_on_c17_directive():
    ...
    s = _R["surfaces"]
    token = "HUMAN_DECISION_C17_ADVANCE_TO_REAL_CANDLE_LABELS_OR_REJECT"
