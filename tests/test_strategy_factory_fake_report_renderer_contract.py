"""Bundle 37 tests for the Strategy Factory Fake Report Renderer Contract v1
(informational, read-only, fake-only, renderer-contract-only, deterministic,
execution-free, NOT IMPLEMENTED YET -- NO written report, NO report render, NO
report file write, NO real dry walk, NO orchestrator, NO real pipeline, NO
smoke-test run, NO runtime state write, NO research, NO data access).

Bundle 37's production module imports Bundles 11-36 via real package imports,
...
Coverage (per the Bundle 37 spec):
...
- all 14 posture keys present + all False + keys match Bundle 36,
- posture is mutation-isolated (fresh dicts),
- activation only from an active Bundle 36 review gate with operator_decision
  == READY_FOR_FAKE_REPORT_RENDERER_CONTRACT and next_gate ==
  FAKE_REPORT_RENDERER_CONTRACT_REQUIRED,
- blocked / wrong-gate / not-ready / inactive / malformed never activate,
- active next_gate == FAKE_REPORT_RENDERER_BUILD_REVIEW_REQUIRED,
- renderer_scope exact + inputs/outputs fake-only,
- prohibited real artifact references + prohibited runtime side effects,
- no execution/data/runtime/report-write flag True,
- markdown says fake-report-renderer-contract-only + not-implemented-yet +
  no-report-file-write + planning-only + placeholder-only +
  no-runtime-state-write + research-only + execution-free, writes nothing,
- prose verb audit, scoped dirty-tree guard, Bundle 11-36 regression imports,
...
import sparta_commander.strategy_factory_fake_report_renderer_contract as RR
...
_MODPATH = (
    ...
    / "sparta_commander"
    / "strategy_factory_fake_report_renderer_contract.py"
)
_SAFETY_PATH = (
    pathlib.Path(__file__).resolve().parents[1]
    / "sparta_commander"
...
_REQUIRED_BLOCKED = (
    ...
    "report_renderer_implementation_execution", "runtime_state_write",
)
...
def _contract(gate=None) -> dict:
    return build_fake_report_renderer_contract(
...
def test_state_and_gate_constants_pinned():
    assert RENDERER_STATE_ACTIVE == "FAKE_REPORT_RENDERER_CONTRACT_ACTIVE"
    assert RENDERER_STATE_BLOCKED == "FAKE_REPORT_RENDERER_CONTRACT_BLOCKED"
    assert (
        NEXT_GATE_FAKE_REPORT_RENDERER_BUILD_REVIEW_REQUIRED
...
# 4 -- renderer scope exactly as required.

def test_renderer_scope_exact():
    assert RENDERER_SCOPE == (
        "fake_only",
        "in_memory_only",
        "deterministic",
        "no_real_data",
...
def test_posture_keys_present_and_all_false():
    posture = RENDERER_CONTRACT_SAFETY_POSTURE
    assert set(posture.keys()) == _EXPECTED_POSTURE_KEYS
    assert len(posture) == 14
    assert all(v is False for v in posture.values())


# 9 -- posture keys match Bundle 36.

def test_posture_keys_match_bundle36():
    assert (
        set(RENDERER_CONTRACT_SAFETY_POSTURE.keys())
...
def test_posture_mutation_isolated():
    ...
    assert RENDERER_CONTRACT_SAFETY_POSTURE["automation_enabled"] is False


# 11 -- an active READY Bundle 36 gate activates the renderer contract.

def test_active_ready_gate_activates_contract():
    c = _contract()
    assert c["fake_report_renderer_contract_active"] is True
    assert (
        c["fake_report_renderer_contract_state"]
        == "FAKE_REPORT_RENDERER_CONTRACT_ACTIVE"
    )
    assert c["fake_walk_report_operator_review_gate_active"] is True
    assert (
        c["fake_walk_report_operator_review_gate_decision"]
        == "READY_FOR_FAKE_REPORT_RENDERER_CONTRACT"
    )
    assert (
        c["fake_walk_report_operator_review_gate_next_gate"]
        == "FAKE_REPORT_RENDERER_CONTRACT_REQUIRED"
    )
    assert c["next_gate"] == "FAKE_REPORT_RENDERER_BUILD_REVIEW_REQUIRED"
    assert c["schema_version"] == RENDERER_CONTRACT_SCHEMA_VERSION
    assert (
        c["fake_walk_report_operator_review_gate_schema_version"]
        == REPORT_REVIEW_GATE_SCHEMA_VERSION
    )


# 12 -- a blocked (inactive) upstream gate does not activate.
...
def test_blocked_gate_does_not_activate():
    ...
    assert (
        c["fake_report_renderer_contract_state"]
        == "FAKE_REPORT_RENDERER_CONTRACT_BLOCKED"
    )
    assert c["next_gate"] == "AWAIT_FAKE_WALK_REPORT_OPERATOR_REVIEW_GATE"


# 13 -- active gate but non-READY decision does not activate.
...
def test_non_ready_decisions_never_activate():
    for decision in (
        ...
        gate = build_fake_walk_report_operator_review_gate(
            _active_report(), operator_decision=decision)
        c = _contract(gate)
        assert c["fake_report_renderer_contract_active"] is False, decision
        assert (
            c["next_gate"] == "AWAIT_FAKE_WALK_REPORT_OPERATOR_REVIEW_GATE"
        ), decision


# 14 -- active READY gate but wrong next_gate does not activate.

def test_active_gate_wrong_next_gate_does_not_activate():
    import copy
...
# 15 -- inactive gate flag does not activate even with READY + right gate.

def test_inactive_gate_flag_does_not_activate():
    import copy
    g = copy.deepcopy(_ready_gate())
    g["fake_walk_report_operator_review_gate_active"] = False
    c = _contract(g)
    assert c["fake_report_renderer_contract_active"] is False
    assert c["next_gate"] == "AWAIT_FAKE_WALK_REPORT_OPERATOR_REVIEW_GATE"
...
def test_malformed_input_no_raise():
    for bad in (None, 42, "nope", {},
                {"fake_walk_report_operator_review_gate_active": True}, []):
        c = build_fake_report_renderer_contract(bad)
        assert c["fake_report_renderer_contract_active"] is False
...
# 17 -- renderer scope / inputs / outputs present on the contract.

def test_scope_inputs_outputs_present():
    c = _contract()
    assert tuple(c["renderer_scope"]) == RENDERER_SCOPE
    assert tuple(c["allowed_in_memory_inputs"]) == ALLOWED_IN_MEMORY_INPUTS
    assert tuple(c["allowed_in_memory_outputs"]) == ALLOWED_IN_MEMORY_OUTPUTS


# 18 -- all allowed inputs/outputs are fake/sample/placeholder-only.

def test_inputs_outputs_are_fake_sample_placeholder_only():
    for coll in (ALLOWED_IN_MEMORY_INPUTS, ALLOWED_IN_MEMORY_OUTPUTS):
        assert len(coll) >= 1
        for item in coll:
            assert (
                "fake" in item or "sample" in item or "placeholder" in item
            ), item


# 19 -- renderer / markdown / section / attestation contracts present.

def test_render_contracts_present():
    c = _contract()
    assert tuple(c["renderer_function_contract"]) == RENDERER_FUNCTION_CONTRACT
    assert tuple(c["markdown_render_contract"]) == MARKDOWN_RENDER_CONTRACT
    assert (
        tuple(c["report_section_render_contract"])
        == REPORT_SECTION_RENDER_CONTRACT
    )
    assert (
        tuple(c["safety_attestation_render_contract"])
        == SAFETY_ATTESTATION_RENDER_CONTRACT
    )
    for coll in (RENDERER_FUNCTION_CONTRACT, MARKDOWN_RENDER_CONTRACT,
                 REPORT_SECTION_RENDER_CONTRACT,
                 SAFETY_ATTESTATION_RENDER_CONTRACT):
        assert len(coll) >= 1
        for item in coll:
...
# 20 -- prohibited real artifact references include the required real names.

def test_prohibited_real_artifact_references():
    c = _contract()
    prohibited = set(c["prohibited_real_artifact_references"])
    for name in ("Crypto-D1", "qa_report.json", "manifest.json",
                 "CHECKSUMS.txt", "FREEZE_RECORD.txt", "fees.json",
                 ".csv", ".parquet"):
        assert name in prohibited, name
    # dataset names, baseline outputs, and a real market-data path are named.
    assert "real_dataset_name" in prohibited
    assert "baseline_output" in prohibited
    assert "real_market_data_path" in prohibited


# 21 -- prohibited runtime side effects include the required surfaces.

def test_prohibited_runtime_side_effects():
    c = _contract()
    prohibited = set(c["prohibited_runtime_side_effects"])
    for effect in ("file_write", "report_file_write", "runtime_state_write",
                   "dashboard_update", "registry_write", "ledger_write",
                   "network_call", "subprocess_spawn",
                   "broker_exchange_order_action"):
        assert effect in prohibited, effect


# 22 -- blocked capabilities include the required set + renderer-specific ones.
...
def test_blocked_capabilities():
    ...
    assert "report_renderer_build_execution" in blocked
    assert "report_renderer_implementation_execution" in blocked
    assert "runtime_state_write" in blocked


# 23 -- no authorization flag can become True (any state).

def test_authorization_flags_always_false():
    states = [
        _contract(),
        _contract(_ready_gate()),
        build_fake_report_renderer_contract({}),
        _contract(build_fake_walk_report_operator_review_gate(
            _active_report(),
            operator_decision=OPERATOR_DECISION_PARK)),
    ]
    for c in states:
        for flag in _AUTH_FLAGS:
            assert c[flag] is False
        ...
        assert c["stage"] == "FAKE_RENDERER_CONTRACT_ONLY"


# 24 -- even an active contract authorizes no data/runtime/report-write surface.
...
def test_active_contract_authorizes_no_runtime_or_report_write():
    ...
    assert "report_render_execution" in c["blocked_capabilities"]
...
# 25 -- the embedded upstream gate contract is preserved.

def test_upstream_gate_embedded():
    g = _ready_gate()
    c = build_fake_report_renderer_contract(g)
    assert (
        c["fake_walk_report_operator_review_gate"][
            "fake_walk_report_operator_review_gate_active"]
        is True
    )
    assert (
        c["fake_walk_report_operator_review_gate"]["read_only"] is True
    )


# 26 -- validate passes for a clean contract + each failure mode.

def test_validate_passes_and_detects_failures():
    import copy
    c = _contract()
    v = validate_fake_report_renderer_contract(c)
    assert v["valid"] is True
    assert v["missing_required_fields"] == ()

    bad = copy.deepcopy(c)
    bad["execution_authorized"] = True
    assert validate_fake_report_renderer_contract(bad)["valid"] is False

    bad2 = copy.deepcopy(c)
    bad2["safety_posture"]["network_enabled"] = True
    assert validate_fake_report_renderer_contract(bad2)["valid"] is False

    bad3 = copy.deepcopy(c)
    bad3["mode"] = "LIVE"
    assert validate_fake_report_renderer_contract(bad3)["valid"] is False

    bad4 = copy.deepcopy(c)
    bad4["stage"] = "EXECUTE"
    assert validate_fake_report_renderer_contract(bad4)["valid"] is False

    bad5 = copy.deepcopy(c)
    bad5["human_approval_required"] = False
    assert validate_fake_report_renderer_contract(bad5)["valid"] is False


# 27 -- validate flags wrong scope / input / output / render tuples.

def test_validate_flags_wrong_tuples():
    import copy
    c = _contract()
    for key in ("renderer_scope", "allowed_in_memory_inputs",
                "allowed_in_memory_outputs", "renderer_function_contract",
                "markdown_render_contract", "report_section_render_contract",
                "safety_attestation_render_contract",
                "prohibited_real_artifact_references",
                "prohibited_runtime_side_effects"):
        bad = copy.deepcopy(c)
        ...
        assert (
            validate_fake_report_renderer_contract(bad)["valid"] is False
        ), key


# 28 -- validate never requires the contract's own validation field.
...
# 29 -- a blocked contract still validates (renderer safety, not activation).
...
# 30 -- validate flags missing required top-level fields.

def test_validate_flags_missing_required_fields():
    import copy
    c = copy.deepcopy(_contract())
    c.pop("fake_walk_report_operator_review_gate", None)
    v = validate_fake_report_renderer_contract(c)
    assert v["valid"] is False
    assert (
        "fake_walk_report_operator_review_gate" in v["missing_required_fields"]
    )


# 31 -- markdown non-empty, says the required descriptors + sections.

def test_markdown_renderer_contract_only_and_execution_free():
    c = _contract()
    md = render_fake_report_renderer_contract_markdown(c)
    assert isinstance(md, str) and md
    assert "# Strategy Factory Fake Report Renderer Contract" in md
    assert "fake-report-renderer-contract-only" in md
    ...
    assert "Stage: FAKE_RENDERER_CONTRACT_ONLY" in md
    assert "Mode: RESEARCH_ONLY" in md
    assert "Read only: True" in md
    assert "Executes: False" in md
    assert "Fake report renderer contract active: True" in md
    assert "Next gate: FAKE_REPORT_RENDERER_BUILD_REVIEW_REQUIRED" in md
    assert "## Source Reference" in md
    assert "## Renderer Scope" in md
    assert "## Allowed In Memory Inputs" in md
    assert "## Allowed In Memory Outputs" in md
    assert "## Renderer Function Contract" in md
    assert "## Markdown Render Contract" in md
    assert "## Report Section Render Contract" in md
    assert "## Safety Attestation Render Contract" in md
...
# the bare token "Crypto-D1" in the prohibited list, so we only assert the real
...
# 34 -- prose verb audit over notes / source ref / guard / markdown prose.
...
def test_prose_has_no_execution_or_trading_verb():
    ...
    texts.append(
        str(c["source_fake_report_review_gate_reference_placeholder"]))
    texts.append(str(c["placeholder_only_guard"]))
...
def test_commander_2_safety_allowlist_includes_new_paths():
    ...
    assert (
        '"tests/test_strategy_factory_fake_report_renderer_contract.py"' in src
    )


# 37 -- Bundle 36 regression import still works.

def test_bundle36_regression_import_still_works():
    g = build_fake_walk_report_operator_review_gate({})
