"""Bundle 27 tests for the Strategy Factory End-to-End Fake Pipeline Contract
template v1 (informational, read-only, template-only, FAKE artifacts only --
NO pipeline run, NO runtime state write, NO orchestration, NO research, NO
data access).

Bundle 27's production module imports Bundles 11-26 via real package imports,
...
Coverage (per the Bundle 27 spec):
...
- all 14 posture keys present + all False + keys match Bundle 26,
- posture is mutation-isolated (fresh dicts),
- activates only when safety_kill_switch_contract_active True AND gate
  next_gate is END_TO_END_FAKE_PIPELINE_CONTRACT_REQUIRED,
- blocked / wrong-gate / malformed gate never activate, never raise,
- next_gate defaults to BACKBONE_CLOSURE_REPORT_REQUIRED when active,
- no execution/data/backtest/broker/upload/autopilot/live/dashboard-runtime/
  ledger-write/runtime-state flag True,
- the required template description fields present,
...
- markdown says end-to-end-fake-pipeline-contract-only + fake-artifacts-only +
  no-runtime-state-write + research-only + execution-free, writes nothing,
- prose verb audit over notes / failures / source / markdown prose,
- scoped dirty-tree guard, Bundle 11-26 regression imports,
...
import sparta_commander.strategy_factory_end_to_end_fake_pipeline_contract as EP
...
_MODPATH = (
    ...
    / "sparta_commander"
    / "strategy_factory_end_to_end_fake_pipeline_contract.py"
)
_SAFETY_PATH = (
    pathlib.Path(__file__).resolve().parents[1]
    / "sparta_commander"
...
_REQUIRED_TEMPLATE_FIELDS = (
    ...
    "fake_pipeline_stage_sequence", "fake_artifact_inputs_required",
    "fake_artifact_outputs_expected", "expected_trace_fields",
    "expected_operator_review_fields", "expected_failure_modes",
...
def _ready_gate() -> dict:
    """A Bundle 26 safety kill-switch contract that is active."""
...
def _blocked_gate() -> dict:
    """A Bundle 26 safety kill-switch contract that is NOT active."""
    draft = build_research_protocol_draft_contract(
        _good_item(),
        memo_decision=MEMO_DECISION_READY_FOR_PROTOCOL_DRAFT,
        human_research_approved=True,
...
def _active_contract() -> dict:
    return build_end_to_end_fake_pipeline_contract(_ready_gate())
...
def test_public_api_is_limited_to_expected_names():
    assert set(EP.__all__) == _expected_public()
...
def test_posture_keys_present_and_all_false():
    posture = END_TO_END_FAKE_PIPELINE_CONTRACT_SAFETY_POSTURE
    assert set(posture.keys()) == _EXPECTED_POSTURE_KEYS
    assert len(posture) == 14
    assert all(v is False for v in posture.values())


# 6 -- posture keys match Bundle 26.

def test_posture_keys_match_bundle26():
    assert (
...
def test_posture_mutation_isolated():
    a = build_end_to_end_fake_pipeline_contract(_ready_gate())
    a["safety_posture"]["automation_enabled"] = True
    a["read_only"] = False
    fresh = build_end_to_end_fake_pipeline_contract(_ready_gate())
    assert fresh["safety_posture"]["automation_enabled"] is False
    assert fresh["read_only"] is True
    assert (
        END_TO_END_FAKE_PIPELINE_CONTRACT_SAFETY_POSTURE["automation_enabled"]
        is False
    )


# 10 -- gate active + END_TO_END_FAKE_PIPELINE gate activates the contract.

def test_active_gate_activates():
    c = _active_contract()
    assert c["end_to_end_fake_pipeline_contract_active"] is True
    assert (
        c["end_to_end_fake_pipeline_state"]
        == "END_TO_END_FAKE_PIPELINE_CONTRACT_ACTIVE"
    )
    assert c["next_gate"] == "BACKBONE_CLOSURE_REPORT_REQUIRED"
    assert c["safety_kill_switch_contract_active"] is True
    assert (
        c["safety_kill_switch_next_gate"]
        == "END_TO_END_FAKE_PIPELINE_CONTRACT_REQUIRED"
    )


# 11 -- a blocked (inactive) gate contract does not activate.

def test_blocked_gate_does_not_activate():
    c = build_end_to_end_fake_pipeline_contract(_blocked_gate())
    assert c["end_to_end_fake_pipeline_contract_active"] is False
    assert (
        c["end_to_end_fake_pipeline_state"]
        == "END_TO_END_FAKE_PIPELINE_CONTRACT_BLOCKED"
    )
    assert c["next_gate"] == "AWAIT_SAFETY_KILL_SWITCH_CONTRACT"


# 12 -- gate active but wrong next_gate does not activate.

def test_active_gate_wrong_gate_does_not_activate():
    import copy
    g = copy.deepcopy(_ready_gate())
    g["next_gate"] = "SOMETHING_ELSE"  # tamper: no longer the pipeline gate
    c = build_end_to_end_fake_pipeline_contract(g)
    assert c["end_to_end_fake_pipeline_contract_active"] is False
    assert c["next_gate"] == "AWAIT_SAFETY_KILL_SWITCH_CONTRACT"


# 13 -- malformed gate never raises and never activates.

def test_malformed_gate_no_raise():
    for bad in (None, 42, "nope", {},
                {"safety_kill_switch_contract_active": True}, []):
        c = build_end_to_end_fake_pipeline_contract(bad)
        assert c["end_to_end_fake_pipeline_contract_active"] is False
...
def test_authorization_flags_always_false():
    for c in (_active_contract(),
              build_end_to_end_fake_pipeline_contract(_blocked_gate())):
        for flag in _AUTH_FLAGS:
            assert c[flag] is False
...
def test_required_template_fields_present():
    ...
    assert isinstance(c["fake_pipeline_stage_sequence"], tuple)
    assert len(c["fake_pipeline_stage_sequence"]) >= 1
    assert isinstance(c["fake_artifact_inputs_required"], tuple)
    assert len(c["fake_artifact_inputs_required"]) >= 1
    assert isinstance(c["fake_artifact_outputs_expected"], tuple)
    assert len(c["fake_artifact_outputs_expected"]) >= 1
    assert isinstance(c["expected_trace_fields"], tuple)
    assert len(c["expected_trace_fields"]) >= 1
    assert isinstance(c["expected_operator_review_fields"], tuple)
    assert len(c["expected_operator_review_fields"]) >= 1
    assert isinstance(c["expected_failure_modes"], tuple)
    assert len(c["expected_failure_modes"]) >= 1
    assert c["asset_lane"] == "MNQ"
    assert c["timeframe_lane"] == "5m"


# 17 -- every named artifact is a fake placeholder (no real artifact name).

def test_artifacts_are_fake_placeholders_only():
    c = _active_contract()
    for x in c["fake_artifact_inputs_required"]:
        assert "fake" in x and "placeholder" in x
    for x in c["fake_artifact_outputs_expected"]:
        assert "fake" in x and "placeholder" in x
...
# 19 -- the embedded safety gate contract is preserved.

def test_safety_gate_embedded():
    g = _ready_gate()
    c = build_end_to_end_fake_pipeline_contract(g)
    assert c["safety_kill_switch_contract"]["schema_version"] == (
        g["schema_version"]
    )
    assert c["safety_kill_switch_contract"]["read_only"] is True
...
def test_validate_passes_and_detects_failures():
    import copy
    c = _active_contract()
    v = validate_end_to_end_fake_pipeline_contract(c)
    assert v["valid"] is True
    assert v["missing_required_fields"] == ()

    bad = copy.deepcopy(c)
    bad["execution_authorized"] = True
    assert validate_end_to_end_fake_pipeline_contract(bad)["valid"] is False

    bad2 = copy.deepcopy(c)
    bad2["safety_posture"]["network_enabled"] = True
    assert validate_end_to_end_fake_pipeline_contract(bad2)["valid"] is False

    bad3 = copy.deepcopy(c)
    bad3["mode"] = "LIVE"
    assert validate_end_to_end_fake_pipeline_contract(bad3)["valid"] is False

    bad4 = copy.deepcopy(c)
    bad4["fake_pipeline_stage_sequence"] = ()
    assert validate_end_to_end_fake_pipeline_contract(bad4)["valid"] is False

    bad5 = copy.deepcopy(c)
    bad5["fake_artifact_inputs_required"] = ()
    assert validate_end_to_end_fake_pipeline_contract(bad5)["valid"] is False

    bad6 = copy.deepcopy(c)
    bad6["fake_artifact_outputs_expected"] = ()
    assert validate_end_to_end_fake_pipeline_contract(bad6)["valid"] is False
...
def test_results_are_fresh_and_isolated():
    ...
    assert a == b
    assert a is not b
    a["fake_pipeline_stage_sequence"] = ()
    a["safety_kill_switch_contract"]["read_only"] = False
    fresh = _active_contract()
    assert len(fresh["fake_pipeline_stage_sequence"]) >= 1
    assert fresh["safety_kill_switch_contract"]["read_only"] is True


# 23 -- markdown non-empty, says the five required descriptors, sections.

def test_markdown_template_only_and_execution_free():
    c = _active_contract()
    md = render_end_to_end_fake_pipeline_contract_markdown(c)
    assert isinstance(md, str) and md
    assert "# Strategy Factory End-to-End Fake Pipeline Contract" in md
    assert "Template only" in md
    assert "end-to-end-fake-pipeline-contract-only" in md
    assert "fake-artifacts-only" in md
    ...
    assert "End to end fake pipeline contract active: True" in md
    assert "Next gate: BACKBONE_CLOSURE_REPORT_REQUIRED" in md
    assert "## Source References" in md
    assert "## Fake Pipeline Stage Sequence" in md
    assert "## Fake Artifact Inputs Required" in md
    assert "## Fake Artifact Outputs Expected" in md
...
# 25 -- prose verb audit over notes / failures / source / markdown prose.
...
def test_prose_has_no_execution_or_trading_verb():
    ...
    for ln in md.splitlines():
        ...
        # only: ... end-to-end-fake-pipeline-contract-only ...") is a metadata
...
# 29 -- Bundle 25 regression import still works.
