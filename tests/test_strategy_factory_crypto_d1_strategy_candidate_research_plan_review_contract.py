"""Tests for the Crypto-D1 Strategy Candidate Research Plan Review Contract
(Block 105).

Pure stdlib, read-only, side-effect-free. These tests assert the contract
reviews the Block 103 external-bot-evidence research plan, stays gated behind a
clean READY research-plan signal pointed at this review gate, upholds every
external-bot-evidence safety stance, never authorizes anything real, and never
...
import sparta_commander.strategy_factory_crypto_d1_strategy_candidate_research_plan_review_contract as m  # noqa: E501
...
def _active_signal() -> dict:
    """A clean, READY Block 103 research-plan signal pointed at this gate."""
    return {
        "crypto_d1_bot_evidence_research_plan_contract_active": True,
        "bot_evidence_research_plan_verdict": (
            m.UPSTREAM_REQUIRED_RESEARCH_PLAN_VERDICT
        ),
        "next_gate": m.UPSTREAM_REQUIRED_RESEARCH_PLAN_NEXT_GATE,
        "mode": m.UPSTREAM_REQUIRED_RESEARCH_PLAN_MODE,
        "read_only": True,
        "executes": False,
    }
...
def _valid_packet() -> dict:
    ...
    p: dict = {}
    for f in m.RESEARCH_PLAN_REVIEW_REQUIRED_TEXT_FIELDS:
        p[f] = f + "_value"
    ...
    for a in m.RESEARCH_PLAN_REVIEW_REQUIRED_AFFIRMATIONS:
        p[a] = True
    return p
...
def test_schema_version_preserved():
    assert m.RESEARCH_PLAN_REVIEW_SCHEMA_VERSION == (
        "strategy_factory_crypto_d1_strategy_candidate_research_plan_review_"
        "contract.v1"
    )


def test_label_is_research_plan_review():
    assert "Research Plan Review" in m.DEFAULT_RESEARCH_PLAN_REVIEW_LABEL


def test_status_is_read_only_research_plan_review():
    assert m.RESEARCH_PLAN_REVIEW_STATUS == (
        "READ_ONLY_CRYPTO_D1_STRATEGY_CANDIDATE_RESEARCH_PLAN_REVIEW_CONTRACT"
    )


def test_market_and_timeframe_scope_from_block_103():
...
def test_upstream_gating_constants():
    assert m.UPSTREAM_REQUIRED_RESEARCH_PLAN_NEXT_GATE == (
        "CRYPTO_D1_STRATEGY_CANDIDATE_RESEARCH_PLAN_REVIEW_CONTRACT_REQUIRED"
    )
    assert m.UPSTREAM_REQUIRED_RESEARCH_PLAN_MODE == "RESEARCH_ONLY"
    assert m.UPSTREAM_REQUIRED_RESEARCH_PLAN_VERDICT == (
        "BOT_EVIDENCE_RESEARCH_PLAN_READY"
    )


def test_verdict_constants_distinct():
    assert len(set(m.ALLOWED_RESEARCH_PLAN_REVIEW_VERDICTS)) == 5


# --------------------------------------------------------------------------- #
# Upstream gating (only a clean Block 103 READY-for-this-gate signal activates)
# --------------------------------------------------------------------------- #

def test_inactive_signal_awaits():
    c = m.build_crypto_d1_strategy_candidate_research_plan_review_contract(
        {}, None
    )
    assert c["research_plan_review_verdict"] == (
        m.RESEARCH_PLAN_REVIEW_VERDICT_AWAIT
    )
    assert c[
...
def test_active_signal_activates_and_is_ready():
    ...
    assert c["research_plan_review_verdict"] == (
        m.RESEARCH_PLAN_REVIEW_VERDICT_READY
    )


def test_dirty_signal_missing_active_flag_awaits():
    sig = _active_signal()
    del sig["crypto_d1_bot_evidence_research_plan_contract_active"]
    c = m.build_crypto_d1_strategy_candidate_research_plan_review_contract(
        sig, _valid_packet()
    )
    assert c["research_plan_review_verdict"] == (
...
def test_wrong_verdict_awaits():
    sig = _active_signal()
    sig["bot_evidence_research_plan_verdict"] = "SOMETHING_ELSE"
    c = m.build_crypto_d1_strategy_candidate_research_plan_review_contract(
        sig, _valid_packet()
    )
    assert c["research_plan_review_verdict"] == (
...
def test_valid_packet_is_ready():
    r = m.evaluate_crypto_d1_strategy_candidate_research_plan_review(
        _valid_packet()
    )
    assert r["verdict"] == m.RESEARCH_PLAN_REVIEW_VERDICT_READY


def test_empty_packet_needs_more_info():
    r = m.evaluate_crypto_d1_strategy_candidate_research_plan_review({})
    assert r["verdict"] == m.RESEARCH_PLAN_REVIEW_VERDICT_NEEDS_MORE_INFO


def test_missing_text_field_needs_more_info():
    p = _valid_packet()
    del p["review_decision_rationale"]
    r = m.evaluate_crypto_d1_strategy_candidate_research_plan_review(p)
    assert r["verdict"] == m.RESEARCH_PLAN_REVIEW_VERDICT_NEEDS_MORE_INFO
...
def test_go_reviewed_bot_state_rejected():
    ...
    r = m.evaluate_crypto_d1_strategy_candidate_research_plan_review(p)
    assert r["verdict"] == m.RESEARCH_PLAN_REVIEW_VERDICT_REJECTED
...
def test_weaken_research_plan_rejected():
    p = _valid_packet()
    p["weaken_research_plan"] = True
    r = m.evaluate_crypto_d1_strategy_candidate_research_plan_review(p)
    assert r["verdict"] == m.RESEARCH_PLAN_REVIEW_VERDICT_REJECTED
...
def test_reviewer_parks_lane():
    ...
    r = m.evaluate_crypto_d1_strategy_candidate_research_plan_review(p)
    assert r["verdict"] == m.RESEARCH_PLAN_REVIEW_VERDICT_PARKED
...
def test_build_returns_fresh_dicts():
    c1 = m.build_crypto_d1_strategy_candidate_research_plan_review_contract(
        _active_signal(), _valid_packet()
    )
    c2 = m.build_crypto_d1_strategy_candidate_research_plan_review_contract(
        _active_signal(), _valid_packet()
    )
    assert c1 is not c2
...
def test_validate_ready_contract():
    ...
    v = m.validate_crypto_d1_strategy_candidate_research_plan_review_contract(c)
...
def test_render_markdown_non_empty():
    ...
    md = (
        m
        .render_crypto_d1_strategy_candidate_research_plan_review_contract_markdown(  # noqa: E501
            c
        )
    )
    assert isinstance(md, str)
    assert "Research Plan Review" in md
    assert "SOL" in md
    assert "XRP" in md
...
def test_evaluate_never_raises_on_garbage():
    for junk in (None, 1, "x", [], (), 3.14, True):
        r = m.evaluate_crypto_d1_strategy_candidate_research_plan_review(junk)
        assert r["verdict"] in m.ALLOWED_RESEARCH_PLAN_REVIEW_VERDICTS
