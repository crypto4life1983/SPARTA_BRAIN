...
# ---- C21 is the ACTIVE open candidate at the DETECTOR-SPEC DRY-RUN review stage --
...
def test_c21_active_candidate_at_detector_dry_run_review():
    ...
    assert _R["next_stage"] == "c21_real_candle_labels_decision"
    ...
    assert det["verdict"] == "C21_DETECTOR_DRY_RUN_FROZEN_FOR_HUMAN_REVIEW"
    ...
    assert det["detector_synthetic_dry_run_frozen_for_human_review"] is True
    assert det["real_candle_detection_labels_replay_locked"] is True
    assert len(det["proposal_commit"]) == 40
    assert len(det["spec_commit"]) == 40
    assert det["next_action"] == "HUMAN_DECISION_C21_ADVANCE_TO_REAL_CANDLE_LABELS_OR_REJECT"
    ...
    assert c21["stage"] == "detector_spec_dry_run"
    # tamper: cannot unlock detector/labels/replay while at the spec review stage
    bad_det = {**det, "real_candle_detection_labels_replay_locked": False}
...
def test_c21_detector_dry_run_invariants_no_permissions_opened():
    ...
    assert _R["active_candidate_detail"]["real_candle_detection_labels_replay_locked"] is True
...
def test_summarize_for_morning_report():
    ...
    assert summ["active_candidate_verdict"] == "C21_DETECTOR_DRY_RUN_FROZEN_FOR_HUMAN_REVIEW"
    ...
    assert summ["next_stage"] == "c21_real_candle_labels_decision"
