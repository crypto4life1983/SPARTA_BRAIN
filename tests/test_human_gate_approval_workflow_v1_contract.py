...
GATE = "HUMAN_DECISION_C19_ADVANCE_TO_CANDIDATE_SPEC_OR_REJECT"
...
def test_mirrors_lane_c19_open_gate():
    ls = lane.get_lane_status()
    # C19 is the ACTIVE open candidate -> the lane has an open human gate
    assert _R["active_candidate"] == "C19" == ls["active_candidate"]
    ...
    # tamper: dropping the approval text while a gate is open must fail
    bad = {**_R, "has_open_human_gate": False}
    assert hgw.validate_human_gate_workflow(bad)["valid"] is False


# ---- 4 recommended safe next decision (C19 open gate) ----------------------

def test_recommended_decision_advance_c19_to_spec():
    assert _R["recommended_decision"] == "ADVANCE C19 TO CANDIDATE SPEC"
    assert _R["stage_after_approval"] == "candidate_spec"


# ---- 5 copyable approval text when the C19 gate is open --------------------
...
def test_c18_rejected_and_downstream_locked():
...
def test_summarize_for_panel():
    ...
    assert s["active_candidate"] == "C19"
    assert s["has_open_human_gate"] is True
    assert s["current_human_gate"] == GATE
    assert s["recommended_decision"] == "ADVANCE C19 TO CANDIDATE SPEC"
