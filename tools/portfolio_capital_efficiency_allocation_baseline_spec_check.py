"""SPARTA Portfolio Capital-Efficiency Allocation Baseline Spec — validator (research-only, stdlib).

Validates `reports/portfolio_capital_efficiency_allocation_baseline_spec_v1/allocation_baseline_spec.json`
against the required schema and the hard safety contract:
  * research_only + advisory_only + framework_only are True; compute_enabled +
    capital_deployment_enabled + eight execution flags are False
  * lane == 'portfolio_capital_efficiency'; phase == 'P3_allocation_baseline_spec'
  * lane_status_self_declared == 'WATCH'; verdict_ceiling max == 'WATCH'
  * companion_documents reference P0 protocol + P1 input contract + P2 metric spec
  * the 6 required allocation baselines are present, each with method + inputs +
    constraints + non-empty validation_rules + computed_in_this_bundle == False
  * ranking_only_marginal_efficiency carries ranking_only == True
  * zero_for_inadmissible + cash_unallocated_bucket are hard_constraint status
  * allocation_constraints includes the core invariants (sum-to-one incl cash,
    long-only, inadmissible->0, concentration cap, budget B, no real allocation)
  * Candidate #10 deferred: c10_boundary.must_not_touch_c10 True and
    connection_status == 'deferred_not_connected'
  * pass_watch_fail_rules + required_future_artifacts + safety_boundaries +
    no_profit_claim_policy non-empty
  * no profitability / live-readiness / allocate-now / connect-c10 claim

Standard library only. No network. No broker/exchange imports. No subprocess.
No credential / env reads.

CLI:
  python tools/portfolio_capital_efficiency_allocation_baseline_spec_check.py validate
  python tools/portfolio_capital_efficiency_allocation_baseline_spec_check.py show
"""
from __future__ import annotations

import argparse
import json
from pathlib import Path

REPO_ROOT = Path(__file__).resolve().parent.parent
SPEC_DIR_REL = "reports/portfolio_capital_efficiency_allocation_baseline_spec_v1"
SPEC_JSON = "allocation_baseline_spec.json"
SPEC_MD = "allocation_baseline_spec.md"

REQUIRED_TOP_LEVEL_KEYS = (
    "allocation_baseline_spec_id",
    "title",
    "version",
    "phase",
    "research_only",
    "advisory_only",
    "framework_only",
    "compute_enabled",
    "capital_deployment_enabled",
    "data_fetch_enabled",
    "exchange_connection_enabled",
    "live_trading_enabled",
    "broker_control_enabled",
    "paper_order_execution_enabled",
    "live_capital_allocation_enabled",
    "position_sizing_execution_enabled",
    "backtest_enabled",
    "lane",
    "lane_status_self_declared",
    "companion_documents",
    "verdict_ceiling_rules",
    "c10_boundary",
    "allocation_baselines",
    "allocation_constraints",
    "pass_watch_fail_rules",
    "required_future_artifacts",
    "safety_boundaries",
    "no_profit_claim_policy",
)

MUST_BE_FALSE_FLAGS = (
    "compute_enabled",
    "capital_deployment_enabled",
    "data_fetch_enabled",
    "exchange_connection_enabled",
    "live_trading_enabled",
    "broker_control_enabled",
    "paper_order_execution_enabled",
    "live_capital_allocation_enabled",
    "position_sizing_execution_enabled",
    "backtest_enabled",
)

REQUIRED_BASELINE_IDS = (
    "equal_weight",
    "equal_risk",
    "capped_concentration",
    "zero_for_inadmissible",
    "ranking_only_marginal_efficiency",
    "cash_unallocated_bucket",
)

HARD_CONSTRAINT_BASELINES = (
    "zero_for_inadmissible",
    "cash_unallocated_bucket",
)

REQUIRED_ALLOCATION_CONSTRAINT_KEYS = (
    "weights_sum_to_one_incl_cash",
    "long_only_research_weights",
    "inadmissible_weight_zero",
    "concentration_cap_L_enforced",
    "capital_at_risk_budget_B_respected",
    "frozen_admissible_inputs_only",
    "no_real_allocation_in_this_bundle",
)

NON_EMPTY_LIST_SECTIONS = (
    "required_future_artifacts",
    "safety_boundaries",
    "no_profit_claim_policy",
)

DISTINCTION_PHRASES = (
    "Advisory-only",
    "No real allocation is computed in this bundle.",
    "A capital-efficiency score does not imply future returns.",
    "An advisory allocation is not an execution instruction.",
    "Candidate #10 remains deferred and is not connected by this spec.",
)

FORBIDDEN_PHRASES = (
    "guaranteed profit",
    "risk-free profit",
    "guaranteed alpha",
    "live-ready",
    "production-ready",
    "live trading enabled",
    "this is profitable",
    "we have an edge",
    "place the order",
    "place an order",
    "connect to exchange",
    "fetch live data",
    "allocate capital now",
    "increase position size",
    "execute this allocation",
    "deploy capital now",
    "connect c10 now",
)


def _load(repo_root: Path):
    p = repo_root / SPEC_DIR_REL / SPEC_JSON
    if not p.exists():
        return None, f"missing: {p.as_posix()}"
    try:
        return json.loads(p.read_text(encoding="utf-8")), None
    except Exception as exc:  # noqa: BLE001
        return None, f"invalid JSON ({type(exc).__name__}): {p.as_posix()}"


def validate(repo_root: Path = REPO_ROOT):
    errors: list = []
    data, err = _load(repo_root)
    if err:
        return False, [err]
    if not isinstance(data, dict):
        return False, ["allocation_baseline_spec.json is not a JSON object"]

    for k in REQUIRED_TOP_LEVEL_KEYS:
        if k not in data:
            errors.append(f"missing key: {k}")

    for flag in MUST_BE_FALSE_FLAGS:
        if data.get(flag) is not False:
            errors.append(f"safety flag {flag} must be False (got {data.get(flag)!r})")
    for true_flag in ("research_only", "advisory_only", "framework_only"):
        if data.get(true_flag) is not True:
            errors.append(f"{true_flag} must be True")

    if data.get("lane") != "portfolio_capital_efficiency":
        errors.append(f"lane must be 'portfolio_capital_efficiency' (got {data.get('lane')!r})")
    if data.get("phase") != "P3_allocation_baseline_spec":
        errors.append(f"phase must be 'P3_allocation_baseline_spec' (got {data.get('phase')!r})")
    if data.get("lane_status_self_declared") != "WATCH":
        errors.append(
            f"lane_status_self_declared must be 'WATCH' (got {data.get('lane_status_self_declared')!r})"
        )

    # Companion documents reference P0 + P1 + P2.
    cd = data.get("companion_documents")
    if not isinstance(cd, dict):
        errors.append("companion_documents must be a dict")
    else:
        joined = " ".join(str(v) for v in cd.values())
        for needle in (
            "portfolio_capital_efficiency_protocol_v1",
            "portfolio_capital_efficiency_input_contract_v1",
            "portfolio_capital_efficiency_metric_spec_v1",
        ):
            if needle not in joined:
                errors.append(f"companion_documents must reference {needle}")

    # Verdict ceiling.
    vcr = data.get("verdict_ceiling_rules")
    if not isinstance(vcr, dict):
        errors.append("verdict_ceiling_rules must be a dict")
    else:
        if vcr.get("max_surfaced_verdict") != "WATCH":
            errors.append(f"verdict_ceiling_rules.max_surfaced_verdict must be 'WATCH' (got {vcr.get('max_surfaced_verdict')!r})")
        if vcr.get("pass_active_strong_forbidden") is not True:
            errors.append("verdict_ceiling_rules.pass_active_strong_forbidden must be True")

    # C10 boundary.
    c10b = data.get("c10_boundary")
    if not isinstance(c10b, dict):
        errors.append("c10_boundary must be a dict")
    else:
        if c10b.get("must_not_touch_c10") is not True:
            errors.append("c10_boundary.must_not_touch_c10 must be True")
        if c10b.get("connection_status") != "deferred_not_connected":
            errors.append(
                f"c10_boundary.connection_status must be 'deferred_not_connected' (got {c10b.get('connection_status')!r})"
            )

    # Allocation baselines.
    baselines = data.get("allocation_baselines")
    if not isinstance(baselines, list) or not baselines:
        errors.append("allocation_baselines must be a non-empty list")
        baselines = []
    b_by_id = {b.get("id"): b for b in baselines if isinstance(b, dict)}
    for bid in REQUIRED_BASELINE_IDS:
        if bid not in b_by_id:
            errors.append(f"allocation_baselines missing id: {bid}")
    for b in baselines:
        if not isinstance(b, dict):
            continue
        bid = b.get("id")
        if b.get("computed_in_this_bundle") is not False:
            errors.append(f"baseline {bid!r} computed_in_this_bundle must be False (spec-only)")
        if not b.get("method"):
            errors.append(f"baseline {bid!r} missing method")
        if not isinstance(b.get("inputs"), list) or not b.get("inputs"):
            errors.append(f"baseline {bid!r} inputs must be a non-empty list")
        if not isinstance(b.get("constraints"), list) or not b.get("constraints"):
            errors.append(f"baseline {bid!r} constraints must be a non-empty list")
        if not isinstance(b.get("validation_rules"), list) or not b.get("validation_rules"):
            errors.append(f"baseline {bid!r} validation_rules must be a non-empty list")
    # ranking-only baseline must be ranking_only True.
    rk = b_by_id.get("ranking_only_marginal_efficiency")
    if isinstance(rk, dict) and rk.get("ranking_only") is not True:
        errors.append("ranking_only_marginal_efficiency must carry ranking_only == True")
    # hard-constraint baselines.
    for bid in HARD_CONSTRAINT_BASELINES:
        b = b_by_id.get(bid)
        if isinstance(b, dict) and b.get("status") != "hard_constraint":
            errors.append(f"baseline {bid!r} status must be 'hard_constraint' (got {b.get('status')!r})")

    # Allocation constraints.
    ac = data.get("allocation_constraints")
    if not isinstance(ac, dict):
        errors.append("allocation_constraints must be a dict")
    else:
        for k in REQUIRED_ALLOCATION_CONSTRAINT_KEYS:
            if k not in ac:
                errors.append(f"allocation_constraints missing key: {k}")

    # pass_watch_fail_rules.
    pwf = data.get("pass_watch_fail_rules")
    if isinstance(pwf, dict):
        for k in ("PASS", "WATCH", "FAIL"):
            if k not in pwf:
                errors.append(f"pass_watch_fail_rules missing key: {k}")
    else:
        errors.append("pass_watch_fail_rules must be a dict")

    # Non-empty list sections.
    for k in NON_EMPTY_LIST_SECTIONS:
        v = data.get(k)
        if not isinstance(v, list) or not v:
            errors.append(f"section {k} must be a non-empty list")

    # safety_boundaries phrases.
    sb = data.get("safety_boundaries") or []
    sb_join = " ".join(str(x) for x in sb).lower()
    for needle in ("research-only", "advisory-only", "no broker", "no live", "candidate #10", "no capital deployment"):
        if needle not in sb_join:
            errors.append(f"safety_boundaries missing safety phrase: {needle!r}")

    # Distinction phrases in markdown.
    mpath = repo_root / SPEC_DIR_REL / SPEC_MD
    if mpath.exists():
        md = mpath.read_text(encoding="utf-8")
        for phrase in DISTINCTION_PHRASES:
            if phrase not in md:
                errors.append(f"allocation_baseline_spec.md missing distinction phrase: {phrase!r}")
        for phrase in FORBIDDEN_PHRASES:
            if phrase.lower() in md.lower():
                errors.append(f"allocation_baseline_spec.md contains forbidden phrase: {phrase!r}")
    else:
        errors.append(f"missing: {mpath.as_posix()}")

    # JSON scan.
    blob = json.dumps(data, ensure_ascii=False).lower()
    for phrase in FORBIDDEN_PHRASES:
        if phrase.lower() in blob:
            errors.append(f"allocation_baseline_spec.json contains forbidden phrase: {phrase!r}")

    return (not errors), errors


def show(repo_root: Path = REPO_ROOT) -> int:
    data, err = _load(repo_root)
    if err:
        print(err)
        return 1
    print(f"allocation_baseline_spec_id: {data.get('allocation_baseline_spec_id')}")
    print(f"title:                {data.get('title')}")
    print(f"version:              {data.get('version')}")
    print(f"phase:                {data.get('phase')}")
    print(f"lane:                 {data.get('lane')}")
    print(f"lane_status:          {data.get('lane_status_self_declared')}")
    print(f"compute_enabled:      {data.get('compute_enabled')}")
    print(f"capital_deployment:   {data.get('capital_deployment_enabled')}")
    c10b = data.get("c10_boundary") or {}
    print(f"c10 must_not_touch:   {c10b.get('must_not_touch_c10')}")
    print(f"c10 connection:       {c10b.get('connection_status')}")
    print("safety flags (must all be False):")
    for f in MUST_BE_FALSE_FLAGS:
        print(f"  {f}: {data.get(f)}")
    baselines = data.get("allocation_baselines") or []
    print(f"allocation_baselines ({len(baselines)}):")
    for b in baselines:
        if isinstance(b, dict):
            print(f"  - {str(b.get('id', '?')):>34}  status={b.get('status')} computed={b.get('computed_in_this_bundle')} ranking_only={b.get('ranking_only')}")
    return 0


def main(argv=None) -> int:
    parser = argparse.ArgumentParser(
        description="SPARTA Portfolio Capital-Efficiency Allocation Baseline Spec validator (research-only)",
    )
    parser.add_argument("command", choices=("validate", "show"))
    parser.add_argument("--repo-root", default=str(REPO_ROOT))
    args = parser.parse_args(argv)
    root = Path(args.repo_root).resolve()
    if args.command == "validate":
        ok, errs = validate(root)
        if ok:
            print("validate: OK")
            return 0
        print("validate: FAIL")
        for e in errs:
            print(f"  - {e}")
        return 2
    if args.command == "show":
        return show(root)
    return 2


if __name__ == "__main__":
    raise SystemExit(main())
