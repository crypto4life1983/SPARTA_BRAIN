"""C14 conviction-bar follow-through real-candle detection / labels one-off runner
(READ-ONLY against existing FROZEN local BTC/ETH/SOL 1d data; RESEARCH ONLY; NO
data fetch; NO relabel; NO replay; NO PnL; NO baseline comparison; NO robustness;
NO portfolio compute; NO trading / broker / credentials / orders; NO parameter
fitting; NO paper/live readiness claim).

Runs the committed C14 detector (conviction-bar follow-through) PER ASSET over the
SHA-pinned local daily candles for BTCUSD / ETHUSD / SOLUSD, applies a labels-
stage per-asset non-overlap (REDUCE-OR-KEEP-ONLY, entry-spacing form: a later
same-asset setup whose entry is within MAX_HOLD bars of the last kept entry is
...
BEFORE any replay. It also records the weekday-neutral DEPENDENCY test (never a
trigger). NO replay, NO PnL, and NO baseline comparison are computed in this gate;
those run at the replay stage. The detector reads no date and no weekday; regime /
weekday / year are classified here ONLY for the labels-stage structural evidence.
"""
...
OUT_DIR = (REPO_ROOT / "data" / "conviction_bar_follow_through_c14"
           / "detector_labels")
LABELS_PATH = OUT_DIR / "c14_detector_labels.json"
SUMMARY_PATH = OUT_DIR / "c14_detector_summary.json"
...
def labels_stage_non_overlap(setups_sorted, max_hold):
    ...
    no PnL -- a pure labels-stage spacing rule."""
    kept = []
    block_until = None
    dropped = 0
...
def main():
    ...
    for s in SYMBOLS:
        ...
        setups = det.scan_c14_setups(bars)
        for st in setups:
    ...
    battery = {
        "sample_size_gate_passed": sample_size["passed"],
    ...
    battery["passed"] = (battery["sample_size_gate_passed"]
                         and battery["cross_asset_coverage_ok"]
    ...
    common_meta = {
        ...
        "range_atr_multiple": det.RANGE_ATR_MULTIPLE,
        "close_location_min": det.CLOSE_LOCATION_MIN,
        ...
        "honest_framing": (
            "real-candle conviction-bar follow-through labels over FROZEN local "
            "data; single-asset conviction EVENT (range outlier + top-quartile "
            "close), not long-drift, not calendar; the STRUCTURAL sample-size "
            "gate (>=100 / >=20 per asset / >=20 per regime + populated "
            "forward-OOS) is run BEFORE replay (the C13 lesson); NO replay; NO "
            "PnL; NO baseline comparison in this gate; not a profitability or "
            "paper/live-readiness claim"),
    }
    labels_payload = dict(common_meta)
    labels_payload["artifact"] = "c14_detector_labels"
    ...
    summary_payload["artifact"] = "c14_detector_summary"
