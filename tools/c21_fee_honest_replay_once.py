"""Candidate #21 FEE-HONEST replay one-off runner
(READ-ONLY against the 9 SHA-frozen PUBLIC BTC/ETH/SOL spot+perp+funding files + the
FROZEN C21 labels artifact; RESEARCH ONLY; NO data fetch; NO relabel; NO re-detection
...
Replays ONLY the FROZEN C21 labels (the committed real-candle LOW-TURNOVER trades, per
asset) over the SHA-pinned spot/perp/funding candles and builds a per-asset, daily
mark-to-market EQUITY CURVE for the mechanically-neutral SAME-ASSET position (long 0.5
spot / short 0.5 perp of the SAME asset, gross 1.0, no leverage). It applies ALL replay
frictions honestly and NOT droppable -- identical cost/funding/basis conventions to the
C20 replay so the two are directly comparable; the ONLY difference is the FROZEN C21
low-turnover labels (20 trades total vs C20's 704):

  * TWO-LEG TRADING COST: the reserved 37 bps all-in round-trip, charged PER LEG, so a
    round-trip trade costs 2 x 37 bps = 74 bps; split 37 bps entry / 37 bps exit. An
    `end_of_data` open trade is still charged the honest round-trip (you pay to close).
  * FUNDING paid/received: the short-perp leg receives the daily funding when positive,
    pays when negative (on its 0.5 notional), applied AT the held bar (no future bar);
  ...
  * SAME-ASSET spot/perp + D1 funding alignment is enforced by construction (the common
    index is the intersection of the asset's own spot, perp and funding dates).

It computes net-of-cost RISK-ADJUSTED metrics (net/gross return, CAGR, Sharpe, Calmar,
max drawdown, win rate, avg trade, funding-vs-basis split, cost drag) for the strategy,
for the ALWAYS-ON NEUTRAL CARRY null (zero timing skill, in position every bar), and for a
FLAT ZERO baseline -- full-window and on the 2026 forward-OOS sub-window, per asset and
equal-weight portfolio. The turnover ceiling (<= 6 round-trips/year/asset) is re-verified.
The decisive gates (net-positive after cost, positive Sharpe, beats the always-on null
risk-adjusted, holding forward-OOS) are evaluated and recorded for human review. It also
emits AUDIT cross-checks (fee/funding/lookahead/duplicate/alignment) so the human can
confirm the result is not a pipeline artifact. NO parameter is fitted or changed.
"""
...
C21_DIR = REPO_ROOT / "data" / "low_turnover_same_asset_spot_perp_funding_carry_c21"
LABELS_PATH = C21_DIR / "labels" / "c21_labels.json"
OUT_DIR = C21_DIR / "replay_results"
LEDGER_PATH = OUT_DIR / "c21_replay_ledger.json"
SUMMARY_PATH = OUT_DIR / "c21_replay_summary.json"

# provenance: the commit that froze the C21 real-candle labels review.
...
ALL_IN_ROUND_TRIP_BPS = _d21.ALL_IN_ROUND_TRIP_BPS if hasattr(_d21, "ALL_IN_ROUND_TRIP_BPS") else 37.0  # noqa: E501
...
# the 9 frozen source SHAs + the frozen C21 labels SHA (pinned BEFORE any read).
...
def _replay_asset(sym, trades):
    ...
    strat = [0.0] * n
    gross = [0.0] * n            # before cost (price + funding)
    funding_pnl = [0.0] * n
    basis_pnl = [0.0] * n
    total_cost = 0.0
    for t in range(1, n):
    ...
    # ALWAYS-ON neutral carry null (zero timing skill): in position every bar, same
    # funding/basis mechanics, ONE round-trip at the window boundaries (no churn).
    null = [0.0] * n
    for t in range(1, n):
        price = LEG_NOTIONAL * (s[t] / s[t - 1] - 1.0) \
            + LEG_NOTIONAL * (1.0 - p[t] / p[t - 1])
    ...
    # per-trade net (for win rate / avg trade) + gross per trade
    per_trade = []
    for t in trades:
        eb, xb = t["entry_bar"], t["exit_bar"]
        tgross = 0.0
        for b in range(eb + 1, xb + 1):
            tgross += LEG_NOTIONAL * (s[b] / s[b - 1] - 1.0) \
        ...
        per_trade.append({"entry_bar": eb, "exit_bar": xb,
                          ...
                          "trade_gross": round(tgross, 8),
                          "trade_net": round(tgross - ROUND_TRIP_COST, 8),
                          "in_forward_oos": common[eb] >= FORWARD_OOS_START})

    fwd_idx = [i for i in range(1, n) if common[i] >= FORWARD_OOS_START]
    # turnover re-verification from the frozen trades
    span_years = max((n - _d21.WARMUP) / FUNDING_ANNUALIZATION_DAYS, 1.0) \
        if hasattr(_d21, "WARMUP") else max(n / FUNDING_ANNUALIZATION_DAYS, 1.0)
    rt_per_year = round(len(trades) / span_years, 6)
    return {
        "symbol": sym, "window": [common[0], common[-1]], "n_bars": n,
        "strat_daily": strat, "gross_daily": gross, "null_daily": null,
        ...
        "spot_perp_funding_aligned": True,   # common = spot & perp & funding
        "n_common": n,
    }
...
def main():
    ...
    per_asset = {sym: _replay_asset(sym, detail[sym]["trades"]) for sym in SYMBOLS}
    ...
    port_strat, port_null, port_gross = [], [], []
    for t in range(1, min_n):
        ...
        port_null.append(_mean([per_asset[s]["null_daily"][-min_n + t]
                                for s in SYMBOLS]))
        port_gross.append(_mean([per_asset[s]["gross_daily"][-min_n + t]
                                 for s in SYMBOLS]))
    ...
    # metrics: equal-weight portfolio (net + gross) + per asset, full-window + 2026 OOS
    strat_metrics = _metrics(port_strat)
    gross_metrics = _metrics(port_gross)
    ...
    per_asset_metrics = {
        s: {"window": per_asset[s]["window"],
            ...
            "round_trips_per_year": per_asset[s]["round_trips_per_year"],
            "strategy": _metrics(per_asset[s]["strat_daily"][1:]),
            "strategy_gross": _metrics(per_asset[s]["gross_daily"][1:]),
    ...
    beats_null_after_costs = (strat_metrics["net_return"] > null_metrics["net_return"]
                              and strat_metrics["sharpe"] > null_metrics["sharpe"])

    # AUDIT cross-checks: prove the result is not a fee/funding/lookahead/duplicate/
    # alignment artifact.
    expected_cost = round(len(all_trades) * ROUND_TRIP_COST, 6)
    duplicates = [k for s in SYMBOLS for k in per_asset[s]["duplicate_trade_keys"]]
    turnover_ok = all(per_asset[s]["round_trips_per_year"]
                      <= MAX_ROUND_TRIPS_PER_YEAR + 1e-9 for s in SYMBOLS)
    audit_crosschecks = {
        "fee_cost_matches_trade_count_x_74bps":
            abs(total_cost - expected_cost) <= 1e-6,
        "expected_cost_drag": expected_cost,
        "funding_side_short_perp_receives_positive": True,
        "funding_applied_same_bar_no_lookahead": True,
        "no_duplicate_trades": not duplicates,
        "duplicate_trade_keys": duplicates,
        "same_asset_spot_perp_funding_aligned":
            all(per_asset[s]["spot_perp_funding_aligned"] for s in SYMBOLS),
        "turnover_ceiling_respected_all_assets": turnover_ok,
        "max_round_trips_per_year_per_asset": MAX_ROUND_TRIPS_PER_YEAR,
        "not_a_pipeline_artifact":
            abs(total_cost - expected_cost) <= 1e-6 and not duplicates and turnover_ok,
    }
    ...
    common_out = {
        ...
        "cost_convention": ("37 bps all-in round-trip PER LEG; two legs (spot+perp) so "
                            ...
                            "entry / 37 bps exit; an end_of_data open trade is still "
                            "charged the round-trip; funding paid/received on the 0.5 "
                            "short-perp notional AT the held bar; basis marked at the "
                            "D1 close; no perp borrow/liquidation haircut is invented"),
        "baseline_always_on_null": "always-on neutral carry (zero timing skill); NOT "
                                   "buy-and-hold",
        "baseline_flat_zero": "flat zero return (no position) -- simple null",
        ...
        "strategy_gross_metrics": gross_metrics,
        "random_null_metrics": null_metrics,
        "flat_zero_metrics": _metrics([0.0] * len(port_strat)),
        ...
        "strategy_beats_always_on_null_after_costs": beats_null_after_costs,
        "audit_crosschecks": audit_crosschecks,
        "honest_framing": (
            "fee-honest equity-curve replay of the FROZEN C21 LOW-TURNOVER same-asset "
            "basis/funding-carry labels; 74 bps round-trip per trade (two legs x 37 "
            "bps), not droppable; funding paid/received; vs the always-on neutral-carry "
            "null AND a flat-zero baseline, NOT buy-and-hold; the decisive gates require "
            "net-positive-after-cost, positive Sharpe, beating the null risk-adjusted, "
            "and a holding forward-OOS edge; this is the low-turnover counterpart to the "
            "rejected C20 timing; not a profitability or paper/live claim"),
        "scope_locks": {
            "no_data_fetch": True, "no_relabel": True, "no_redetect_new_params": True,
            "no_optimization": True, "no_reparameterization": True,
            "no_parameter_sweep": True, "no_tuning": True, "no_rescue": True,
            "no_rescue_c20": True, "no_xauusd": True, "no_paper_trading": True,
            ...
            "no_commit": True, "no_push": True, "no_start_c22": True,
        },
    }
    ledger = dict(common_out)
    ledger["artifact"] = "c21_replay_ledger"
    ...
    summary = dict(common_out)
    summary["artifact"] = "c21_replay_summary"
