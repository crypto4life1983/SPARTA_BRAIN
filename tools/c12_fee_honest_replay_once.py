"""C12 failed-breakdown reclaim reversal fee+slippage-honest replay one-off runner
(READ-ONLY against the FROZEN, SHA-pinned C12 detector-labels artifact and the
...
Replays the 206 frozen failed-breakdown reclaim labels (the FROZEN labels
artifact) over the SAME frozen, SHA-pinned daily candles, PER ASSET, with the
committed C12 geometry (entry at the reclaim close; stop = reclaim_low -
0.25*ATR(14); targets 1.5R/2R/3R; <=3-bar hold; stop-first same-bar straddle) and
a per-asset RESOLVED-EXIT non-overlap (reduce-or-keep-only). Costs are 37 bps
all-in (27 fee + 10 slippage) as R-units of each setup's stop distance.

It then runs the DECISIVE C12 gates the spec requires at the replay stage:
  * net after costs by variant,
  * MUST BEAT a matched BUY-AND-HOLD baseline (same entry windows, passive hold
    to the +3 close, same cost) -- tests whether the stop/target management beats
    passively holding the same reclaim windows,
  * MUST BEAT a matched RANDOM-ENTRY baseline (same #trades per asset, random
    ...
    averaged over deterministic resamples) -- tests whether the reclaim trigger
    beats random timing,
...
data/failed_breakdown_reclaim_reversal_c12/replay_results/ (gitignored) and
re-verifies every input SHA before/during/after. Side effects only in main().
"""
...
LABELS_INPUT = (REPO_ROOT / "data" / "failed_breakdown_reclaim_reversal_c12"
                / "detector_labels" / "c12_detector_labels.json")
EXPECTED_LABELS_SHA = (
...
OUT_DIR = (REPO_ROOT / "data" / "failed_breakdown_reclaim_reversal_c12"
           / "replay_results")
REPLAY_LEDGER_PATH = OUT_DIR / "c12_replay_ledger.json"
REPLAY_SUMMARY_PATH = OUT_DIR / "c12_replay_summary.json"
...
def _cost_r(stop_distance_bps):
    return (FEE_ROUND_TRIP_BPS / stop_distance_bps,
            ALL_IN_ROUND_TRIP_BPS / stop_distance_bps)


def _net_from_gross(gross_r, entry, stop_distance):
    bps = (stop_distance / entry) * 10000.0
    fee_r, all_in_r = _cost_r(bps)
    return gross_r - fee_r, gross_r - all_in_r


def _bucket():
    return {"trades": 0, "wins": 0, "net_all_in_r_sum": 0.0,
            "gross_r_sum": 0.0}
...
def replay_variant(labels_by_asset, bars_by_asset, variant_r):
    """Strategy replay for one variant: per-asset resolved-exit non-overlap +
    horizon walk + 37 bps costs. Returns per-setup ledger + aggregates."""
    kept_rows = []
    ...
    bh_net_all_sum = 0.0
    # chronological equity (by date) for drawdown + worst streak
    chrono = []
    ...
    for s in SYMBOLS:
        ...
        for k in res["kept"]:
            ...
            kept_rows.append({"symbol": s, "setup_index": t,
                              "date": k["date"], "regime": k["regime"],
...
def _random_entry_total(bars_by_asset, kept_count_by_asset, variant_r, seed):
    """One deterministic random-entry resample total (net all-in R), identical
    geometry + cost + per-asset non-overlap, random entry bars."""
    ...
    for s in SYMBOLS:
        ...
        for t in sorted(picks):
            ...
            if atr is None:
                continue
            stop = det.compute_stop(entry, float(bars[t]["low"]), atr)
            if not stop["valid"]:
                continue
...
def random_entry_baseline(bars_by_asset, kept_count_by_asset, variant_r):
    ...
    return {"resamples": RANDOM_RESAMPLES, "master_seed": RANDOM_MASTER_SEED,
            "mean_net_all_in_total": mean,
            "min_net_all_in_total": min(totals),
...
def main():
    ...
    # integrity: each label index lines up with the frozen bars
    for s in SYMBOLS:
    ...
    for name, mult in VARIANTS:
        ...
        rb = random_entry_baseline(bars_by_asset, kept_count_by_asset, mult)
        # percentile of strategy vs random distribution
        totals = [_random_entry_total(bars_by_asset, kept_count_by_asset, mult,
                                      RANDOM_MASTER_SEED * 1000 + i)
                  for i in range(RANDOM_RESAMPLES)]
        ...
        gates = decisive_gates(agg, agg["buy_and_hold_net_all_in_total"],
                               rb["mean_net_all_in_total"], pctl)
        ledger_rows = agg.pop("_kept_rows")
        per_variant[name] = {"aggregates": agg, "random_entry_baseline": rb,
                             "decisive_gates": gates,
                             "kept_rows": ledger_rows}
    ...
    scope_locks = {
        "no_data_fetch": True, "no_relabel": True, "no_parameter_optimization":
        True, "no_one_edit_allowance_used": True, "no_robustness": True,
    ...
    common = {
        ...
        "honest_framing": (
            "out-of-sample real-candle replay of the 206 frozen labels; "
    ...
    ledger_payload["artifact"] = "c12_replay_ledger"
    ledger_payload["per_variant"] = per_variant
    summary_payload = dict(common)
    summary_payload["artifact"] = "c12_replay_summary"
    ...
    report = {
        ...
        "any_variant_passes_all_decisive_gates": any_variant_pass,
        "structural_rejection_pressure": structural_rejection_pressure,
        "variant_summaries": summary_payload["variant_summaries"],
    }
