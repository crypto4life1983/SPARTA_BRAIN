...
Status counts: SHADOW 3, APPLIED 4

| id | status | kind | registered | in-sample evidence (frozen) | fwd n (rows) | mean ΔR | sum ΔR | p_positive | next threshold |
|---|---|---|---|---|---|---|---|---|---|
| block_long_in_TREND_DOWN | **APPLIED** | block | 2026-09-11 | avg_R=-0.338 n=4 n_signals=4 sum_R=-1.352 win_rate=0.250 | 0 (0) | - | 0.000 | - | rollback check at n>=20 (have 3): mean R < baseline - 0.2R at n >= 20 ⚠ baseline is RETIRED evidence |
| blockwhere__strategy=D__direction=short | **SHADOW** | block_where | 2026-09-23 | long_mean_R=0.342 long_n=57 long_p_pos=0.987 post_hoc=True short_2nd_half_mean_R=-0.387 short_mean_R=-0.019 short_n=140 short_p_pos=0.433 source=reports/trade_learning/d_signal_slot_and_exit_study_2026-09-23.md | 0 (0) | - | 0.000 | - | need 20 forward signals (have 0) |
...
- **blockwhere__strategy=D__direction=short** — block strict-D short entries (Donchian-20 breakdowns in TREND_DOWN); replay 2026-02-26..09-22 on the 42-symbol universe showed D shorts at mean -0.019R (n=140, p(mean>0)=0.43), -0.387R in the second half (n=54, p=0.001), vs D longs +0.342R (n=57, p=0.98); post-hoc cut, so forward test only · params {"where": {"direction": "short", "strategy": "D"}} · last change 2026-09-23: registered from learning report; in-sample evidence frozen
