"""P4 synthetic smoke battery for s20-d1 combined-universe weekly RS rotation runner. Authored at P3 BUILD; run at P4.

Synthetic data + invariant checks only. NO real CSV reads, NO signal computation on real data. Weekly R=5, top-8, 48-name union.
"""
...
def test_T2_runner_class_instantiable(runner_harness_module):
    ...
    assert algo.config["candidate_record_id"] == "s20-d1-combined-universe-weekly-relative-strength-rotation-48name-large-cap-long-history"
...
def test_T12_universe_48name_locked(runner_harness_module):
    ...
    assert len(main.CONFIG["universe"]) == 48
    assert len(set(main.CONFIG["universe"])) == 48
...
def test_T15_validator_harness_pass(runner_harness_module):
    ...
    for k in ("assert_seal_inheritance", "assert_locked_strategy_params", "assert_rotation_exit",
              "assert_equal_weight_sizing", "assert_long_only", "assert_momentum_params_locked", "assert_weekly_cadence",
              "assert_k13_fold_scheme_locked", "assert_no_per_fold_refit", "assert_split_only_convention",
              "assert_universe_locked", "assert_no_leverage_cap", "safety_counters_all_zero"):
...
def test_T18_K13_fold_scheme_locked(runner_harness_module):
    ...
    bad = dict(main.CONFIG); bsch = {k: v for k, v in main.CONFIG["k13_fold_scheme"].items()}
    bsch["folds"] = [dict(f) for f in bsch["folds"]]; bsch["folds"][0]["idx_end"] = 500; bad["k13_fold_scheme"] = bsch
...
def test_T24_combined_universe_sample_size_markers(runner_harness_module):
    c = runner_harness_module["main"].CONFIG
    assert c["is_combined_universe_sample_size_test"] is True
    assert c["edge_not_inherited_from_s18_s19"] is True
    assert c["is_s18_or_s19_revN"] is False
    assert c["selection_bias_caveat_binding"] is True
    # params identical to s18/s19 weekly (126-21 / R=5 / top-8); M=8 unchanged
