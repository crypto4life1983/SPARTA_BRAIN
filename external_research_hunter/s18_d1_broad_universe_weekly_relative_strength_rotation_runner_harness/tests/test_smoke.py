"""P4 synthetic smoke battery for s18-d1 weekly RS rotation runner. Authored at P3 BUILD; run at P4.

Synthetic data + invariant checks only. NO real CSV reads, NO signal computation on real data. Weekly R=5, top-8.
"""
...
def test_T2_runner_class_instantiable(runner_harness_module):
    ...
    assert algo.config["candidate_record_id"] == "s18-d1-broad-universe-weekly-relative-strength-rotation-24name-large-cap-long-history"
...
def test_T6_equal_weight_targets_1_over_8(runner_harness_module):
    ...
    assert all(v == pytest.approx(15000.0) for v in t.values())  # 1/8 each, NOT 1/len
...
def test_T8_rotation_exits_entries(runner_harness_module):
    main = runner_harness_module["main"]
    prev = list("ABCDEFGH"); new = list("ABCDEFGI")  # H out, I in
...
def test_T16_weekly_cadence_guard(runner_harness_module):
    ...
    bad = dict(main.CONFIG); bad["rebalance_cadence_R_days"] = 10  # fortnightly -> drift
...
def test_T22_end_to_end_synthetic_rotation_weekly_invariants(runner_harness_module, synthetic_prices):
    """Mini weekly synthetic rotation: never hold > M; equal-weight 1/M; stable monotone ranks => no churn. NOT real data."""
    main = runner_harness_module["main"]
    ...
    assert max_held == M and closed == 0  # monotone => stable top-4 => no churn
