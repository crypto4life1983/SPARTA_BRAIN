"""s18-d1 out-of-sample driver. OOS window only; run_out_of_sample() STUB. OOS isolation: no IS constants/tokens. NO execution."""
...
def verify_csv_sha256(symbol):
    ...
    if actual != entry["sha256"]:
        raise Exception(f"S18_D1_OOS_CSV_SHA_MISMATCH: symbol={symbol} expected {entry['sha256']} got {actual}")
    return True
...
def run_out_of_sample(*args, **kwargs):
    raise RuntimeError(
        ...
        "OOS gate). P3 BUILD scope and this s18 continuation bundle do NOT authorize any OOS signal computation, "
        "backtest, or simulator run."
    )
