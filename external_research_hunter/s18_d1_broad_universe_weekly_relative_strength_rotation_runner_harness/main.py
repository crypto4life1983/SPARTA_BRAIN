"""s18-d1 broad-universe WEEKLY relative-strength rotation algorithm.

Inherits Phase 2 safety contracts (C1-C8). Cross-sectional relative-strength rotation family; long-only; WEEKLY.
Fresh sibling of terminal s17-d1 (monthly) and the K9-blocked fortnightly s18 PLAN. Same parameterized primitives;
weekly CONFIG (R=5, top-8). Pre-SEAL K9 gate measured-PASS; the EDGE is untested (re-proven at P6 IS).

Anchors:
  tier_n_spec_seal     = 06e96e3cc7902a196cba33cc5cb57587fb4fd3c1034737715b85aebaf546940e
  plan_lock_seal       = 22584b83561a5ca53524c8f36205690224822fb71fca603d5abde47581c5be35
  p2_phase2_plan_seal  = c988cefaaa60c8e38d649a301fed7ed47065a1f91925a61c290c179cd85732c7
...
# K13 walk-forward fold scheme LOCKED byte-equivalent at SEAL (DA22; identical to the s17 24-name calendar; UNSEARCHED).
K13_FOLD_SCHEME = {
...
CONFIG = {
    ...
    "phase_prefix": "PHASE2-S18-D1-WEEKLY-RS-ROTATION",
    "algo_version_for_run_id": "s18_d1_weekly_rs_rotation_v0_1_0",
    "title": "s18-d1 broad-universe weekly relative-strength rotation fresh-sibling (paper-research-only; DIAGNOSTIC_ONLY_NOT_LIVE_GRADE)",
    "tier_spec_seal": "06e96e3cc7902a196cba33cc5cb57587fb4fd3c1034737715b85aebaf546940e",
    "plan_lock_seal": "22584b83561a5ca53524c8f36205690224822fb71fca603d5abde47581c5be35",
    "p2_phase2_plan_seal": "c988cefaaa60c8e38d649a301fed7ed47065a1f91925a61c290c179cd85732c7",
    ...
    # Strategy params (LOCKED at SEAL; weekly cross-sectional relative-strength rotation)
    ...
    "output_schema_id": "sparta.s18.d1.weekly_rs_rotation.diagnostic_run_report.v1",
    "rec1_equivalent_oos_k9_disclosure": (
        "OOS K9 >=50/y. Pre-SEAL turnover measurement projects ~100-113/2y (clears marginally) but the TRUE OOS K9 "
    ...
    "edge_not_inherited_from_s17_monthly": True,  # weekly edge UNTESTED; re-prove at P6 IS
}


# --- Cross-sectional relative-strength primitives (NO execution, NO order placement; parameterized) ---
...
def add_pyramid_unit(*args, **kwargs):
    raise RuntimeError(
        "PYRAMID_FORBIDDEN: s18-d1 holds the top-M rotation set with one lot per name; pyramiding is structurally "
...
def open_short_position(*args, **kwargs):
    raise RuntimeError(
        "SHORTING_FORBIDDEN: s18-d1 is long-only (DA20); the short leg is structurally forbidden per SEAL."
    )


class Algo(_QCAlgorithmBase):
    """s18-d1 weekly relative-strength rotation algorithm. P3 BUILD scope; NO backtest."""

    def Initialize(self):  # noqa: N802
        if self.LiveMode:
            raise Exception(
                "LIVE_PATH_DETECTED: s18-d1-broad-universe-weekly-relative-strength-rotation is paper-only forever; "
