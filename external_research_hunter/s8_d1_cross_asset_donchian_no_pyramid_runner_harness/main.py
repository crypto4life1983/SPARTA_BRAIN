"""s8 D1 cross-asset Donchian (NQ + GC + ZN + CL) -- algorithm runner.
...
Inherits Tier-N spec seal:    8cff6babf8e4a451adf02e94a684924ff8b32a7e0f5a795a13c65c845a12e0f4
Inherits plan-lock seal:      612abbbda7235c8c01239000cf997c804cd8178d88d2afbb9752004aed34e0a1
Inherits Phase-2 plan seal:   5e6fccd1aeb40db7daf850ab60eff2947a03a082a6bcb5b332c967e2d8f9c826
Predecessor (s8 selection):   6b7bdb4c350f4a779611546dcb32f6a83db2371c66d7b6ba0118121783801441
"""
...
ALGO_VERSION_FOR_RUN_ID = "s8_d1_v0_1_0"
PHASE_PREFIX            = "PHASE2-S8-D1-XAD-NP"
CANDIDATE_RECORD_ID     = "s8-cross-asset-donchian-no-pyramid-nq-gc-zn-cl"
...
CONFIG = {
    ...
    "max_units_per_market":           1,   # S8-D1 DELTA: was 4 in s7-D1; no-pyramid invariant
    "pyramid_spacing_n_multiplier":   0.5, # vestigial under max_units=1; never fires (per S8-D1 plan-lock)
    ...
    "portfolio_cap_max_units":        4,   # S8-D1 DERIVED: 4 markets x 1 unit; structurally non-binding
...
class PortfolioCapTracker:
    """Tracks total open UNITS across markets. S8-D1: Cap = 4 markets * 1 unit = 4.
    ...
    S8-D1 invariant: per_market_cap=1; any unit_count > 1 fails safety. This is
    ...
    def update_market_units(self, market: str, unit_count: int) -> None:
        if unit_count > self.per_market_cap:
            ...
            #   2) S8-D1 no-pyramid invariant violation: per-market unit count exceeded 1
            raise ValueError(
                ...
                f"S8-D1 no-pyramid invariant: per-market open unit count must be 0 or 1. "
...
# Algorithm class
# ---------------------------------------------------------------------------
class EightD1CrossAssetDonchianNoPyramidAlgo(_QCAlgorithmBase):
    """s8 D1 cross-asset Donchian NO-PYRAMID algorithm.
    ...
    Single delta from s7-D1: max_units_per_market = 1 (was 4). All other locked
    ...
    def Initialize(self):  # noqa: N802 (QC convention)
        ...
        if getattr(self, "LiveMode", False):
            raise Exception(
                "LIVE_PATH_DETECTED: s8-cross-asset-donchian-no-pyramid-nq-gc-zn-cl "
