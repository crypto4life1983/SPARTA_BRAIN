"""s8 D1 T1-T15 smoke test SCAFFOLD (synthetic-only).
...
from external_research_hunter.s8_d1_cross_asset_donchian_no_pyramid_runner_harness import main as runner_main
from external_research_hunter.s8_d1_cross_asset_donchian_no_pyramid_runner_harness import execution_guard
...
def test_t1_module_imports_clean():
    assert runner_main.CANDIDATE_RECORD_ID == "s8-cross-asset-donchian-no-pyramid-nq-gc-zn-cl"
    assert runner_main.ALGO_VERSION_FOR_RUN_ID == "s8_d1_v0_1_0"
    assert runner_main.PHASE_PREFIX == "PHASE2-S8-D1-XAD-NP"
    # Inheritance shas embedded
    assert runner_main.TIER_N_SPEC_SEAL_SHA256 == "8cff6babf8e4a451adf02e94a684924ff8b32a7e0f5a795a13c65c845a12e0f4"
    assert runner_main.PLAN_LOCK_SEAL_SHA256   == "612abbbda7235c8c01239000cf997c804cd8178d88d2afbb9752004aed34e0a1"
    assert runner_main.PHASE2_PLAN_SEAL_SHA256 == "5e6fccd1aeb40db7daf850ab60eff2947a03a082a6bcb5b332c967e2d8f9c826"
...
def test_t2_runner_class_instantiable():
    algo = runner_main.EightD1CrossAssetDonchianNoPyramidAlgo()
    # Stubbed Initialize should not raise when QC is absent (LiveMode defaults to False)
    algo.Initialize()
    assert algo._backtest_run_id.startswith("PHASE2-S8-D1-XAD-NP-")
...
def test_t7b_add_pyramid_unit_raises_under_max_units_1():
    """S8-D1 no-pyramid invariant: add_pyramid_unit must raise when called with
...
def test_t9_portfolio_cap_uses_unit_count_not_contract_count():
    # S8-D1: 4 markets * 1 unit = 4 units total. per_market_cap=1.
