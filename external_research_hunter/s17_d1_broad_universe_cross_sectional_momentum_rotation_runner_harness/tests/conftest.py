"""pytest fixtures + env guards for s17-d1 cross-sectional momentum runner harness tests."""
...
def synthetic_prices():
    """Deterministic synthetic multi-symbol close series (NOT real data). 6 symbols x 200 bars.

    Constructed so the cross-sectional ranking is unambiguous: each symbol grows at a distinct
    constant daily rate, so momentum rank == symbol order, exercising rank/select/rotation logic.
    """
    n = 200
...
def synthetic_csv_path():
    return pathlib.Path(__file__).parent / "fixtures" / "synthetic_broad_universe_daily.csv"
...
def runner_harness_module():
    ...
    pkg = ("external_research_hunter."
           "s17_d1_broad_universe_cross_sectional_momentum_rotation_runner_harness")
    import importlib
    return {
