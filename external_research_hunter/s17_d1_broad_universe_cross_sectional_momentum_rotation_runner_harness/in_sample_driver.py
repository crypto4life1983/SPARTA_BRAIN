"""s17-d1 in-sample driver. Filters reused 24-name DR9-passed CSVs to the IS window. NO execution."""
...
def verify_csv_sha256(symbol):
    ...
    if actual != entry["sha256"]:
        raise Exception(f"S17_D1_CSV_SHA_MISMATCH: symbol={symbol} expected {entry['sha256']} got {actual} path={entry['path']!r}")
    return True
...
def run_in_sample(*args, **kwargs):
    raise RuntimeError(
        ...
        "(Authorize s17-d1 broad-universe cross-sectional momentum P6 IS diagnostic only). P3 BUILD scope "
