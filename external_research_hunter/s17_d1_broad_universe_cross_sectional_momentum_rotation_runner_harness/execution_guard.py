"""s17-d1 execution guard. C1-C8 attestation; momentum rotation guards; K13 fold-scheme + no-refit guards. NO execution/order/fetch."""

EXPECTED_TIER_SPEC_SEAL = "ff4b237436cf116b5ab3f1f28cfbd966a99d21020589bec4a1892004fc16b5dc"
EXPECTED_PLAN_LOCK_SEAL = "e848b7c4efe900f029ebc51222c7b15bb6652a21a7f306cc38327f21303888ae"
EXPECTED_P2_PHASE2_PLAN_SEAL = "4b92816c9d1825924c87554e13534e2fc74b59fbcb73569078cd3edb1bc85115"
...
S16_D1_P11_TERMINAL_COMMIT = "99f58bd"
...
def assert_no_forbidden_order_paths(config):
    if config.get("brokerage_model_name") != "NOT_APPLICABLE_CSV_SIMULATOR_AT_P6":
        raise Exception("BROKERAGE_MODEL_FORBIDDEN: s17-d1 P3 BUILD authorizes only CSV-simulator at P6.")
...
def assert_rotation_exit(config):
    """DA12: exit MUST be relative-rank rotation (NOT a trailing/ATR stop or oscillator threshold)."""
...
def assert_equal_weight_sizing(config):
    """DA3: equal-weight 1/M sizing; long-only, unlevered."""
    if config.get("sizing_method") != "equal_weight":
        raise Exception(f"SIZING_DRIFT: {config.get('sizing_method')!r} expected 'equal_weight'")
    if abs((config.get("per_position_weight_fraction") or 0) - (1.0 / 6.0)) > 1e-9:
        raise Exception("PER_POSITION_WEIGHT_DRIFT: expected 1/6")


def assert_long_only(config):
    """DA20: long-only; no shorting; no leverage."""
...
def assert_momentum_params_locked(config):
    """DA6/DA8/DA10/DA11: lookback/skip/top-M/rebalance locked; NOT optimized."""
    for key, want in (("momentum_lookback_L", 126), ("momentum_skip_S", 21), ("top_m_held", 6), ("rebalance_cadence_R_days", 21)):
        if config.get(key) != want:
...
def assert_k13_fold_scheme_locked(config):
    """DA22: the 5 fixed pre-committed UNSEARCHED fold boundaries must match byte-equivalent."""
...
def assert_no_per_fold_refit(config):
    """K13 = VALIDATION of locked params, not optimization; no per-fold refit."""
    scheme = config.get("k13_fold_scheme") or {}
