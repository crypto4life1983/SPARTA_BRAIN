"""s14-d1-cross-sector out-of-sample driver. Filters sealed cross-sector CSVs to OOS window only.
...
REC1-equivalent BINDING reminder (carried from SEAL):
  ...
  expected effective rate 45-79/y vs 50/y OOS floor). If observed effective IS rate falls below
  25/y basket-summed, OOS K9 unreachability becomes structurally probable. Expected terminal
  verdict if K9 fires at OOS: PARKED_SAFE_BUT_OOS_INDETERMINATE_K9_FIRED.

By construction, this file does NOT contain in-sample-window constants, does NOT contain
standalone in-sample year tokens, and does NOT contain a run_in_sample function. OOS isolation is
structurally enforced by separate files.
...
def run_out_of_sample(*args, **kwargs):
    ...
    REC1-equivalent BINDING: OOS K9 is reachable with improved margin vs all-tech but still
    borderline at the lower bound; expected terminal verdict if K9 fires at OOS is
    ...
    raise RuntimeError(
        ...
        "(Authorize s14-d1-cross-sector cash-equity P10 OOS gate only). P3 BUILD scope does NOT "
        "authorize OOS inspection, signal computation, or simulator run on OOS rows."
    )
