"""s21-d1 K13 walk-forward driver. The 5 fixed pre-committed UNSEARCHED DA22 folds. run_walk_forward() STUB.
K13 = VALIDATION of the LOCKED rule across folds, NOT optimization. NO per-fold refit/fold-search/cadence tuning. NO execution at BUILD."""
...
def run_walk_forward(*args, **kwargs):
    raise RuntimeError(
        "P6_7_K13_NOT_AUTHORIZED: run_walk_forward() is gated by separate operator authorization (P6.7 K13 "
        "walk-forward validation). P3 BUILD scope does NOT authorize any signal computation, backtest, or simulator run."
    )
