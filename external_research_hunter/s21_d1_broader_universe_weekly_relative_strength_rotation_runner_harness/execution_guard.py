"""s21-d1 execution guard. C1-C8 attestation; weekly RS rotation guards; K13 fold-scheme + no-refit guards;
48-name calendar-alignment guard. NO execution/order/fetch."""

EXPECTED_TIER_SPEC_SEAL = "0057308a4b8d6b9aa4f7126537fca43389336579871960330e4045d60444244f"
EXPECTED_PLAN_LOCK_SEAL = "369a4fce800c9d3525bd65a50e717fe065224fca27e4a0ac9f553b3cba16af3b"
EXPECTED_P2_PHASE2_PLAN_SEAL = "f55d898d5d292a3647c98ba640a30cdb5612d59fe8b328ae585c0ffc9df8186e"

S21_DR9_RESULT_SEAL = "9835c0d2a21f40eb0e24317af1aa196a71bbb59b9339f7cc5dcaa7b23f48c07a"
WALK_FORWARD_K13_SEAL = "4268d6f75bbc095a795510f7d8ccc50c2d8886eef36f50f769b79342002893d2"
FRAMEWORK_DR10_V2_COMMIT = "78cd22e"
S20_D1_SEAL_FROZEN_COMMIT = "e9ec72f"  # parent OOS_CONFIRMED s20; frozen/untouched

LOCKED_UNIVERSE = ["AVGO", "QCOM", "TXN", "INTC", "MU", "AMAT", "IBM", "INTU", "NOW", "ADI", "VZ", "T", "CHTR", "EA", "SBUX", "TGT", "BKNG", "MAR", "GM", "TJX", "ROST", "MO", "CL", "KMB", "GIS", "STZ", "PFE", "BMY", "AMGN", "GILD", "CVS", "CI", "ISRG", "SYK", "C", "USB", "SCHW", "BLK", "SPGI", "CB", "BA", "ITW", "UPS", "RTX", "LMT", "DE", "PSX", "VLO"]
...
def assert_no_forbidden_order_paths(config):
    if config.get("brokerage_model_name") != "NOT_APPLICABLE_CSV_SIMULATOR_AT_P6":
        raise Exception("BROKERAGE_MODEL_FORBIDDEN: s21-d1 P3 BUILD authorizes only CSV-simulator at P6.")
...
def assert_locked_strategy_params(config):
    expected = {
        "mechanic_family": "cross_sectional_relative_strength_rotation_weekly_long_only",
...
def assert_rotation_exit(config):
    if config.get("exit_rule") != "ROTATION_RELATIVE_RANK":
        raise Exception(f"EXIT_RULE_DRIFT: {config.get('exit_rule')!r} expected 'ROTATION_RELATIVE_RANK'")
    if config.get("exit_is_trailing_or_atr_stop") is not False:
        raise Exception("EXIT_DESIGN_DRIFT: exit_is_trailing_or_atr_stop must be False")
...
def assert_split_only_convention(config):
    ...
    a = next((x for x in actions if x.get("symbol") == "AVGO" and x.get("date") == "2024-07-15"), None)
    if a is None or not (a.get("applied") and a.get("dr9_verified")):
        raise Exception("C5_AVGO_SPLIT_NOT_DOCUMENTED_OR_NOT_VERIFIED")
    for x in actions:
        if not (x.get("applied") and x.get("dr9_verified")):
            raise Exception(f"C5_CORP_ACTION_NOT_VERIFIED: {x.get('symbol')} {x.get('date')}")


def assert_universe_locked(config):
    if list(config.get("universe") or []) != LOCKED_UNIVERSE:
        raise Exception("UNIVERSE_DRIFT: CONFIG['universe'] expected the 48-name fresh s21 universe")
...
def assert_calendar_aligned_48(series_by_symbol, dates_by_symbol=None, expected_count=48):
    ...
    if len(lengths) != 1:
        raise Exception(f"CALENDAR_ALIGN_LENGTH_MISMATCH: lengths {sorted(lengths)} not uniform")
    if dates_by_symbol is not None:
        ...
        for sym, ds in dates_by_symbol.items():
            if ref is None:
                ...
                raise Exception(f"CALENDAR_ALIGN_DATE_MISMATCH: {sym} differs from reference (HALT, not silent reindex)")
    return True
...
def full_guard_check(config, safety_counters=None):
    ...
    for name, fn in (("assert_seal_inheritance", assert_seal_inheritance), ("assert_no_forbidden_order_paths", assert_no_forbidden_order_paths),
                     ("assert_rec1_equivalent_binding_preserved", assert_rec1_equivalent_binding_preserved), ("assert_locked_strategy_params", assert_locked_strategy_params),
                     ("assert_rotation_exit", assert_rotation_exit), ("assert_equal_weight_sizing", assert_equal_weight_sizing), ("assert_long_only", assert_long_only),
                     ("assert_momentum_params_locked", assert_momentum_params_locked), ("assert_weekly_cadence", assert_weekly_cadence),
                     ("assert_k13_fold_scheme_locked", assert_k13_fold_scheme_locked), ("assert_no_per_fold_refit", assert_no_per_fold_refit),
                     ("assert_boundary_alignment", assert_boundary_alignment), ("assert_split_only_convention", assert_split_only_convention),
                     ("assert_universe_locked", assert_universe_locked), ("assert_no_leverage_cap", assert_no_leverage_cap)):
        _run(name, fn)
