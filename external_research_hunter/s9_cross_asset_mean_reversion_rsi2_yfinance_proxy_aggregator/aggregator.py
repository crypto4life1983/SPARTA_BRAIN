"""Result-aggregation module for s9 cross-asset RSI-2 mean-reversion
(ETF-proxy track).

Pure deterministic in-memory aggregator. Consumes one s9 SimulationResult
per cost tier (S0/S1/S2/S3) plus the four LoadedSymbol structures plus
the C1-C8 safety attestations. Computes per-trade statistics, per-symbol
statistics, portfolio-level statistics, the cross-symbol pairwise
dependence measure, effective independent bets, the cost-stress matrix
with DR2/DR3/DR5 fail-fast evaluation, K1-K12 rejection criteria, A1-A10
acceptance gates, and the IS-close verdict from the 8-value closed
VerdictReason enum.

Verdict values (byte-equivalent to Step 07 / s9 Tier-N spec sec 17):
  PARKED_PROVENANCE_BROKEN, REJECT_FAST, PARKED_SAFETY_FAILED,
  ...
  does NOT auto-trigger any onward work and does NOT change any
...
The s9 mechanic differs from the parent s7 D1 mechanic in three ways
that this aggregator handles structurally:

  - The s9 simulator emits flat TradeRecord rows (no group / unit
    hierarchy; MAX_UNITS_PER_SYMBOL = 1 per Tier-N section 12).
    Per-trade statistics read each TradeRecord directly without any
    inner-loop traversal over a unit list.
  - The s9 simulator has no hard stop and no volatility-based sizing
    (equal-dollar 1% allocation per Tier-N section 11), so this
    aggregator does NOT compute an R-multiple at the per-trade level
    (a 2N-risk denominator is undefined for s9). The trade_r_multiple
    field is omitted from PerTradeStats.
  - The s9 mechanic does not amplify per symbol; the structural cap
    binding events count is identically zero (the aggregator reads
    the count as 0 for every TradeRecord); A10 and K11 trivially
    clear.

This module does NOT:

  - run a simulator; the caller does that and passes results;
  - call the loader, validator, signal module, or simulator module;
  ...
  - mutate any downstream review queue, idea memory, ORB artifact,
    or any production-side artifact;
  - run any optimization, sweep, parameter search, filter, regime
    gate, asset selection, or winner-selection rule;
...
# Pinned upstream csv sha256 values (inherited byte-equivalent from the
# Step 02b audit + Step 07 aggregator; the s9 chain reuses the same
# sealed dataset; cf. Tier-N spec section 6).
_CSV_SHA_PINS = {
...
class PerTradeStats:
    """One row per closed long entry-exit cycle. Long-only by type:
    direction is always 'long'. No R-multiple field (s9 has no
    ATR-based per-trade risk denominator)."""
    symbol: str
    trade_id: str
...
class AggregationResult:
    ...
    # FORBIDDEN_TOKEN_EXCLUSION: spec sec 4 mandates these four negative-attestation fields.
    oos_aggregation_intentionally_omitted: bool
    post_oos_aggregation_intentionally_omitted: bool
...
def _looks_like_simulation_result(obj):
    ...
    # s9 simulator dataclass; this is a structural duck-type check,
    # not an active-OOS or live-action runtime reference. The s9
    # simulator's SimulationResult exposes trade_records (flat) and
    # num_closed_trades_total (no group / unit hierarchy).
    _SIM_FIELDS = (
        "starting_cash", "final_cash_balance", "cost_tier",
        "cost_tier_slippage_scalar", "cost_tier_commission_scalar",
        "trade_records", "num_closed_trades_total", "daily_equity_ledger",
        "max_drawdown_pct_observed", "k4_fired", "in_sample_window",
        "first_signal_date_processed", "last_signal_date_processed",
        # FORBIDDEN_TOKEN_EXCLUSION: schema-field-name from sealed s9 simulator.
...
def _validate_inputs(loaded, simulation_results, safety_attestations):
    ...
    if not isinstance(simulation_results, Mapping):
        raise AggregatorInputError(
            f"simulation_results must be Mapping; "
            f"got {type(simulation_results).__name__}"
        )
    ...
    if not isinstance(safety_attestations, Mapping):
        raise AggregatorInputError(
            f"safety_attestations must be Mapping; "
            f"got {type(safety_attestations).__name__}"
        )
...
def _verify_dates_in_sim(sr, tier):
    ...
    for r in sr.trade_records:
        _verify_date_is(r.entry_trigger_date,
                        f"simulation_results[{tier!r}].trade_records.entry_trigger_date")
        _verify_date_is(r.entry_fill_date,
                        f"simulation_results[{tier!r}].trade_records.entry_fill_date")
        _verify_date_is(r.exit_trigger_date,
                        f"simulation_results[{tier!r}].trade_records.exit_trigger_date")
        _verify_date_is(r.exit_fill_date,
                        f"simulation_results[{tier!r}].trade_records.exit_fill_date")
    for dp in sr.daily_equity_ledger:
        _verify_date_is(dp.date,
                        f"simulation_results[{tier!r}].daily_equity_ledger.date")
...
def _build_per_trade_stats(simulation_result):
    """Build PerTradeStats rows from the s9 SimulationResult.trade_records.

    Each TradeRecord IS a complete closed long entry-exit cycle (no
    group/unit hierarchy; the s9 mechanic does not amplify per symbol).
    No R-multiple is reported (the s9 mechanic has no per-trade risk
    denominator; equal-dollar sizing makes R undefined).
    """
    out = []
    for r in simulation_result.trade_records:
        out.append(PerTradeStats(
            symbol=r.symbol,
            trade_id=r.trade_id,
            trade_pnl_dollars=r.net_pnl_dollars,
            trade_entry_date=r.entry_fill_date,
            trade_exit_date=r.exit_fill_date,
            trade_duration_days=_date_diff_days(r.entry_fill_date,
                                                r.exit_fill_date),
            trade_direction=r.direction,
            trade_exit_reason=(r.exit_reason.value
                               if hasattr(r.exit_reason, "value")
                               else str(r.exit_reason)),
            trade_is_win=(r.net_pnl_dollars > 0),
        ))
    return tuple(out)
...
def _build_per_symbol_stats(simulation_result):
    ...
    for sym, recs in by_sym.items():
        trades_count = len(recs)
        net = sum(r.net_pnl_dollars for r in recs)
        gross = sum(r.gross_pnl_dollars for r in recs)
        wins = [r.net_pnl_dollars for r in recs if r.net_pnl_dollars > 0]
        losses = [abs(r.net_pnl_dollars) for r in recs if r.net_pnl_dollars < 0]
        breakevens = [r for r in recs if r.net_pnl_dollars == 0]
...
def _build_portfolio_stats_from_records(records, starting_cash):
    """Build portfolio stats from a list of TradeRecord. Used for both
    the S1-baseline portfolio_stats and the per-tier CostStressRow rows.

    Sharpe proxy per trade: mean(net_pnl) / stdev(net_pnl). Per-trade,
    not annualized. Win rate, P/L ratio, implied breakeven WR, WR gap.
    Trade-curve cumulative PnL, high-water mark, max drawdown dollars,
    max drawdown pct vs starting cash. Cap binding events count is
    structurally zero on s9 (MAX_UNITS_PER_SYMBOL = 1).
    """
    pnls = [r.net_pnl_dollars for r in records]
    total = len(pnls)
    total_net = sum(pnls)
    total_gross = sum(r.gross_pnl_dollars for r in records)
    mean_pnl = (sum(pnls) / total) if total > 0 else None
    stdev_pnl = statistics.stdev(pnls) if total >= 2 else None
    sharpe = (mean_pnl / stdev_pnl) if (mean_pnl is not None
                                        and stdev_pnl is not None
                                        and stdev_pnl > 0) else None
    expectancy = mean_pnl
    ...
    ibwr = (1.0 / (1.0 + pl)) if pl is not None and pl > 0 else None
    wr_gap = ((wr - ibwr) * 100.0) if (wr is not None and ibwr is not None) else None
    # Trade curve max drawdown: sort by close date for deterministic curve.
    ordered = sorted(records,
                     key=lambda r: (r.exit_fill_date, r.symbol, r.trade_id))
    cum = 0.0
    curve = []
    hwm = 0.0
    max_dd_dollars = 0.0
    for r in ordered:
        cum += r.net_pnl_dollars
        curve.append((r.exit_fill_date, cum))
        if cum > hwm:
            hwm = cum
        dd = hwm - cum
    ...
    # Cap binding events: structurally zero on s9 (MAX_UNITS_PER_SYMBOL = 1).
    cap_binding = 0
...
def _build_cost_stress_row(tier, sim_result):
    ps = _build_portfolio_stats_from_records(list(sim_result.trade_records),
                                             sim_result.starting_cash)
...
def _in_sample_adj_close_daily_returns(loaded, sym):
    """Return list of daily IS-window returns for one symbol.

    Computed on adj_close (the dividend-adjusted series) over the
    in-sample slice. Values strictly within IN_SAMPLE_WINDOW; no OOS
    bar is read.
    """
    ...
    if len(dates) != len(adj):
        raise AggregatorInputError(
            f"loaded[{sym}] dates/adj_close length mismatch"
        )
...
def _avg_pairwise_dependence_measure(loaded):
    """Return the average Pearson coefficient across all pairs of the
    four symbols' IS adj_close daily-return series. Byte-equivalent to
    the Step 07 aggregator's diversification metric for inheritance
    fidelity (the diversification finding is a universe property, not
    a mechanic property; the same series and same coefficient are
    used)."""
    sym_returns = {sym: _in_sample_adj_close_daily_returns(loaded, sym)
                   for sym in sorted(_ALLOWED_SYMBOLS)}
...
def _evaluate_dr_rules(cost_stress_matrix):
    ...
    #      OR total_net_pnl at S2/S3 < threshold_fraction * S1 while
    #      both positive.
    ...
    # DR3: S0 positive AND any of (S1, S2, S3) non-positive.
    ...
    return {
        ...
        # FORBIDDEN_TOKEN_EXCLUSION: DR4 is the deferred-to-next-window
        # diagnostic; the literal string here is a NEGATIVE attestation
        # ("DEFERRED") and not an active-OOS computation reference.
...
def _evaluate_k(portfolio_stats, avg_pair_dep, safety_attestations,
                dr_fires, k8_drift):
    ...
    k7_a = bool(safety_attestations.get(
        "filter_silently_introduced_attestation", False))
    k7_b = bool(safety_attestations.get(
        "dependence_gate_silently_introduced_attestation",
        safety_attestations.get(
            "correlation_gate_silently_introduced_attestation", False)))
    k7 = k7_a or k7_b
    k8 = k8_drift
...
def _assemble_verdict(k_results, a_results):
    """Priority order per Tier-N spec section 17 / Step 07 sec 16:
    K8 PROVENANCE > K12 REJECT_FAST > K6/K7 SAFETY > K10 DIVERSIFICATION
    > K9 SAMPLE > K11 CAP > K1/K2/K4 MONEY-NOT-PROVEN > A1-A10 evaluation
    > ELIGIBLE_FOR_OOS.
    """
    ...
    if k_results.K10_avg_pairwise_dependence_above_threshold:
        return (VerdictReason.PARKED_FAILED_AT_DIVERSIFICATION_HYPOTHESIS,
                "K10 fires: avg pairwise dependence measure > 0.50; "
                "diversification hypothesis empirically rejected.")
    ...
    if (k_results.K1_sharpe_below_zero or
        ...
        reasons = []
        if k_results.K1_sharpe_below_zero:
            reasons.append("K1 Sharpe<0")
        if k_results.K2_expectancy_nonpositive:
            reasons.append("K2 expectancy<=0")
        if k_results.K4_trade_curve_maxdd_above_50:
            reasons.append("K4 MaxDD>50%")
    ...
    if all_a_pass:
        return (VerdictReason.ELIGIBLE_FOR_OOS,
                ...
                "any subsequent-window inspection requires a separately "
                "authorized turn.")
...
def aggregate(loaded, simulation_results, safety_attestations, **kwargs):
    """Aggregate s9 simulator outputs into a verdict-grade
    AggregationResult per the Tier-N spec sections 16-17. IS-only.
    Raises AggregatorOosBlockedError on any drift outside IS window.
    """
    if kwargs:
    ...
    # Provenance (K8) check.
    k8_drift, drift_reasons, observed_provenance = _check_provenance(
        loaded, simulation_results,
    )
    ...
    portfolio = _build_portfolio_stats_from_records(
        list(s1.trade_records), s1.starting_cash,
    )
    ...
    # Upstream phases all pass: A6 check.
    upstream_all_pass = bool(safety_attestations.get(
        "upstream_phases_all_pass", True,
    ))
    ...
    return AggregationResult(
        schema="sparta.s9.rsi2.aggregation_result.v1",
        ...
        # FORBIDDEN_TOKEN_EXCLUSION: spec sec 4 mandates these negative attestations.
        oos_aggregation_intentionally_omitted=True,
        post_oos_aggregation_intentionally_omitted=True,
