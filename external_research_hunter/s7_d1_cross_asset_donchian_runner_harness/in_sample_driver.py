"""s7 D1 in-sample driver -- local Databento DBN decoder + Donchian backtest.
...
Inherited sealed-chain anchors:
  ...
  runner_build_report_seal_sha256:              10610a6ad47c2fd584b556e773edb3ea09c8dfa9fb67aaa350b526938df03946
  execution_guard_build_report_seal_sha256:     5cfbfdbbb9fc695673e5d8dabce0019a67d053a7b18ad962962a9a97311b017e
  smoke_pass_report_seal_sha256:                ec244e92953ab850f68f7ec88945c80263bb40f154a90bba19bf930f4c9133e8
  blocked_report_seal_sha256:                   f0f465d4c9b9199c4a45c060b8ff2552368128c5086354307394d2f8999fccf0

This module is a P3.5 PATCH: it is BUILT but NOT EXECUTED at P3.5. Execution
of `run_in_sample()` requires a separate operator authorization (P6.5).
"""
...
# ---------------------------------------------------------------------------
CACHE_ROOT = Path(r"C:\SPARTA_BRAIN\data\databento_cache")
SYMBOLS = {
    "NQ": "NQ.c.0",
    "GC": "GC.c.0",
    "ZN": "ZN.c.0",
...
IN_SAMPLE_END   = datetime.date(2022, 12, 30)
...
OUTPUT_DIAGNOSTIC_JSON = Path(r"C:\SPARTA_BRAIN\reports\external_research_hunter\s7_d1_cross_asset_donchian_in_sample_diagnostic_result_sealed.json")
OUTPUT_DIAGNOSTIC_MD   = Path(r"C:\SPARTA_BRAIN\reports\external_research_hunter\s7_d1_cross_asset_donchian_in_sample_diagnostic_result_sealed.md")
...
EXPECTED_SEALS = {
    ...
    "runner_build_report":           RUNNER_BUILD_REPORT_SEAL_SHA256,
    "execution_guard_build_report":  EXECUTION_GUARD_BUILD_REPORT_SEAL_SHA256,
    "smoke_pass_report":             SMOKE_PASS_REPORT_SEAL_SHA256,
    "blocked_report":                BLOCKED_REPORT_SEAL_SHA256,
...
# ---------------------------------------------------------------------------
# Hard runtime checks (called by run_in_sample BEFORE any DBN decode)
# ---------------------------------------------------------------------------
...
def derive_rth_daily_bars(market: str) -> List[Dict[str, Any]]:
    ...
    Window filter: bars whose ET timestamp date in [IN_SAMPLE_START, IN_SAMPLE_END]
    ...
    for bar in iterate_market_minute_bars(market):
        ...
        if not (IN_SAMPLE_START <= ts_et.date() <= IN_SAMPLE_END):
            continue
...
def _import_runner_main():
    ...
    """
    from external_research_hunter.s7_d1_cross_asset_donchian_runner_harness import main as runner_main  # noqa: WPS433
    return runner_main
...
def run_in_sample(*, expected_seals: Optional[Dict[str, str]] = None,
    ...
    sizing on NQ + GC + ZN + CL across the in-sample window, applies the
    ...
    return {
        "schema_id": "sparta.external_research_hunter.s7_d1_cross_asset_donchian_in_sample_diagnostic.v1",
        ...
        "in_sample_window": {"start": IN_SAMPLE_START.isoformat(), "end": IN_SAMPLE_END.isoformat()},
...
if __name__ == "__main__":
    print("s7 D1 in_sample_driver is BUILD-only scaffolding at P3.5.")
    print(f"Candidate: {CANDIDATE_RECORD_ID}")
    print(f"In-sample window: {IN_SAMPLE_START.isoformat()} -> {IN_SAMPLE_END.isoformat()}")
    print(f"Cache root: {CACHE_ROOT}")
    print()
    print("Running this script directly does NOT execute the backtest by default.")
    print("To execute (after explicit P6.5 operator authorization), call:")
    print("  from external_research_hunter.s7_d1_cross_asset_donchian_runner_harness "
          "import in_sample_driver")
    print("  result = in_sample_driver.run_in_sample(cost_tier='S1')")
    print()
    print("This script will now exit WITHOUT calling run_in_sample().")
