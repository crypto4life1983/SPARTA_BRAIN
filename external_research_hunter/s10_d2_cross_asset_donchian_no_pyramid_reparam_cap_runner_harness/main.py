"""s10 D2 cross-asset Donchian (NQ + GC + ZN + CL) -- algorithm runner.
...
Inherits Tier-N spec seal:    f5ca5ee63024e9c8b7b1e85762558ec35eb3d4ebb9ce2a160cbe0f1a0e80a679
Inherits plan-lock seal:      ba8bf954d44b373c0ffdc38a1cf8f73dec31a9fc538f0b7584a20659d79ea354
Inherits Phase-2 plan seal:   7a48ad64236971e6fd2a2fa58ab7ab746a71543778ed0a868f3cdf9ff8f74ac3
Predecessor (s10 selection):   007110ff5a57dd04b184409bad64fd667374cd049155416c869a73f7af0c1f97
"""
...
CONFIG = {
    ...
    "max_units_per_market":           1,   # S10-D2 DELTA: was 4 in s7-D1; no-pyramid invariant
    "pyramid_spacing_n_multiplier":   0.5, # vestigial under max_units=1; never fires (per S10-D2 plan-lock)
    ...
    "starting_cash_mnq_equivalent":   500_000,  # S10-D2 DELTA: was 100_000 in s8-D1
    "skip_if_contract_count_lt_one":  True,
    "portfolio_cap_max_units":        4,   # S10-D2 DERIVED: 4 markets x 1 unit; structurally non-binding
...
class PortfolioCapTracker:
    """Tracks total open UNITS across markets. S10-D2: Cap = 4 markets * 1 unit = 4.

    CRITICAL (s6 bugfix): update_market_units() MUST receive pyr.current_unit_count
    (the UNIT count, max 1 per market under s8-D1 no-pyramid), NOT pyr.total_quantity
    (the contract count).

    S10-D2 invariant: per_market_cap=1; any unit_count > 1 fails safety. This is
    EITHER the s6 contract-vs-unit-count bug OR a no-pyramid invariant violation.
    """

    def __init__(self, max_total_units: int = 4, per_market_cap: int = 1):
        ...
        self.per_market_unit_count_invariant_violation_count = 0

    def update_market_units(self, market: str, unit_count: int) -> None:
        if unit_count > self.per_market_cap:
            self.per_market_unit_count_invariant_violation_count += 1
            # Two possible causes under s8-D1:
            #   1) s6 bug: caller passed pyr.total_quantity (e.g. 10 contracts for NQ)
            #   2) S10-D2 no-pyramid invariant violation: per-market unit count exceeded 1
            raise ValueError(
                f"PortfolioCapTracker.update_market_units received unit_count={unit_count} "
                f"for market={market!r}; max per market is {self.per_market_cap}. "
                f"S10-D2 no-pyramid invariant: per-market open unit count must be 0 or 1. "
                f"This indicates EITHER the s6 portfolio-cap bug was re-introduced "
                f"(caller passed pyr.total_quantity instead of pyr.current_unit_count) "
                f"OR the no-pyramid invariant was violated."
            )
...
# Algorithm class
# ---------------------------------------------------------------------------
class TenD2CrossAssetDonchianNoPyramidReparamCapAlgo(_QCAlgorithmBase):
    """s10-D2 cross-asset Donchian NO-PYRAMID REPARAM-CAP algorithm.

    Strategy: locked Donchian-55/Exit-20 with NO PYRAMID (max_units_per_market=1)
    on NQ + GC + ZN + CL. NO filter (AMB6 NONE). 1 % portfolio sizing. 2N stop.

    Single delta from s8-D1: starting_cash_mnq_equivalent = $500,000 (was $100,000). All other locked
    parameters byte-equivalent.

    Safety: Phase 2 C1-C8 byte-equivalent inheritance + s6 cap-bugfix preserved.
    ...
    def Initialize(self):  # noqa: N802 (QC convention)
        ...
        if getattr(self, "LiveMode", False):
            raise Exception(
                "LIVE_PATH_DETECTED: s10-cross-asset-donchian-no-pyramid-reparam-cap-nq-gc-zn-cl "
                "is paper-only forever; refuse to run in live mode."
            )

        # ---- S10-D2 S-STOP-14: starting_cash invariant check ----
        if CONFIG["starting_cash_mnq_equivalent"] != 500_000:
            raise Exception(
                f"S10_D2_STARTING_CASH_INVARIANT_VIOLATION: "
                f"CONFIG['starting_cash_mnq_equivalent']={CONFIG['starting_cash_mnq_equivalent']} "
                f"!= expected 500_000. S10-D2 is locked to starting_cash=$500,000 per plan-lock; "
                f"any other value requires a fresh candidate_record_id (not s10-D2)."
            )
