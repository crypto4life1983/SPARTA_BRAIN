"""s19-d1 K13 walk-forward driver. The 5 fixed pre-committed UNSEARCHED DA22 folds. run_walk_forward() STUB.

K13 = VALIDATION of the LOCKED rule across folds, NOT optimization. NO per-fold refit. NO fold-scheme search.
NO cadence tuning. NO execution at BUILD. (Identical fold scheme to s18.)
"""
...
def run_walk_forward(*args, **kwargs):
    raise RuntimeError(
        ...
        "(Authorize s19-d1 ... P6.7 K13 walk-forward validation). P3 BUILD scope does NOT authorize any signal "
        "computation, backtest, or simulator run. K13 is VALIDATION of LOCKED params; no per-fold refit."
    )
