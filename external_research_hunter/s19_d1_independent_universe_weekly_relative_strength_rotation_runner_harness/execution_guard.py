"""s19-d1 execution guard. C1-C8 attestation; weekly RS rotation guards; K13 fold-scheme + no-refit guards. NO execution/order/fetch."""

EXPECTED_TIER_SPEC_SEAL = "648793b40995f9fb2678ed1cff874299f8a28dd14063a68464a84ee3cf2de76b"
EXPECTED_PLAN_LOCK_SEAL = "5832d5cb0ad7c3bad2f00b915c725792d3496dfef6ab1874beed45bbf74e9e8b"
EXPECTED_P2_PHASE2_PLAN_SEAL = "b0f5a5dd3f3acdfab1dda4acf474f481f84e1c1281c73bb350b08e85b2576080"
...
S18_D1_SEAL_FROZEN_COMMIT = "7e6aa36"  # parent OOS_CONFIRMED s18; frozen/untouched (this is a replication)

LOCKED_UNIVERSE = ["ORCL", "CSCO", "ADBE", "CRM", "AMD", "NFLX", "TMUS", "CMCSA", "MCD", "NKE", "LOW", "PEP",
...
def assert_no_forbidden_order_paths(config):
    if config.get("brokerage_model_name") != "NOT_APPLICABLE_CSV_SIMULATOR_AT_P6":
        raise Exception("BROKERAGE_MODEL_FORBIDDEN: s19-d1 P3 BUILD authorizes only CSV-simulator at P6.")
...
def assert_split_only_convention(config):
    ...
    for sym, ex, fac in (("NFLX", "2025-11-17", 10.0), ("HON", "2025-10-30", 1.061)):
...
def assert_universe_locked(config):
    if list(config.get("universe") or []) != LOCKED_UNIVERSE:
        raise Exception(f"UNIVERSE_DRIFT: CONFIG['universe']={config.get('universe')!r} expected 24-name s19 independent universe")
