"""P4 synthetic smoke battery for s19-d1 independent-universe weekly RS rotation runner. Authored at P3 BUILD; run at P4.

Synthetic data + invariant checks only. NO real CSV reads, NO signal computation on real data. Weekly R=5, top-8. REPLICATION of s18.
"""
...
def test_T2_runner_class_instantiable(runner_harness_module):
    ...
    assert algo.config["candidate_record_id"] == "s19-d1-independent-universe-weekly-relative-strength-rotation-24name-large-cap-long-history"
...
def test_T24_replication_of_s18_markers(runner_harness_module):
    c = runner_harness_module["main"].CONFIG
    assert c["replication_of_s18"] is True and c["edge_not_inherited_from_s18"] is True
    # params identical to s18 weekly (126-21 / R=5 / top-8)
